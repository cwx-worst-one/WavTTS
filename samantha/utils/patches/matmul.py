...
@triton.heuristics({"EVEN_K": lambda nargs: nargs["K"] % nargs["TILE_K"] == 0})
@triton.jit
def _sdd_kernel(
    A,
    B,
...
def sdd_matmul(a, b, trans_a, trans_b, trans_c, spdims, block, lut, widths, out=None):
    ...
    if out is None:
        c = torch.empty(
            (a.shape[0], lut.shape[0], block, block), dtype=a.dtype, device=a.device
        )
    else:
    ...
    count = 0
    while count < c.shape[1]:
        grid = [1, min(57344, c.shape[1] - count), c.shape[0]]
        _sdd_kernel[grid](
            a,
            b,
            c,
...
def dsd_lut(layout, block, step, trans, device):
    ...
    header = (
        torch.stack((offsets, segments, col_id, head_id), dim=1).view(-1).contiguous()
    )
    # create increments
...
def dds_matmul(a, b, trans_a, trans_b, trans_c, spdims, block, lut, width, out=None):
    return dsd_matmul(
        b, a, not trans_b, not trans_a, not trans_c, spdims, block, lut, width, out=out
    )
...
class _matmul(torch.autograd.Function):
    ...
    def forward(
        ...
        c = _matmul.fn[mode](
            a, b, trans_a, trans_b, trans_c, spdims, block, c_lut, c_width, out=out
        )
        # save for backward
...
class matmul:
    def __init__(
        self, layout, block, mode, device, trans_a=False, trans_b=False, trans_c=False
    ):
