...
def init_language():
    ...
    meta_punc_2_pwpp = dict(transition=1, pause=2, separation=3, silence=4)
    ...
    for language in language_list:
        ...
        if "phone_set" in module_var:
            if "phone_set" in language_info:
                language_info["phone_set"] = merge_list_as_set(
        ...
        if "tone_set" in module_var:
            if "tone_set" in language_info:
                language_info["tone_set"] = merge_list_as_set(
...
def export_language_summary():
    ...
    if "EN" in language_set:
        EN_tones_2_CMU = dict(
            (tone, str(int(tone) - 10)) for tone in language_set["EN"]["tone_set"]
        )
    else:
        EN_tones_2_CMU = None

    punc_2_pwpp = dict(
        (sil_symbols, meta_punc_2_pwpp[sil_level])
        for sil_level in silence_level
        for sil_symbols in punctuation[sil_level]
    )
