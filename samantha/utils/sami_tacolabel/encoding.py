...
from samantha.utils.sami_tacolabel.labels import sanity_check_label
    ...
    with open(lab_path, "r") as f:
        ...
        for i, line in enumerate(lines):
            ...
            metas, should_fix_previous = sanity_check_label(
                ...
                len(lines),
                num_meta=5,
                allow_fix=forced_refix,
            )
            if forced_refix and should_fix_previous:
                phone, tone, wordpost, wordcateg, prosody = (
                    ",",
                    "0",
                    "0.0 0.0 0.0 0.0",
                    "S",
...
def enc_fs_label(lab_path, meta, cfg):
    ...
    with open(lab_path, "r") as f:
        ...
        for i, line in enumerate(lines):
            metas = line.strip().split("\t")
            metas, should_fix_previous = sanity_check_label(
                metas, pre_metas, lab_path, i, len(lines), num_meta=6, allow_fix=True
            )
            if metas is None:
                return None
            else:
