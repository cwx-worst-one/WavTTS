...
def random_side_mask(tensor, min_mask_percentage=0.2, max_mask_percentage=0.8):
    ...
    assert (
        0 <= min_mask_percentage <= 1
    ), "min_mask_percentage should be between 0 and 1."
    assert (
        0 <= max_mask_percentage <= 1
    ), "max_mask_percentage should be between 0 and 1."
    assert (
        min_mask_percentage <= max_mask_percentage
    ), "min_mask_percentage should be less than or equal to max_mask_percentage."
    ...
    for _ in range(tensor.shape[0]):
        ...
        mask_percentage = (
            torch.rand(1).item() * (max_mask_percentage - min_mask_percentage)
            + min_mask_percentage
        )
