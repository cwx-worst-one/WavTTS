...
from samantha.components.ctiga.ops.layer_norm import DropoutAddLayerNorm
from samantha.components.ctiga.ops.rms_norm import DropoutAddRMSNorm, RMSNorm
...
class FlopsProfiler(object):
    """
    Measures the latency, number of estimated floating-point operations
    and parameters of each module in a PyTorch model.

    The flops-profiler profiles the forward pass of a PyTorch model and
    prints the model graph with the measured profile attached to each module.

    It shows how latency, flops and parameters are spent in the model and
    which modules or layers could be the bottleneck.

    It also outputs the names of the top k modules in terms of aggregated latency,
    flops, and parameters at depth l with k and l specified by the user.

    The output profile is computed for each batch of input.

    The DeepSpeed flops profiler can be used with the DeepSpeed runtime or
    as a standalone package.

    When using DeepSpeed for model training, the flops profiler can be configured in
    the deepspeed_config file and no user code change is required.

    If using the profiler as a standalone package, one imports the flops_profiler
    package and use the APIs.
    ...
    def start_profile(self, ignore_list=None):
        ...
        Extra attributes are added recursively to all the modules and
        the profiled torch.nn.functionals are monkey patched.

        Args:
            ignore_list (list, optional): the list of modules to ignore while profiling.
            Defaults to None.
        """
    ...
    def reset_profile(self):
        ...
        def get_param_count_and_ep(param):
            ...
            whether the layer is an MoE layer,
        ...
        def add_or_reset_attrs(module):
            ...
            for p in module.parameters():
                ...
                #  account other expert parallel groups
    ...
    def get_total_flops(self, as_string=False):
        ...
        Args:
            as_string (bool, optional): whether to output the flops as string.
                Defaults to False.
    ...
    def get_total_duration(self, as_string=False):
        ...
        Args:
            as_string (bool, optional): whether to output the duration as string.
                Defaults to False.
    ...
    def get_total_params(self, as_string=False):
        ...
        Args:
            as_string (bool, optional): whether to output the parameters as string.
                Defaults to False.
    ...
    #         if isinstance(module, MoE) and hasattr(
    ...
    def print_model_profile(
        ...
        Args:
            profile_step (int, optional): The global training step at which to profile.
                Note that warm up steps are needed for accurate time measurement.
            module_depth (int, optional): The depth of the model to which to
                print the aggregated module information. When set to -1,
                it prints information from the top to the innermost modules
                (the maximum depth).
            top_modules (int, optional): Limits the aggregated profile output to
                the number of top modules specified.
            detailed (bool, optional): Whether to print the detailed model profile.
            output_file (str, optional): Path to the output file.
                If None, the profiler prints to stdout.
        """
        if not self.started:
            return
        import os.path
        ...
        print(
            ...
            "bwd latency (backward propagation latency),\n"
            "step (weights update latency), "
            "iter latency (sum of fwd, bwd and step latency)\n"
        )
        line_fmt = "{:<70}  {:<8}"
        # if self.ds_engine:
        #     print(line_fmt.format("world size: ", self.ds_engine.world_size))
        #     print(
        #       line_fmt.format("data parallel size: ", self.ds_engine.dp_world_size)
        #     )
        #     print(
        #         line_fmt.format("model parallel size: ", self.ds_engine.mp_world_size)
        #     )
        #     print(
        #         line_fmt.format(
        #             "batch size per GPU: ",
        ...
        #     fwd_latency = (
        #         self.ds_engine.timers(FORWARD_GLOBAL_TIMER).elapsed(False) / 1000.0
        #     )
        ...
        #     bwd_latency = (
        #         self.ds_engine.timers(BACKWARD_GLOBAL_TIMER).elapsed(False) / 1000.0
        #     )
        #     step_latency = (
        ...
        #     print(
        #         line_fmt.format(
        #             f"bwd FLOPS per GPU = {bwd_factor:g} * fwd flops "
        #             "per GPU / bwd latency: ",
        ...
        #     print(
        #         line_fmt.format(
        #             f"fwd+bwd FLOPS per GPU = {bwd_factor + 1:g} * fwd flops "
        #             "per GPU / (fwd+bwd latency): ",
        #             flops_to_string(
        #                 (bwd_factor + 1) * total_flops / (fwd_latency + bwd_latency)
        #             ),
        #         )
        #     )
        ...
        #     print(
        #         line_fmt.format(
        #             f"FLOPS per GPU = {bwd_factor + 1:g} * fwd flops "
        #             "per GPU / iter latency: ",
        ...
        if detailed:
            ...
            print(
                ...
                "\nparams, percentage of total params, MACs, percentage of total MACs,"
                " fwd latency, percentage of total fwd latency, fwd FLOPS"
            )
            print(
                "\nNote: 1. A module can have torch.nn.module or torch.nn.functional"
                " to compute logits (e.g. CrossEntropyLoss). They are not counted as "
                "submodules, thus not to be printed out. However they make up the "
                "difference between a parent's MACs (or latency) and the sum of its "
                "submodules'.\n2. Number of floating-point operations is a theoretical"
                " estimation, thus FLOPS computed using that could be larger than the "
                "maximum system throughput.\n3. The fwd latency listed in the top "
                "module's profile is directly captured at the module forward function "
                "in PyTorch, thus it's less than the fwd latency shown above which is "
                "captured in DeepSpeed.\n"
            )
            print(self.model)
    ...
    def print_model_aggregated_profile(self, module_depth=-1, top_modules=1):
        """
        Prints the names of the top top_modules modules in terms of aggregated time,
        flops, and parameters at depth module_depth.

        Args:
            module_depth (int, optional): the depth of the modules to show.
                Defaults to -1 (the innermost modules).
            top_modules (int, optional): the number of top modules to show.
                Defaults to 1.
        """
        info = {}
        if not hasattr(self.model, "__flops__"):
            print(
                "no __flops__ attribute in the model, "
                "call this function after start_profile and before end_profile"
            )
            return
        ...
        print(
            f"Top {top_modules} modules in terms of params, "
            "MACs or fwd latency at different model depths:"
        )
...
def _scaled_dot_product_attention_compute(
    query, key, value, attn_mask=None, dropout_p=0.0, is_causal=False, scale=None
):
    bq, nq, tq, dq = query.shape
    bk, nk, tk, dk = key.shape
    assert bq == bk and nq == nk and dq == dk
    return _attn_flops(bq, tq, tk, nq, dq), 0
...
def _patch_functionals():
    ...
    F.scaled_dot_product_attention = wrapFunc(
        F.scaled_dot_product_attention, _scaled_dot_product_attention_compute
    )
...
def _reload_functionals():
    ...
    F.scaled_dot_product_attention = old_functions[
        F.scaled_dot_product_attention.__str__
    ]
...
def _attn_flops(bs, tq, tk, nhead, headdim):
    return (
        bs * nhead * tq * tk * headdim * 2
        + bs * nhead * tq * tk
        + bs * nhead * tq * tk * headdim * 2
    )


def _flash_self_attenion_forward_hook(
    flash_self_attn_module: Union[FlashSelfAttention, FlashSelfAttentionV2],
    input,
    output,
):
    flops = 0
    n_input = len(input)
    assert n_input >= 4
    qkv, causal, cu_seqlens, max_seqlen = input[:4]
    if qkv.ndim == 4:  # unpad
        is_unpad = True
    else:
        is_unpad = False
    if not is_unpad:
        ...
        assert not (cu_seqlens is None or max_seqlen is None)
        bs = len(cu_seqlens) - 1
        t = max_seqlen

    n, h = qkv.shape[-2:]

    flops += _attn_flops(bs, t, t, n, h)
    flash_self_attn_module.__flops__ += flops


def _flash_cross_attenion_forward_hook(
    flash_cross_attn_module: Union[FlashCrossAttention, FlashCrossAttentionV2],
    input,
    output,
):
    flops = 0
    ...
    if not is_unpad:
        ...
        bs, tq, tk = *q.shape[:2], kv.shape[1]
    else:
        assert not (cu_seqlens is None or cu_seqlens_k is None)
        assert not (max_seqlen is None or max_seqlen_k is None)
        bs, tq, tk = cu_seqlens.shape[0] - 1, max_seqlen, max_seqlen_k

    n, h = q.shape[-2:]

    flops += _attn_flops(bs, tq, tk, n, h)
    flash_cross_attn_module.__flops__ += flops


def _flash_layernorm_forward_hook(
    flash_layernorm_module: Union[nn.LayerNorm, DropoutAddLayerNorm], input, output
):
    if isinstance(flash_layernorm_module, nn.LayerNorm):
        return
    ...
    has_affine = getattr(flash_layernorm_module, "weight") is not None
    flops = 0
    if resid is not None:
        flops += x.numel()
    flops += x.numel() * (5 if has_affine else 4)
    flash_layernorm_module.__flops__ += flops


def _flash_rmsnorm_forward_hook(
    flash_rmsnorm_module: Union[RMSNorm, DropoutAddRMSNorm], input, output
):
    if isinstance(flash_rmsnorm_module, RMSNorm):
        assert len(input) == 1
        x = input[0]
        resid = None
    else:
    ...
    assert x.ndim in [2, 3]
    has_affine = getattr(flash_rmsnorm_module, "weight") is not None
    flops = 0
    if resid is not None:
        flops += x.numel()
    flops += x.numel() * (3 if has_affine else 2)
    flash_rmsnorm_module.__flops__ += flops
...
MODULE_HOOK_MAPPING = {
    ...
    RMSNorm: _flash_rmsnorm_forward_hook,
    RotaryEmbedding: _flash_rotaryemb_forward_hook,
}
...
def params_to_string(params_num, units=None, precision=DEFAULT_PRECISION):
    ...
    return (
        number_to_string(params_num, units=units, precision=precision)
        .replace("G", "B")
        .strip()
    )
...
def get_model_profile(
    ...
    .. code-block:: python
        ...
        flops, macs, params = get_model_profile(
            model=model, input_shape=(batch_size, 3, 224, 224))
        )

    Args:
        model ([torch.nn.Module]): the PyTorch model to be profiled.
        input_shape (tuple): input shape to the model. If specified,
            the model takes a tensor with this shape as the only positional argument.
        args (list): list of positional arguments to the model.
        kwargs (dict): dictionary of keyword arguments to the model.
        print_profile (bool, optional): whether to print the model profile.
            Defaults to True.
        detailed (bool, optional): whether to print the detailed model profile.
            Defaults to True.
        module_depth (int, optional): the depth into the nested modules.
            Defaults to -1 (the inner most modules).
        top_modules (int, optional): the number of top modules to print
            in the aggregated profile. Defaults to 3.
        warm_up (int, optional): the number of warm-up steps
            before measuring the latency of each module. Defaults to 1.
        as_string (bool, optional): whether to print the output as string.
            Defaults to True.
        output_file (str, optional): path to the output file.
            If None, the profiler prints to stdout.
    ...
    Returns:
        The number of floating-point operations, multiply-accumulate operations (MACs),
