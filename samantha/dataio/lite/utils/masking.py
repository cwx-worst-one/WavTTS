...
class WavMasking(DataBlock):
    """
    Apply masking to generate the x_ctx and l_ctx according to voicebox
    (https://dl.fbaipublicfiles.com/voicebox/paper.pdf, page 9, section "Training")
    1. Drop the audio sequence x with p=0.3 (set x_ctx to be 0)
    2. Drop the duration sequence l with p=0.2 (set l_ctx=0)
    3. If the sequence is not dropped, U[70, 100]% audio segment is dropped
        and U[10,100]% duration sequence is dropped
    ...
    def __init__(
        ...
        args:
            ...
            p_conditional_drop: the probability of dropping the whole (audio sequence,
                per-frame phone transcription) or (duration sequence, phone sequence)
            p_drop_audio_frames: the range of the proportion of dropped audio frames
            p_drop_from_0_audio_frame:
                the probability of dropping from the 0th audio frame
            st_drop_audio_frame:
                the start frame of the dropped segment other than the 0th frame
            p_drop_duration_frames:
                the range of the proportion of dropped duration frames
            p_drop_from_0_duration_frame:
                the probability of dropping from the 0th duration frame
            st_drop_duration_frame:
                the start frame of the dropped segment other than the 0th frame
        """
        super().__init__()
    ...
    def _mask_audio_frames(self, x, l=None):
        ...
        All frames that aligned to a phone are either entirelly masked
        or unmasked if l is given.
        args:
        ...
        if (
            ...
            assert (
                ...
                > self._st_drop_audio_frame
            ), "The start frame of the dropped segment is too late"
            while True:
                num_frames_drop = np.random.randint(
                    int(num_frames * self._p_drop_audio_frames[0]),
                    int(num_frames * self._p_drop_audio_frames[1]),
                ...
                if num_frames - num_frames_drop > self._st_drop_audio_frame:
                    drop_start = np.random.randint(
                        self._st_drop_audio_frame, num_frames - num_frames_drop
                    )
                    break
        # check the start and end frames of the dropped segment,
        # make sure all frames aligned to a phone are either entirely masked or unmasked
        if l is not None:
            frame_idx = 0
            for frames in l:
                ...
                if drop_end >= frame_idx and drop_end < frame_idx + frames:
                    ...
                    break
                frame_idx += frames
        x_ctx = torch.concatenate(
            (
                x[:drop_start, :],
                torch.ones((num_frames_drop, x.shape[1]), dtype=x.dtype).to(x.device)
                * self.padding_value,
                x[drop_start + num_frames_drop :, :],
            ),
            axis=0,
        )

        x_ctx_mask = torch.concatenate(
            (
                torch.zeros((drop_start), dtype=bool),
                torch.ones((num_frames_drop), dtype=bool),
                torch.zeros((num_frames - drop_start - num_frames_drop), dtype=bool),
            ),
            axis=0,
    ...
    def _mask_duration_frames(self, l):
        ...
        if (
            self._p_drop_from_0_duration_frame is not None
            and np.random.rand() < self._p_drop_from_0_duration_frame
        ):
            drop_start = 0
            num_frames_drop = np.random.randint(
                int(num_frames * self._p_drop_duration_frames[0]),
                int(num_frames * self._p_drop_duration_frames[1]),
            )
        else:
            assert (
                num_frames - int(num_frames * self._p_drop_duration_frames[0])
                > self._st_drop_duration_frame
            ), "The start frame of the dropped segment is too late"
            while True:
                num_frames_drop = np.random.randint(
                    int(num_frames * self._p_drop_duration_frames[0]),
                ...
                if num_frames - num_frames_drop > self._st_drop_duration_frame:
                    drop_start = np.random.randint(
                        self._st_drop_duration_frame, num_frames - num_frames_drop
                    )
                    break
        l_ctx = np.concatenate(
            (
                l[:drop_start],
                np.zeros((num_frames_drop), dtype=l.dtype),
                l[drop_start + num_frames_drop :],
            ),
            axis=0,
        )
        # get the mask for the dropped segment
        l_ctx_mask = np.concatenate(
            (
                np.zeros((drop_start), dtype=bool),
                np.ones((num_frames_drop), dtype=bool),
                np.zeros((num_frames - drop_start - num_frames_drop), dtype=bool),
            ),
            axis=0,
        )  # true for dropped frames
