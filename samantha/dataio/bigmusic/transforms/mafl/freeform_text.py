...
def _infer_freeform_text_from_style_text(style_tags: dict) -> FreeformText:
    ...
    def randomly_capitalize_word(word: str) -> str:
        word = word.replace("_", " ")
        cap_type = random.choice(["lower", "upper", "capitalize"])
        if cap_type == "lower":
            return word.lower()
        elif cap_type == "upper":
            return word.upper()
    ...
    def get_pairs(tags: dict) -> str:
        t = [
            (randomly_capitalize_word(k), [randomly_capitalize_word(i) for i in v])
            for k, v in tags.items()
            if v and k != "is_sinking"
        ]
        random.shuffle(t)
        return t

    def format_style_tags_1(tags: dict) -> str:
        t = get_pairs(tags)
        return "".join([f"[{k}: {', '.join(v)}]" for k, v in t])

    def format_style_tags_2(tags: dict) -> str:
        t = get_pairs(tags)
        return "[" + ", ".join([f"{k}: {', '.join(v)}" for k, v in t]) + "]"
    ...
    freeform_type = random.choice(["desc_1", "desc_2", "bag"])

    if freeform_type == "desc_1":
        return FreeformText(keywords=[], descriptions=[format_style_tags_1(style_tags)])
    elif freeform_type == "desc_2":
        return FreeformText(keywords=[], descriptions=[format_style_tags_2(style_tags)])
    else:
        return FreeformText(
            keywords=[],
            descriptions=[
                "[" + ", ".join([randomly_capitalize_word(w) for w in keywords]) + "]"
            ],
        )
