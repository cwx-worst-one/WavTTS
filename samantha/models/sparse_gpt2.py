...
layout_fn_dict = {
    ...
    "coarse_10s_layout_v4_3": coarse_10s_layout_fn_v4_3,
...
class GPT2Attention(nn.Module):
    def __init__(self, config, is_cross_attention=False, layer_idx=None):
        super().__init__()
        self.config = config
        self.sparse = config_get(config, "sparse_attn", True)
        ...
        if self.pos_type == "rope_attn":
            self.wpe = FixedAbsolutePositionEmbedding(
                max_positions, self.head_dim, self.pos_type, ffn_fp32
            )
            ...
            if self.sparse:
                ...
                if not hasattr(self, "full_mask"):
                    ...
                    def dense_mask(layout, block):
                        ...
                        h, t, d = layout.shape
                        mask = torch.zeros(
                            [h, t * block, d * block], device=layout.device
                        )
