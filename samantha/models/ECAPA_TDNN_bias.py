...
class ECAPA_TDNN(nn.Module):
    def __init__(self, in_dim, C, out_dim, bn_dim):
        super().__init__()

        self.conv1 = nn.Conv1d(in_dim, C, kernel_size=5, stride=1, padding=2, bias=True)
        self.relu = nn.GELU()
        ...
        self.layer4 = nn.Conv1d(3 * C, 1536, kernel_size=1, bias=True)
        self.attention = nn.Sequential(
            nn.Conv1d(4608, 256, kernel_size=1, bias=True),
            nn.GELU(),
            nn.GroupNorm(1, 256),
            nn.Tanh(),  # I add this layer
            nn.Conv1d(256, 1536, kernel_size=1, bias=True),
            nn.Softmax(dim=2),
        )
        self.bn5 = nn.GroupNorm(8, 3072)
        self.fc6 = nn.Linear(3072, bn_dim, bias=True)
        self.bn6 = nn.GroupNorm(4, bn_dim)
        self.fc7 = nn.Linear(bn_dim, out_dim, bias=True)
