...
from samantha.models.umm.umm_mkii import (
...
class Base(nn.Module):
    def __init__(self, config):
        ...
        self.encoder_layers = nn.Sequential(
            *[
                ConformerLayer(conformer_config, None, i)
        ...
        if self.config.get("enable_profile", False):
            ...
            def trace_handler(p):
                ...
                trace_name = "umm_profile_" + str(p.step_num) + ".pt.trace.json"
            ...
            self.profile = torch.profiler.profile(
                ...
                schedule=torch.profiler.schedule(
                    wait=50, warmup=10, active=3, repeat=1
                ),
    ...
    def pad_audio(self, x):
        rate = (
            int(self.config.sample_rate / self.config.frame_rate)
...
class Stage3(Stage2):
    ...
    def forward(self, input_dict):
        ...
        hidden_states = self.encoder_input_dropout(audio_feature)
        hehe = hidden_states.shape
        if hehe[1] % 32 != 0:
            print("fuck hidden states error")
        ...
        for i, m in enumerate(self.encoder_layers):
            if i == self.config.vq_layer_idx:
                ...
                hehe = hidden_states.shape
                if hehe[1] % 32 != 0:
                    print("fuck vq hidden states error")
    ...
    def _get_vq_ids(self, hidden_states, position_embeddings):
        ...
        for i, layer in enumerate(self.encoder_layers):
            ...
            # Returns: ((hidden_states, pos_emb), mask)
            hidden_states = layer(
                ((hidden_states, position_embeddings), None), is_training=True
            )[0][0]
