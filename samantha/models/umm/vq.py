...
class EuclideanCodebook(nn.Module):
    def __init__(
        ...
        ), "kmeans init is not compatible with multiple"
        " codebooks in distributed environment for now"
    ...
    def replace(self, batch_samples, batch_mask):
        for ind, (samples, mask) in enumerate(
            ...
            if sampled.size(0) < mask.sum().item():
                sampled = torch.cat(
                    [sampled, sampled[-(mask.sum().item() - sampled.size(0)) :]], dim=0
                )
    ...
    def forward(self, x, sample_codebook_temp=None, mask=None):
...
class VectorQuantize(nn.Module):
    def __init__(
        ...
        affine_param_batch_decay=0.99,
        affine_param_codebook_decay=0.9,
        # the v that controls optimistic vs pessimistic update for synchronous
        # update rule (21) https://minyoungg.github.io/vqtorch/assets/draft_050523.pdf
        sync_update_v=0.0,
    ):
        super().__init__()
        self.dim = dim
        ...
        # whether to use cross entropy loss to codebook as commitment loss
    ...
    def forward(self, x, indices=None, mask=None, sample_codebook_temp=None):
        orig_input = x
        ...
        codebook_forward_kwargs = dict(
            sample_codebook_temp=sample_codebook_temp, mask=mask
        )
        ...
        # used for (1) naturalspeech2 training residual vq latents to be
        # close to the correct codes and (2) cross-entropy based commitment loss
        ...
        if self.training:
            ...
            if self.has_codebook_orthogonal_loss:
                ...
                if self.orthogonal_reg_active_codes_only:
                    ...
                    ), "orthogonal regularization for only active codes not"
                    " compatible with multi-headed with separate codebooks yet"
...
class EMAEmbedding(nn.Module):
    def __init__(self, n_e, e_dim, decay=0.99, eps=1e-5):
        ...
        self.register_buffer("cluster_size", torch.zeros(n_e) + 8)
...
class EMAVectorQuantizer(nn.Module):
    def __init__(
        ...
        # ema for dist
        self.dist = dist
        self.proj_in = torch.nn.Linear(dim, codebook_dim, bias=False)
        self.proj_out = torch.nn.Linear(codebook_dim, dim, bias=False)
    ...
    def forward(self, z):
        z = self.proj_in(z)
        z_flattened = z.view(-1, self.codebook_dim)  # [b*h, c]
