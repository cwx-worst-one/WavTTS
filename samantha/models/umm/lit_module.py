...
class BestRQInference(BestRQ):
    def __init__(
        ...
        pattern = "hdfs://haruna/home/byte_speech_sv/data/speech/librilight"
        f"/embeddings/medium/16020565-step=110000/rank-{self.global_rank}-%05d.tar"
    ...
    def predict_step(self, batch, batch_idx: int):
        ...
        for batch_idx, e in enumerate(embeds):
            id = f"{batch['speaker_id'][batch_idx]}-{batch['chapter_id'][batch_idx]}-"
            f"{batch['utterance_id'][batch_idx]}-{batch['utterance_sub_id'][batch_idx]}"
            obj = {
                "__key__": id,
...
class Stage2VocoderOnly(pl.LightningModule):
    ...
    def load_required_modules(self):
        print("Loading bestrq_mel_ctc_vq")
            ...
            if k.startswith("model.shared_encoder.layers"):
                ...
                if k_i < c.num_hidden_layers:
                    ...
                    if k_i < c.num_syllable_pre_layers + c.num_hidden_layers:
                        k_new = "model.syllable_pre_"
                        f"layers.{k_i - c.num_hidden_layers}.{k_suffix}"
                    elif (
                        k_i
                        ...
                        k_new = "model.syllable_post_layers"
                        f".{k_i - (c.num_syllable_pre_layers + c.num_hidden_layers)}."
                        f"{k_suffix}"
                    state_dict[k_new] = state_dict[k]
                    print(f"[MSD] {k} -> {k_new}")
                    if k_i < c.num_chroma_pre_layers + c.num_hidden_layers:
                        k_new = "model.chroma_pre_layers"
                        f".{k_i - c.num_hidden_layers}.{k_suffix}"
                    elif (
                        k_i
                        ...
                        k_new = "model.chroma_post_layers"
                        f".{k_i - (c.num_chroma_pre_layers + c.num_hidden_layers)}."
                        f"{k_suffix}"
                    state_dict[k_new] = state_dict[k]
                    print(f"[MSD] {k} -> {k_new}")
                del state_dict[k]
    ...
    def predict(self, input_ids, temperature=1.0, sample_mode="gumbel"):
        ...
        for _ in pbar:
            ...
            if input_ids.item() == self.extra_params.eos_id:
                print(
                    f"[UnifiedMirModel] EOS({self.extra_params.eos_id}) "
                    "sampled, finish generation",
                    flush=True,
                )
                break
