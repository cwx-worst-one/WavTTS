    ...
    def __init__(
        ...
        if self.text_encoder_type in ["flan-T5-large", "byte-T5-base"]:
            ...
            self.text_linear = nn.Linear(
                self.text_encoder.config.d_model, self.params.dim, bias=False
            )
        ...
        if use_cache:
            ...
            if self.text_encoder is not None:
                ...
                bpe_mask = sequence_mask_binary(
                    bpe_lens, max_len=max_bpe_len, device=bns.device
                )
...
class VAELLaMaLangSpkSerAli(LLaMa):
    ...
    def forward(
        ...
        if prompt_bns.shape[1] > 0:
            ...
            prompt_bn_in_z = torch.zeros([1, 0, self.params.out_dim]).to(
                prompt_bns.device
            )
            ...
            if self.text_encoder is not None:
                ...
                for i in range(bsz):
                    ...
                    if prompt_bn_lens[i] > 0:
                        h[i, : prompt_bn_lens[i], :] = prompt_bn_in_h[
                            i, : prompt_bn_lens[i], :
                        ]
                        h[
                            i, prompt_bn_lens[i] : prompt_bn_lens[i] + text_lens[i], :
                        ] = (
                            token_in_h[i, : text_lens[i], :]
                            + attn_bpe_in_h[i, : text_lens[i], :]
                        )
            ...
            for i in range(bsz):
                if spk_shift > 0:
                    if prompt_bn_lens[i] > 0:
                        h[
                            i,
                            prompt_bn_lens[i]
                            + text_lens[i] : prompt_bn_lens[i]
                            + text_lens[i]
                            + spk_shift,
                            :,
                        ] = spk_embeds[i]
                    else:
                        h[i, text_lens[i] : text_lens[i] + spk_shift, :] = spk_embeds[i]
                ...
                if prompt_bn_lens[i] > 0:
                    h[
                        ...
                        + text_lens[i]
                        + spk_shift
                        + ser_tag_shift : prompt_bn_lens[i]
                        + text_lens[i]
                        + spk_shift
                        + ser_tag_shift
                        + bn_lens[i],
