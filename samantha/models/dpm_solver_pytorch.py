...
class NoiseScheduleVP:
    ...
    def marginal_log_mean_coeff(self, t):
        ...
        if self.schedule == "discrete":
            ...
            log_alpha_fn = lambda s: torch.log(  # noqa: E731
...
class DPM_Solver:
    ...
    def get_orders_and_timesteps_for_singlestep_solver(
        ...
        if skip_type == "logSNR":
            ...
            timesteps_outer = timesteps_outer[
    ...
    def denoise_fn(self, x, s):
        """
        Denoise at the final step, which is equivalent to solve the ODE from
        lambda_s to infty by first-order discretization.
        """
    ...
    def singlestep_dpm_solver_update(
        ...
        Args:
            ...
            return_intermediate: A `bool`.
                If true, also return the model value at time `s`, `s1` and `s2` (the intermediate times).
            solver_type: either 'dpm_solver' or 'taylor'. The type for the high-order solvers.
                The type slightly impacts the performance. We recommend to use 'dpm_solver' type.
    ...
    def dpm_solver_adaptive(
        ...
        Args:
            ...
            t_err: A `float`. The tolerance for the time. We solve the diffusion ODE until the absolute error between
                the current time and `t_0` is less than `t_err`. The default setting is 1e-5.
            solver_type: either 'dpm_solver' or 'taylor'. The type for the high-order solvers.
                The type slightly impacts the performance. We recommend to use 'dpm_solver' type.
        Returns:
            x_0: A pytorch tensor. The approximated solution at time `t_0`.

        [1] A. Jolicoeur-Martineau, K. Li, R. Piché-Taillefer, T. Kachman, and I. Mitliagkas, "Gotta go fast when
        ...
        if order == 2:
            r1 = 0.5
            lower_update = lambda x, s, t: self.dpm_solver_first_update(  # noqa: E731
                x, s, t, return_intermediate=True
            )
            higher_update = lambda x, s, t, **kwargs: self.singlestep_dpm_solver_second_update(  # noqa: E731
                x, s, t, r1=r1, solver_type=solver_type, **kwargs
            )
        elif order == 3:
            r1, r2 = 1.0 / 3.0, 2.0 / 3.0
            lower_update = (
                lambda x, s, t: self.singlestep_dpm_solver_second_update(  # noqa: E731
                    x, s, t, r1=r1, return_intermediate=True, solver_type=solver_type
                )
            )
            higher_update = lambda x, s, t, **kwargs: self.singlestep_dpm_solver_third_update(  # noqa: E731
        ...
        while torch.abs((s - t_0)).mean() > t_err:
            ...
            norm_fn = lambda v: torch.sqrt(  # noqa: E731
    ...
    def sample(
        ...
        We support the following algorithms for both noise prediction model and data prediction model:
            - 'singlestep':
                Singlestep DPM-Solver (i.e. "DPM-Solver-fast" in the paper), which combines different orders of
                    singlestep DPM-Solver.
                We combine all the singlestep solvers with order <= `order` to use up all the function
                    evaluations (steps).
                ...
                Given a fixed NFE == `steps`, the sampling procedure is:
                    ...
                    - If `order` == 3:
                        - Denote K = (steps // 3 + 1). We take K intermediate time steps for sampling.
                        - If steps % 3 == 0, we use (K - 2) steps of singlestep DPM-Solver-3, and 1 step of singlestep
                            DPM-Solver-2 and 1 step of DPM-Solver-1.
                        - If steps % 3 == 1, we use (K - 1) steps of singlestep DPM-Solver-3 and 1 step of DPM-Solver-1.
                        - If steps % 3 == 2, we use (K - 1) steps of singlestep DPM-Solver-3 and 1 step of singlestep
                            DPM-Solver-2.
            - 'multistep':
                ...
                Given a fixed NFE == `steps`, the sampling procedure is:
                    ...
                    - If `order` == 3:
                        - We firstly use 1 step of DPM-Solver-1, then 1 step of multistep DPM-Solver-2, then (K - 2)
                            step of multistep DPM-Solver-3.
            - 'singlestep_fixed':
                Fixed order singlestep DPM-Solver (i.e. DPM-Solver-1 or singlestep DPM-Solver-2 or singlestep
                    DPM-Solver-3).
            ...
            - 'adaptive':
                ...
                You can adjust the absolute tolerance `atol` and the relative tolerance `rtol` to balance the
                    computatation costs
                (NFE) and the sample quality.
                    - If `order` == 2, we use DPM-Solver-12 which combines DPM-Solver-1 and singlestep DPM-Solver-2.
                    - If `order` == 3, we use DPM-Solver-23 which combines singlestep DPM-Solver-2 and singlestep
                        DPM-Solver-3.
        ...
        Args:
            ...
            skip_type: A `str`. The type for the spacing of the time steps. 'time_uniform' or 'logSNR' or
                'time_quadratic'.
            ...
            solver_type: A `str`. The taylor expansion type for the solver. `dpm_solver` or `taylor`. We recommend
                `dpm_solver`.
        ...
        if method == "adaptive":
            ...
            for i, order in enumerate(orders):
                ...
                vec_s, vec_t = t_T_inner.repeat(x.shape[0]), t_0_inner.repeat(
                    x.shape[0]
                )
