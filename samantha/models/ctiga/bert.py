...
from samantha.components.ctiga.embedding import BertEmbeddings
from samantha.components.ctiga.mha import MHA
from samantha.components.ctiga.mlp import FusedMLP, Mlp
from samantha.utils.ctiga.padding import (
    index_first_axis,
...
try:
    from samantha.criterion.cross_entropy import CrossEntropyLoss
except ImportError:
...
def create_mixer_cls(config, cross_attn=False, return_residual=False):
    ...
    flashattn_version = getattr(config, "flash_attn_version", 1)
    mixer_cls = partial(
        MHA,
...
def create_mlp_cls(config, layer_idx=None, return_residual=False):
    ...
    if not fused_mlp:
        approximate = (
            "tanh" if config.hidden_act in ["gelu_new", "gelu_fast"] else "none"
        )
        mlp_cls = partial(
            Mlp,
...
class BertPreTrainedModel(nn.Module):
    ...
    def from_pretrained(cls, model_name, config, *inputs, **kwargs):
        ...
        load_return = model.load_state_dict(
            remap_state_dict(state_dict_from_pretrained(model_name), config),
            strict=False,
        )
...
class BertForPreTraining(BertPreTrainedModel):
    ...
    def forward(
        ...
        """
        masked_tokens_mask = (
            labels > 0 if (self.last_layer_subset and labels is not None) else None
        )
        outputs = self.bert(
            input_ids,
