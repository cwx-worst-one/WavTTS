# Copyright (c) 2023, EleutherAI, ByteDance
#
...
class GradientNoiseScale:
    """
    A class to measure the gradient noise scale of a model while training
    (cf. https://arxiv.org/abs/1812.06162).

    The core thesis of the paper is that, if our batch size is small, there will
    be a lot of noise present in the gradients, and we might update our weights
    only on noise. After several updates the optimizer may still push us in the
    right direction, but we would be better off having used a larger batch size,
    which is more computationally efficient and directly averages out the noise
    in the gradients.

    But there's a limit to the gains large batch sizes can give you - if, after
    a certain batch size, your gradient is already accurate, there's no point in
    increasing the batch size further, as we'll just be wasting compute for little
    to no gain in accuracy.

    This means there is some theoretically optimal batch size for a given model,
    which measuring the gradient noise scale can help us to estimate.

    To estimate the 'simple' noise scale (Bsimple), we need to have a measure of
    the gradients using a large batch size (Bbig) and a small batch size (Bsmall).
    ...
    - With multi-gpu training, we can do this by taking the gradients of the
    microbatch_size_per_gpu for Bsmall,
    and the gradients of the entire batch for Bbig.
    - Alternatively, we can just take Bsmall as a single batch, and Bbig as several
    sequential batches in a row.

    This is the option we've opted for in this implementation because a) it's
    easier to implement and b) also works in single-gpu environments.
    Unfortunately it does come with some memory overhead.
    """
    ...
    def __init__(
        ...
        self.beta = beta
        self.fdsp_strategy = fdsp_strategy
        self.model = model
        self.buffer = None
        self.ema_scale = None
        self.ema_noise = None
        ...
        self.is_pipe_parallel = is_pipe_parallel
    ...
    def flatten_grads(self):
        grads = []
        if self.fdsp_strategy:
            # TODO: Verify that we can use this instead of the state_dict,
            # as .parameters() does not contain buffers (e.g., BatchNorm mean/vars)
    ...
    def update(self):
        if self.is_pipe_parallel:
            ...
            # for mp / dp only, the grads will be the same across all ranks,
            # so we can just do the process on a single rank
