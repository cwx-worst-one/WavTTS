...
from ..triton.k_softmax import _softmax, _softmax_backward

# CREDITS: This is adapted from the vanilla Triton example. See:
# 1. https://openai.com/blog/triton/
# 2. https://triton-lang.org/getting-started/tutorials/02-fused-softmax.html
...
def softmax(
    ...
    Args:
        x: input tensor.
        mask: optional mask, its application will be fused to the softmax computation
            if triton is used

        causal: optional performance optimization, if triton is used and the attention
            is causal
...
def log_softmax(
    ...
    .. math::
        \text{LogSoftmax}(x_{i}) = \log\left(\frac{\exp(x_i) }{ \sum_j \exp(x_j)} \right) # noqa
...
def _softmax_dispatch(
    ...
    # Triton is used if
    # 1. CUDA is enabled
    # 2. There's enough data to make it faster than PyTorch
    # 2a. or Triton benchmarks improve
    # 3. There was no previous failure
    ...
    try:
        ...
        # Catch cases where the current GPU does not have enough registers to hold a
        # full tensor line fallback to PyTorch's implementation, which streams
        # the tensor in and out
        _triton_registered_warnings = True
        logger.warning(
            "Triton softmax kernel register spillover or invalid image"
            " caught.Deactivating this kernel, please file an issue int the"
            " samantha repository"
        )
        logger.warning(e)
