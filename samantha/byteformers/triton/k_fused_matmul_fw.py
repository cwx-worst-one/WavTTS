...
from .k_activations import gelu, leaky_relu, relu
...
def kernel_fma(
    ...
    # then multiple cols/rows in the result would end up pulling in the same
    # row and lines from the inputs. By grouping the computation we ensure some
    # data reuse, which the hardware covers via the L2 cache
    ...
    # We fetch a block memory block from both inputs, matmul and accumulate,
    # then repeat
    mask_rn = rn < N
    mask_rm = rm < M
    ...
    for i in range(0, K, BLOCK_K):
        ...
        a = tl.load(
            input_ptrs + rk[None, :],
            mask=((rk[None, :] < K) & mask_rm[:, None]),
            other=0.0,
        )
        w = tl.load(
            weight_ptrs + rk[:, None],
            mask=((rk[:, None] < K) & mask_rn[None, :]),
            other=0.0,
        )
...
def fused_matmul(
    ...
    ), "Incompatible dimensions in between inputs and weight {}".format(
        x_.shape - weight.shape
    )
    ...
    act_inputs = (
        torch.empty_like(outputs) if save_act_inputs else x
