...
BLOCK_M = 32
BLOCK_N = 64  # TODO: This should ideally be GPU dependent, big impact on perf
...
class _dropout(torch.autograd.Function):
    ...
    def backward(
        ...
        # - over N we compromise in between trying to use as much memory
        # paralellism as possible, (fill in the warps, there are 32 threads per
        # warps, and 4 warps default), and not being too big because of register
        # spilling
        N_BLOCKS_M = triton.cdiv(M, BLOCK_M)

        if ctx.trainable_bias:
            grad_bias = torch.empty(
                (N_BLOCKS_M, N), device=grad_in.device, dtype=grad_in.dtype
            )
        ...
        def grid(meta):
            # NOTE: We use Triton Philox random number generator, which
            # optimally generates 4 blocks for a given seed and offsets.
            # "BLOCK_M" here describes the size of one of these blocks
            # but we need to take this factor of 4 into account when scheduling
            # all the kernels
...
def dropout(
    ...
    return _dropout.apply(
        x, float(p), bias, activation_index, bias is not None and bias.requires_grad
    )
...
class FusedDropoutBias(torch.nn.Module):
    ...
    def __init__(
        ...
        assert self.p < 1.0, (
            f"We don't want to drop all the values, most probably p={p} is not"
            " properly set"
        )
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # This kernel is slower than pytorch for small buffers,
        # bypassing it in that case
