...
# autotune: Triton will test out these configurations, and automatically pick
# the fastest one.

# heuristic: add arguments to the kernel call automatically given some heuristics.
# These arguments are passed in "meta"
...
def _softmax(
    ...
    The softmax is applied over the last dimension, meaning that this is
    equivalent to torch.softmax(tensor, dim=-1)

    Note, if the last dimension is large, say 128K elements, the kernel compile
    time can shot up to many minutes when the kernel is run for the first time.
    """
    ...
    # the memory address of all the elements that we want to load can be
    # computed as follows:
...
def _softmax_backward(
    ...
    ..Note: Not autotuning for now because this would lead to broken accumulated
    gradients
    """
