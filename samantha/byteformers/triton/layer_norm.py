...
from .k_layer_norm import layer_norm_bwd_dwdb, layer_norm_bwd_dx_fused, layer_norm_fw
...
class _LayerNorm(torch.autograd.Function):
    ...
    def backward(
        ...
        assert (
            dy.numel() == x.numel()
        ), "Something is wrong in the backward graph, possibly because of an inplace operation after the layernorm"  # noqa
        ...
        layer_norm_bwd_dwdb[grid](
            _dw, _db, dw, db, GROUP_SIZE_M, N, BLOCK_SIZE_M=32, BLOCK_SIZE_N=64
        )
        # fmt: on
...
class FusedLayerNorm(nn.Module):
    ...
    .. _torch.nn.LayerNorm: https://pytorch.org/docs/stable/generated/torch.nn.LayerNorm.html # noqa

    """

    def __init__(self, ndim: int, bias: bool, affine=True, eps=1e-06):
        super().__init__()
        if affine:
...
def layer_norm(
    ...
    try:
        ...
        # full tensor line fallback to PyTorch's implementation, which streams the
        # tensor in and out
        _triton_registered_warnings = True
        logger.warning(
            "Triton layernorm kernel register spillover or invalid image caught."
            " Deactivating this kernel, please file an issue in the samantha"
            " repository"
        )
        logger.warning(e)
