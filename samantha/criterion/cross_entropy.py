# Inspired by https://github.com/NVIDIA/apex/blob/master/apex/transformer/tensor_parallel/cross_entropy.py # noqa
# But we make it much faster: we compute the local loss and the LSE, and by exchanging
# the LSE and the losses we can get the global loss. There's no need to do it step by
# step (compute local max, exchange, compute exp, compute local sum, exchange, etc.)
# The original xentropy interface is here:
...
class SoftmaxCrossEntropyLossFn(torch.autograd.Function):
    ...
    def forward(
        ...
        labels: (batch,)
        If process_group is not None, we're doing Tensor Parallel: each process is
        responsible for one part of the vocab. The loss needs to be aggregated across
        processes.
        """
        ...
        if world_size == 1:
            ...
            # For tensor parallel cross entropy with smoothing, we want to pass in the
            # total number of classes so that smoothing can be applied correctly. If
            # total_classes=-1, use the last dimension of the input tensor.
            ...
            # If there's no smoothing, if labels are in the vocab of this partition,
            # losses contains lse_local - predicted logit, and 0 otherwise.
            # If there's smoothing=0.1, for labels in the vocab of this partition,
            # losses contains 0.9 * (lse_local - predicted logit) +
            ...
            # 0.9 * (lse_local - predicted_logit) +
            # 0.1 * (sum of all lse_local - sum logit / total_classes)
            # We want 0.9 * (lse - predicted_logit) +
            # 0.1 * (lse - sum logit / total_classes).
