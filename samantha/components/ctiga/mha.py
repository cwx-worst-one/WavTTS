...
try:
    # flash_attn
    from .ops.flash_attn_interface import (
...
class FlashCrossAttention(nn.Module):
    ...
    def forward(
        ...
        if unpadded:
            ...
            if self.triton and (
                self.drop.p == 0.0 or not self.training
            ):  # Triton version doesn't support dropout
...
class FlashSelfAttentionV2(nn.Module):
    """Implement the scaled dot product attention with softmax.
    Arguments
    ---------
        softmax_scale: The temperature to use for the softmax attention.
...
class FlashCrossAttentionV2(nn.Module):
    """Implement the scaled dot product attention with softmax.
    Arguments
    ---------
        softmax_scale: The temperature to use for the softmax attention.
...
class FlashBlocksparseSelfAttention(nn.Module):
    """Implement the scaled dot product attention with softmax.
    Arguments
    ---------
        softmax_scale: The temperature to use for the softmax attention.
    ...
    def __init__(
        self, blockmask, causal=False, softmax_scale=None, attention_dropout=0.0
    ):
        super().__init__()
        self.causal = causal
        self.softmax_scale = softmax_scale
        self.dropout_p = attention_dropout
        self.register_buffer("blockmask", blockmask)
        blockmask_converted = convert_blockmask(self.blockmask)
        self.register_buffer("blockmask_converted", blockmask_converted)
        print(
            f"Attention class {self.__class__}: sparsity={self.blockmask.float().mean()}"
        )
    ...
    def forward(self, qkv, cu_seqlens=None, max_seqlen=None):
        ...
        if cu_seqlens is None:
            ...
            assert seqlen_rounded // 16 <= self.blockmask.shape[0], (
                seqlen_rounded // 256 <= self.blockmask.shape[1]
            )
            # blockmask = self.blockmask[: seqlen_rounded // 16, : seqlen_rounded // 256]
...
class BlocksparseSelfAttention(nn.Module):
    ...
    def forward(
        ...
        output = torch.einsum("bhts,bshd->bthd", attention_drop, v)
        if key_padding_mask:
            output.masked_fill_(rearrange(~key_padding_mask, "b s -> b s 1 1"), 0)
