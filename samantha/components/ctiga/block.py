...
class Block(nn.Module):
    def __init__(
        ...
        if mixer_cls is None:
            mixer_cls = partial(MHA, num_heads=dim // 64, version=version)
        if mlp_cls is None:
        ...
        if self.fused_dropout_add_ln:
            assert (
                dropout_add_layer_norm is not None
            ), "dropout_layer_norm is not installed"
            assert (
                dropout_add_rms_norm is not None
            ), "dropout_layer_norm is not installed"
        ...
        if self.prenorm:
            ...
            if not self.fused_dropout_add_ln:
                if self.drop_path1 is not None:
                    ...
                    hidden_states = self.norm1(
                        (self.dropout1(mixer_out) + hidden_states).to(
                            dtype=self.norm1.weight.dtype
                        )
                    )
            else:
                ...
                if not self.fused_dropout_add_ln:
                    if self.drop_path2 is not None:
                        hidden_states = self.norm2(
                            ...
                            ).to(dtype=self.norm2.weight.dtype)
                        )
                    else:
                        hidden_states = self.norm2(
...
class ParallelBlock(nn.Module):
    ...
    def __init__(
        ...
        self.residual_in_fp32 = residual_in_fp32
        if mixer_cls is None:
            mixer_cls = partial(MHA, num_heads=dim // 64, version=self.version)
        if mlp_cls is None:
            mlp_cls = partial(Mlp, hidden_features=4 * dim)
        self.mixer = mixer_cls(dim)
        self.dropout1 = dropout_cls(resid_dropout1)
