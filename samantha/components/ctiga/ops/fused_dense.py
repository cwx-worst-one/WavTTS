...
from samantha.components.activations import gelu_bwd, relu_bwd, sqrelu_bwd, sqrelu_fwd
from samantha.utils.distributed import (
    all_gather_raw,
    all_reduce,
    all_reduce_raw,
    reduce_scatter,
...
class FusedDense(nn.Linear):
    ...
    @classmethod
    def from_torch(cls, torch_module: nn.Linear, keep_params=False):
        module = cls(
            in_features=torch_module.in_features,
            out_features=torch_module.out_features,
            bias=torch_module.bias is not None,
        )
        if keep_params:
            module.weight.data = torch_module.weight.data
            module.bias.data = torch_module.bias.data
        return module
        ...
        if self.heuristic == "auto":
            if self.activation == "gelu_approx":
                if torch.cuda.get_device_capability("cuda") == (9, 0):
                    ...
                    torch.cuda.get_device_capability("cuda") == (7, 0)
                    and dtype == torch.float16
                ):
                    # FIXME: In v100, heuristic=1 will get big numerical error
                    heuristic = -1
                else:
                    ...
                    heuristic = (
                        0
                        if cuda_ver >= (11, 8)
                        else (1 if dtype == torch.float16 else -1)
                    )
            else:
                heuristic = 0
        else:
