...
from samantha.components.tokenizer import w2v_bert_tokenization
from samantha.utils.audio import RandomResizedCrop
from samantha.utils.mulan_tag import MulanTagger
...
class MulanTagEmbedder(ContinuousEmbedder):
    ...
    def get_embeds(self, requires, input_audio, data_type=None):
        if data_type == "text":
            ...
            metadata = self.mulan_tagger.get_tags(
                requires, audio_embeds=mulan_audio_embeds
            )
...
class BestRQTokenEmbedder(TokenEmbedder):
    def __init__(
        self, vocab_size=32_768, embedding_dim=1024, add_sos=False, add_eos=False
    ):
        super().__init__(vocab_size, embedding_dim, add_sos, add_eos)

    def get_tokens(self, requires, input_audio):
        bestrq_umm_tokens = get_bestrq_umm_tokens(requires, input_audio)
        # print(f"bestrq_umm_tokens: {bestrq_umm_tokens}")
        return bestrq_umm_tokens
