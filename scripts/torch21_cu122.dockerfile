...
RUN apt-get update && \
    ...
    ln -sfn "/usr/share/zoneinfo/$ETC_LOCALTIME" /etc/localtime; \
    # Generate locales
...
ARG CRUISE_VERSION=1.0.0.3609
ARG PANTHER_VERSION=1.7.14.395
...
RUN apt-get update && \
    ...
    apt-get update && \
    apt-get install -y cuda-nsight-compute-12-2 \
...
RUN pip3 install --no-cache-dir --use-pep517 --no-deps \
...
RUN mkdir -p /tmp/py_pkg.dataloader && \
    cd /tmp/py_pkg.dataloader && \
    ...
    pip3 install --no-cache-dir --no-deps byted_dataloader*cp39*.whl && \
...
RUN \
    ...
    pip3 install --no-cache-dir --force-reinstall --no-deps dist/asr_eval_tool-$ASR_EVAL_TOOL_VERSION-py3-none-any.whl && \
...
RUN \
    ...
    cd /tmp/py_pkg.speech_evals && \
    python3 setup.py bdist_wheel && \
    pip3 install --no-cache-dir --force-reinstall --no-deps dist/speech_evals-$SPEECH_EVALS_VERSION-py3-none-any.whl && \
