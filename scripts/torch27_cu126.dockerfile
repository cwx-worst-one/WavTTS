...
FROM aliyun-va-hub.byted.org/compile/seed.speech.pytorch27:2cb59cee757ff3db8f860761a48bf48a AS aliyun_va
FROM use-hub.byted.org/compile/seed.speech.pytorch27:2cb59cee757ff3db8f860761a48bf48a AS us-east
FROM hub.tiktoke.org/compile/seed.speech.pytorch27:2cb59cee757ff3db8f860761a48bf48a AS us-east-red
FROM aliyun-sin-hub.byted.org/compile/seed.speech.pytorch27:2cb59cee757ff3db8f860761a48bf48a AS aliyun_sg
FROM hub.byted.org/compile/seed.speech.pytorch27:2cb59cee757ff3db8f860761a48bf48a AS china-north-lf
...
ARG S3A_VERSION=1.0.0.83
ARG MARIANA_FMHA_PLUS_VERSION=1.0.0.62
...
# Not installing mpi4py as it causes GCC to hang..
# mpi4py==4.0.3

# ByteDance packages, a pain in the xx.
RUN pip3 install \
        --no-cache-dir \
        --no-deps \
...
# 2. install falconclaw && dataloader
# install bytedkms==3.2.10, bytedkms must installed before cruise.
RUN mkdir -p /tmp/py_bytedkms && \
    cd /tmp/py_bytedkms && \
    wget -q http://luban-source.byted.org/repository/scm/seed.speech.scm_packer_$BYTEDKMS_VERSION.tar.gz && \
...
RUN pip3 uninstall -y s3a && \
    wget -q http://luban-source.byted.org/repository/scm/lab_audio.seed.s3a_$S3A_VERSION.tar.gz && \
    mkdir tmp.s3a && \
    tar -xvf lab_audio.seed.s3a_$S3A_VERSION.tar.gz -C tmp.s3a && \
    pip3 install --no-cache-dir --no-deps tmp.s3a/s3a*.whl && \
    ftp_proxy=http://sys-proxy-rd-relay.byted.org:8118 \
    http_proxy=http://sys-proxy-rd-relay.byted.org:8118 \
    https_proxy=http://sys-proxy-rd-relay.byted.org:8118 \
    no_proxy=code.byted.org,anaconda.org \
...
RUN \
    ...
    wget -q http://luban-source.byted.org/repository/scm/lab.speech.asr_eval_tool_$ASR_EVAL_TOOL_VERSION.tar.gz && \
    ...
    wget -q http://luban-source.byted.org/repository/scm/lab.speech.I18N_text_format_$I18N_TEXT_FORMAT_VERSION.tar.gz && \
...
RUN \
    ...
    wget -q http://luban-source.byted.org/repository/scm/lab.speech.speech_evals_$SPEECH_EVALS_VERSION.tar.gz && \
...
RUN \
    ...
    wget -q http://luban-source.byted.org/repository/scm/data.speech.flash_attn_plus_$MARIANA_FMHA_PLUS_VERSION.tar.gz && \
...
RUN mkdir -p /tmp/py_xformers && \
    cd /tmp/py_xformers && \
    wget -q http://luban-source.byted.org/repository/scm/seed.speech.scm_packer_$XFORMERS_VERSION.tar.gz && \
...
ARG BUMI_VERSION=2.5.0.34
ARG LSDP_VERSION=1.4.0.48
ARG OMNISTORE_VERSION=1.0.0.56
ARG OMNIDISPATCHER_VERSION=1.0.0.33
ARG FLASH_ATTENTION_V3_VERSION=1.0.0.34

# TODO(lanzongwei.lan): remove this after omnistore release their new version.
# Install omnistore
RUN scm_install data.aml.omnistore_test $OMNISTORE_VERSION
RUN scm_install seed.speech.OmniDispatcher $OMNIDISPATCHER_VERSION
RUN scm_install data.aml.cruise $CRUISE_VERSION
RUN scm_install lab.speech.openfst_python $OPENFST_VERSION
RUN scm_install seed.speech.bumi $BUMI_VERSION
RUN scm_install seed.speech.lsdp $LSDP_VERSION
RUN scm_install seed.speech.flash_attention_v3 $FLASH_ATTENTION_V3_VERSION

# Install bleurt.
RUN mkdir -p /tmp/py_bleurt && \
    cd /tmp/py_bleurt && \
    wget -q http://luban-source.byted.org/repository/scm/seed.speech.scm_packer_$BLEURT_VERSION.tar.gz && \
...
RUN mkdir -p /tmp/py_apex && \
    cd /tmp/py_apex && \
    wget -q http://luban-source.byted.org/repository/scm/seed.speech.scm_packer_$APEX_VERSION.tar.gz && \
...
COPY ./scripts/setup_wandb_311_distpkg.sh .
RUN bash ./setup_wandb_311_distpkg.sh
