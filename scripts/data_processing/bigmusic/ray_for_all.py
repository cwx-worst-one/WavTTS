...
@ray.remote
@retry(retry_times=30, interval=10, exc_cls=(ray.exceptions.RayActorError, ))
def async_mss(batch, actor):
    result = []
    for item in batch:
        _, vocal, acc = ray.get(actor.predict.remote(item['_npy']))
        result.append((vocal, acc))
    return result
...
def pipeline(input_file, output_dir, batch_size, index_file, filesystem, labels, actor_map):
    ...
    mss_tar_writer = WebdatasetWriter(os.path.join(output_dir, f'mss_{os.path.basename(input_file)}.tar'), filesystem=filesystem)
...
@click.option('--index2url', required=True, type=str)
...
def main(output_dir, asr_model_name, num_asr_actors, num_mss_actors, num_phoneme_actors, labels, max_concurrency, index2url, num_sami_server, batch_size):
    ...
    with Controller(
        ...
        # TTS Frontend actor
        SamiTTSFrontendActor = ray.remote(num_cpus=2, memory=4 << 30)(SamiTTSFrontendPredictor).options(max_restarts=-1)
        tts_frontend_predictors = []
        for _ in range(num_phoneme_actors):
            tts_frontend_predictors.append(SamiTTSFrontendActor.remote())
        tts_frontend_actors = itertools.cycle(tts_frontend_predictors)
        ...
        def args_generator():
            with filesystem.open_input_stream(index2url) as fp:
                for line in fp.read().decode().split('\n'):
                    if not line:
                        continue
                    tar_file, index_file = line.split('\t')
                    yield tar_file, output_dir, batch_size, index_file, filesystem, labels, {
                        'tts_frontend': functools.partial(async_phoneme_recognize.remote, actor=next(tts_frontend_actors)) if tts_frontend_predictors else None,
                        'lyrics': functools.partial(async_lyric_recognize.remote, actor=next(asr_actors)) if asr_actors_list else None,
                        'mss': functools.partial(async_mss.remote, actor=next(mss_actors)) if mss_predictors else None,
