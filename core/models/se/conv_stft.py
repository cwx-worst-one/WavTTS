...
def init_kernels(win_len=400, win_inc=160, fft_len=512, win_type=None, invers=False):
...
class ConvSTFT(nn.Module):
    ...
    def forward(self, inputs):
        ...
        if self.feature_type == 'complex':
            ...
            outputs = torch.stack([outputs[:, :, :dim], outputs[:, :, dim:]], dim=3)
            return outputs
        else:
            dim = self.dim // 2 + 1
            real = outputs[:, :dim, :]
...
class ConviSTFT(nn.Module):
    ...
    def forward(self, inputs, phase=None):
        """
        inputs : [B, T, N//2+1, 2] (complex spec) or [B, N//2+1, T] (mags)
