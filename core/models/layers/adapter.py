...
class LinearAdapter(nn.Module):
    """adapter layer for transfer learning.
    ...
    def __init__(self, idim, hidden_units, odim=None, activation_fn='relu'):
        """Construct an Positionwise FeedForward object."""
        super(__class__, self).__init__()
        if odim is None:
            odim = idim
        self.w_1 = nn.Linear(idim, hidden_units)
        self.w_2 = nn.Linear(hidden_units, odim)
        self.activation_fn = get_activation_fn(activation_fn)
        utils.xavier_init(self.w_1)
        utils.xavier_init(self.w_2)
...
class LowRankAdapter(nn.Module):
    ...
    def __init__(self, idim, hidden_units, odim=None, activation_fn='relu', rank=1):
        super().__init__()
        if odim is None:
            odim = idim
        self.w_1 = LowRankLinear(idim, hidden_units, rank=rank)
        self.w_2 = LowRankLinear(hidden_units, odim, rank=rank)
        self.activation_fn = get_activation_fn(activation_fn)
...
class PHMLinear(torch.nn.Module):
    def __init__(
        ...
        assert (
            idim % hidden_units == 0
        ), f"Argument `in_features`={idim} is not divisble be `phm_dim`{hidden_units}"
        assert (
            odim % hidden_units == 0
        ), f"Argument `out_features`={odim} is not divisble be `phm_dim`{hidden_units}"
        self.in_features = idim
        self.out_features = odim
        self.learn_phm = learn_phm
        self.phm_dim = hidden_units
        self._in_feats_per_axis = idim // hidden_units
        self._out_feats_per_axis = odim // hidden_units
        ...
        if not self.shared_phm_rule:
            if self.factorized_phm_rule:
                self.phm_rule_left = nn.Parameter(
                    torch.FloatTensor(self.phm_dim, self.phm_dim, 1), requires_grad=learn_phm
                )
                self.phm_rule_right = nn.Parameter(
                    torch.FloatTensor(self.phm_dim, 1, self.phm_dim), requires_grad=learn_phm
                )
            else:
        ...
        if not self.shared_W_phm:
            if self.factorized_phm:
                self.W_left = nn.Parameter(
                    torch.Tensor(size=(self.phm_dim, self._in_feats_per_axis, self.phm_rank)),
                    requires_grad=True,
                )
                self.W_right = nn.Parameter(
                    torch.Tensor(size=(self.phm_dim, self.phm_rank, self._out_feats_per_axis)),
                    requires_grad=True,
                )
            else:
                self.W = nn.Parameter(
        ...
        if self.bias_flag:
            self.b = nn.Parameter(torch.Tensor(odim))
        else:
        ...
        if self.w_init == "glorot-normal":
            ...
            for i in range(self.phm_dim):
                self.W_left.data[i] = init_ones(self.W_left.data[i]) / (
                    self._in_feats_per_axis * self.phm_dim
                )
        ...
        if not self.shared_phm_rule:
            if self.factorized_phm_rule:
                ...
                if self.c_init == "uniform":
                    ...
                    self.phm_rule.data = init_ones(self.phm_rule.data) / (
...
class HyperComplexAdapter(nn.Module):
    ...
    def __init__(self, idim: int, hidden_units: int, odim: int = None, activation_fn='relu'):
        super().__init__()
        if odim is None:
            odim = idim
