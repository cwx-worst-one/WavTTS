...
def get_paths(data_paths, data_root, file_prefix, shard_list, shard_num):
    """
    get data paths
    """
    # First priority
    ...
    if data_root is None:
        logging.error("DataIter Error: No file path entered")
        return None
...
class DataIter:
    ...
    def __init__(
        ...
        io_thread_num=3,
        fd_cache_size=40,
        cached_name='data_iter',
        shuffle=False,
        mem_shared=False,
        sampler_mode='native',
    ):
        """init."""
        self.data_paths = get_paths(data_paths, data_root, file_prefix, shard_list, shard_num)
    ...
    def get_reader(self, worker_num=1, pid=0):
        ...
        if self.sampler_mode == 'global':
            self.entry_nums = self.reader.get_entry_num(
                list(range(self.shard_num)), self.mem_shared
            )
    ...
    def update_buf(self):
        ...
        try:
            ...
            logging.warning("Data Iter may occur Error")
...
class MultiDatasIter(DataIter):
    ...
    def __init__(
        ...
        if len(shard_list) == 0:
            shard_list = list(range(self.shard_num))
        self.file_shards = shard_list
    ...
    def pre_get_file_chunks(self):
        ...
        if self.reader is None:
            self.get_reader()
        shard_chunks = dict()
        for file_idx, shard in enumerate(self.file_shards):
            entry_num = self.reader.get_entry_num([file_idx], False)
            shard_chunks[file_idx] = entry_num // self.chunk_size
        all_entry_num = self.reader.get_entry_num(list(range(self.shard_num)), self.mem_shared)
        all_chunks = [i * self.chunk_size for i in range(all_entry_num // self.chunk_size)]
        for file_idx, shard in enumerate(self.file_shards):
            self.mapped_chunks[shard] = all_chunks[: shard_chunks[file_idx]]
            del all_chunks[: shard_chunks[file_idx]]
        del all_chunks
        del shard_chunks
