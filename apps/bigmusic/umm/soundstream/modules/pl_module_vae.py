...
from samantha.utils.sample_pool import SamplePool
from apps.bigmusic.umm.soundstream.utils.flops import (
    FlopsProfiler,
    get_device_flops,
    get_device_memory,
    with_timeout,
)
from samantha.criterion.soundstream_losses import (
    MelSpectrogramLoss,
...
class VocoderModule(pl.LightningModule):
    def __init__(
        ...
        if mix_training:
            ...
            if dataloader_samplerate != encoder_samplerate:
                self.enc_resampler = torchaudio.transforms.Resample(
                    dataloader_samplerate, encoder_samplerate
                )
            if dataloader_samplerate != decoder_samplerate:
                self.dec_resampler = torchaudio.transforms.Resample(
                    dataloader_samplerate, decoder_samplerate
                )
    ...
    def setup(self, stage: str) -> None:
        ...
        if getattr(self, "enc_resampler", False):
            self.enc_resampler = self.enc_resampler.to(
                torch.device(f"cuda:{self.local_rank}")
            )

        if getattr(self, "dec_resampler", False):
            self.dec_resampler = self.dec_resampler.to(
                torch.device(f"cuda:{self.local_rank}")
            )
    ...
    def training_step(self, batch, batch_idx):
        ...
        norm_d = torch.nn.utils.clip_grad_norm_(self.discriminator.parameters(), 1.0)
        opt_d.step()
        sch_d.step()
        # else:
    ...
    def validation_step(self, batch, batch_idx):
        ...
        for meta_song_id, wav in zip(batch["meta_song_id"], gt_audio):
            ...
            if not os.path.exists(file_name):
                try:
                    sf.write(
                        file_name, wav.cpu().numpy().T, self.hparams.decoder_samplerate
                    )
                except:
