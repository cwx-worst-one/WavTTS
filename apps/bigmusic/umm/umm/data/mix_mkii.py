...
class MixTransforms(BaseTransforms):
    ...
    def __init__(
        ...
        self.offline_processed = offline_processed
    ...
    def __call__(self, item: Dict[str, Any]) -> Generator:
        ...
        # Instrumental music
        elif text == "" and not self.offline_processed:
        ...
        # Speech
            ...
            if self.tokenizer is not None and callable(self.tokenizer):
                ...
                if token.size(-1) == 0 and not self.offline_processed:
                    self._update_stats(skipped=True, message="Token zero length")
                    return
                if (
                    token.size(-1)
    ...
    def __init__(
        ...
        def get_dataset(id):
            dataset = MixDataset(
                    ...
                    offline_processed=self.offline_processed,
                )
            if self.offline_processed:
                return dataset
            else:
        ...
        vocal_zh = get_dataset(vocal_zh_id)
        vocal_en = get_dataset(vocal_en_id)
        speech = get_dataset(speech_id)
        if not remove_instrumental:
            non_vocal = get_dataset(non_vocal_id)
