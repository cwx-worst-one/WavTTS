...
from samantha.utils.utils import download_checkpoint, random_side_mask
from apps.bigmusic.umm.diffusion_tools.models.diffusion_sampler import Sampler
from samantha.utils.sample_pool import OfflineFeatureSamplePool, SamplePool
from samantha.utils.custom_flops_profilers.diffusion_flops_profiler import (
    TNTDiffusionNetworkFLOPsCounter,
)
...
class DiffusionModule(pl.LightningModule):
    def __init__(
        ...
        if self.hparams.lora:
            for k, v in self.model.named_parameters():
                if (
                    "lora" not in k
                    and "vc_null_embedding" not in k
                    and "vc_context_embed" not in k
                    and "sep_embedding" not in k
                ):
        ...
        if diffusion_sample_rate != tokenizer_sample_rate:
            self.resampler = torchaudio.transforms.Resample(
                diffusion_sample_rate, tokenizer_sample_rate
            )
    ...
    def load_from_pretrained(self, pretrained_path=None):
        ...
        with rank_zero_first():
            ...
            ckpt_state_dict = torch.load(local_path, map_location=torch.device("cpu"))[
                "state_dict"
            ]
    ...
    def get_condition_embs(self, x):
        ...
        shared_encoder_output = self.requires[
            "Stage3"
        ].model.shared_encoder.forward_to_vq(
    ...
    def get_vocoder_embs(self, x):
        ...
        sample, _, _ = self.requires["vocoder"].sample(
            encoder_out, deterministic=False
        )  # TODO: check wethear deterministic should be True or False
    ...
    def training_step(self, batch, batch_idx):
        with self.profiler.profile("[litmodule]TNTDiffusionNetwork.pre_feature"):
            ...
            if self.hparams.online_feature:
                ...
                with torch.autocast(device_type="cuda", enabled=False):
                    ...
                    if self.hparams.vc:
                        vc_condition_emb = self.get_vc_condition_emb(
                            batch["vocal_audio"].float()
                        )
                        vc_condition_tokens = self.get_condition_tokens(
                            batch["vocal_audio"].float()
                        )

                        # concat vc condition at the front
                        condition_tokens = torch.cat(
                            [vc_condition_tokens, condition_tokens], dim=-1
                        )
                        vocoder_embs = torch.cat(
                            [vc_condition_emb, vocoder_embs], dim=-1
                        )
            else:
            ...
            t = torch.rand(
                size=[b, 1, self.hparams.num_chunks],
                device=self.device,
                dtype=vocoder_embs.dtype,
            )
            ...
            if self.hparams.vc:
                t = torch.cat(
                    [
                        torch.zeros_like(t)[..., : vc_condition_emb.shape[-1]],
                        t[..., vc_condition_emb.shape[-1] :],
                    ],
                    dim=-1,
                )
            else:
                ...
                # inpainting / outpainting
                    ...
                    t = random_side_mask(
                        t, min_mask_percentage=0.2, max_mask_percentage=0.8
                    )
        ...
        with self.profiler.profile("[litmodule]TNTDiffusionNetwork.forward"):
            vt_pred = self.model(
                ...
                vc_context=(
                    vc_condition_emb.mean(dim=-1, keepdim=True)
                    .permute(0, 2, 1)
                    .detach()
                    if self.hparams.vc
                    else None
        ...
        if isinstance(self.model.flops_fn, TNTDiffusionNetworkFLOPsCounter):
            self.model.flops_fn(
                ...
                vc_condition_emb.shape[1] if self.hparams.vc else 0,
            )
    ...
    def on_validation_epoch_start(self) -> None:
        ...
        os.makedirs(
            f"{self.hparams.val_output_samples_dir}/{self.current_step}/{self.local_rank}",
            exist_ok=True,
        )
        ...
        with torch.autocast(device_type="cuda", enabled=False):
            if self.hparams.online_feature:
                ...
                batch["audio"] = batch["audio"][
                    ...
                    : int(self.hparams.duration * self.hparams.diffusion_sample_rate),
                ]

                # pad to int(self.hparams.duration*self.hparams.diffusion_sample_rate)
                if batch["audio"].shape[-1] < int(
                    self.hparams.duration * self.hparams.diffusion_sample_rate
                ):
                    pad_len = (
                        int(self.hparams.duration * self.hparams.diffusion_sample_rate)
                        - batch["audio"].shape[-1]
                    )
                    batch["audio"] = torch.nn.functional.pad(
                        batch["audio"], (0, pad_len), "constant", 0
                    )
                ...
                if self.hparams.vc:
                    batch["vocal_audio"] = batch["vocal_audio"][
                        : self.hparams.val_batch_size
                    ]
                    vc_condition_emb = self.get_vc_condition_emb(
                        batch["vocal_audio"].float()
                ...
                condition_tokens = batch["condition_tokens"][
                    : self.hparams.val_batch_size, 0
                ]
                gt_vocoder_embs = batch["vocoder_embs"][
                    : self.hparams.val_batch_size, 0
                ]
                assert len(condition_tokens) == len(gt_vocoder_embs), (
                    condition_tokens.shape,
                    gt_vocoder_embs.shape,
                )
                # TODO
                if self.hparams.vc:
                    vc_condition_emb = batch["vc_condition_emb"][
            ...
            pred_emb = self.sampler(
                ...
                num_items=condition_tokens.shape[
                    0
                ],  # batch size: how many samples to generate
                ...
                vc_context=(
                    vc_condition_emb.mean(dim=-1, keepdim=True).permute(0, 2, 1)
                    if self.hparams.vc
                    else None
                ),
