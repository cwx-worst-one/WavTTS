...
from apps.bigmusic.umm.diffusion_tools.models.lora import LoRALinearLayer
...
class Attention(nn.Module):
    def __init__(
        self, query_dim, context_dim=None, heads=8, dim_head=64, dropout=0.0, lora=False
    ):
        super().__init__()
        ...
        for idx, transformer in enumerate(self.layers):
            x = (
                checkpoint(
                    ...
                    (
                        checkpoint(
                            transformer["norm_context"], context, use_reentrant=False
                        )
                        if self.context
                        else None
                    ),
...
class TNTBlocks(nn.Module):
    ...
    def forward(
        ...
        # get mask for cfg
        semantic_cfg_prob = (
            self.semantic_cfg_prob if semantic_force_cfg is None else semantic_force_cfg
        )
        semantic_prob_keep_mask = prob_mask_like(
            (b, 1, 1), 1.0 - semantic_cfg_prob, device=x.device
        )
        # get null embeddings for semantic context
        semantic_null_coarse_emb = repeat(
            self.semantic_null_embedding,
            "d -> b l d",
            b=b,
            l=semantic_context_emb.shape[1],
        )
        semantic_context_emb = torch.where(
            semantic_prob_keep_mask, semantic_context_emb, semantic_null_coarse_emb
        )
        coarse_context.append(semantic_context_emb)
        # vc context
        if self.vc:
            vc_cfg_prob = self.vc_cfg_prob if vc_force_cfg is None else vc_force_cfg
            vc_prob_keep_mask = prob_mask_like(
                (b, 1, 1), 1.0 - vc_cfg_prob, device=x.device
            )
            vc_null_coarse_emb = repeat(
                self.vc_null_embedding, "d -> b l d", b=b, l=vc_context_emb.shape[1]
            )
            vc_context_emb = torch.where(
                vc_prob_keep_mask, vc_context_emb, vc_null_coarse_emb
            )
        ...
        time_emb = torch.mean(
            rearrange(time_emb, "b d lf lc-> (b lc) lf d"), axis=1, keepdim=True
        )

        fine_emb = rearrange(x, "b d lf lc -> (b lc) lf d")
        fine_emb = F.pad(
            fine_emb, (0, 0, 1, 0), value=0
        )  # pad class token to the first positions
        ...
        for idx, (
            ...
            coarse_emb_residual = rearrange(
                coarse_emb_residual, "(b t) d -> b t d", b=b
            )
            coarse_emb = coarse_emb + coarse_emb_residual
            coarse_emb = coarse_transformer(
                coarse_emb, rotary_emb=self.coarse_rotary_embedding
            )
            # context cross-attn
            coarse_emb = context_cross_attn(
                coarse_emb,
                context=coarse_context,
                rotary_emb=self.coarse_rotary_embedding,
            )
...
class TNTDiffusionNetwork(nn.Module):
    def __init__(
        ...
        if consistency:
            ...
            self.guidance_embed = nn.Sequential(
                RMSNorm(256), linear_0, nn.GELU(approximate="tanh"), linear_1
            )

        if context_dim == 1:
            emb_first_layer = nn.Sequential(
                nn.Embedding(32_768, feature_dim), RMSNorm(context_dim)
            )
        else:
            emb_first_layer = nn.Sequential(
                RMSNorm(context_dim), nn.Linear(context_dim, feature_dim)
            )
    ...
    def get_w_embedding(
        self, w, embedding_dim=512, dtype=torch.float32, device=torch.device("cpu")
    ):
        """
        ...
        emb = (
            w.to(dtype)[:, :, None] * emb[None, None, :]
            if len(w.shape) == 2
            else w.to(dtype)[:, None] * emb[None, :]
        )
        emb = torch.cat(
    ...
    def forward(
        ...
        # input: (B, D, T)
        # temporal embedding
        t_start_emb = self.time_slerp_points(
            torch.zeros([batch_size, 1], device=x.device, dtype=torch.long)
        )
        t_end_emb = self.time_slerp_points(
            torch.ones([batch_size, 1], device=x.device, dtype=torch.long)
        )
        ...
        so = torch.sin(omega)
        t_emb = (
            torch.sin((1.0 - timesteps.view(batch_size, seq_length, 1)) * omega) / so
        ) * t_start_emb + (
            torch.sin(timesteps.view(batch_size, seq_length, 1) * omega) / so
        ) * t_end_emb
        t_emb = self.time_embed(t_emb)
        if exists(guidance_scale):
            guidance_scale = self.get_w_embedding(
                guidance_scale,
                embedding_dim=256,
                dtype=t_emb.dtype,
                device=t_emb.device,
...
if __name__ == "__main__":
    ...
    out = model(
        xt, t, semantic_context=semantic, vc_context=vc, guidance_scale=guidance_scale
    )
    print(out.shape)
