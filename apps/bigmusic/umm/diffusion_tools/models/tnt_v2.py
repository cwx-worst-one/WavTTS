...
class Attention(nn.Module):
    ...
    def forward(
        self, x, context=None, rotary_emb=None, is_causal=False, attn_mask=None
    ):
        q = self.to_q(x)
    ...
    def forward(
        ...
        if self.use_checkpoint:
            return self._forward_checkpoint(
                x, context, rotary_emb, is_causal, attn_mask
            )
        else:
    ...
    def forward(
        ...
        for idx, (
            ...
            coarse_emb = coarse_transformer(
                coarse_emb, rotary_emb=self.coarse_rotary_embedding, is_causal=is_causal
            )
            ...
            if self.unet and idx >= sum(self.unet_stages[:2]):
                fine_emb = unet_bridge(
                    torch.cat([fine_emb, unet_cache.pop(-1)], dim=-1)
                )
            ...
            # fine transformer
            fine_emb = fine_transformer(
                fine_emb,
                rotary_emb=self.fine_rotary_embedding,
                attn_mask=fine_attn_mask,
            )
