...
class TNTBlocks(nn.Module):
    ...
    def forward(self, x, time_emb, semantic_context_emb, semantic_force_cfg=None):
        ...
        # concat at dim1
        coarse_context = semantic_context_emb
...
class TNTDiffusionNetwork(nn.Module):
    def __init__(
        ...
        if context_dim == 1:
            ...
            emb_first_layer = nn.Sequential(
                RMSNorm(context_dim), Linear(context_dim, feature_dim)
            )
        ...
        self.output = nn.Sequential(nn.Linear(self.feature_dim, self.input_dim))

    def forward(
        self, x, timesteps=None, semantic_context=None, semantic_force_cfg=None
    ):
