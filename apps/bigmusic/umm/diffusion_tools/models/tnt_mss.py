...
class TNTDiffusionNetwork(nn.Module):
    ...
    def forward(self, x, timesteps=None, context=None, force_cfg=None):
        ...
        prob_keep_mask = prob_mask_like(
            (batch_size, 1, 1), 1.0 - cfg_prob, device=x.device
        )

        # get null embeddings for semantic context
        null_context_emb = repeat(
            self.context_null_embedding,
            "d -> b l d",
            b=batch_size,
            l=context_emb.shape[1],
        )
        context_emb = torch.where(prob_keep_mask, context_emb, null_context_emb)
        ...
        x = self.input_map(x)
        x = self.dpp.unfold(x)
        out = self.blocks(x, time_emb=t_emb, context_emb=context_emb).view(
