...
from apps.bigmusic.umm.ar.datasets.lyrics import default_bucket_batcher_fn
from apps.bigmusic.umm.ar.datasets.mix import DataModule
from apps.bigmusic.umm.ar.datasets.transforms.lyrics import SemanticTokenLengthTransform
from apps.bigmusic.umm.ar.datasets.mcc.mcc import WrappedMCC40MDataset
    ...
    def __init__(
        ...
        if dataset_name == "MCC40M_US":
            ...
            train_urls_and_weights = [
                (
                    f"{hdfs_dir}/nonvocal-A-alternative-rock+indie-pop+sertanejo+trap-rap.txt",
                    0.33,
                ),
                (
                    f"{hdfs_dir}/nonvocal-A-blues+childhood+country+devotional+k-pop+soundtrack+trance+world-music.txt",
                    0.66,
                ),
                ...
                (
                    f"{hdfs_dir}/nonvocal-B-alternative-rock+indie-pop+sertanejo+trap-rap.txt",
                    0.33,
                ),
                (
                    f"{hdfs_dir}/nonvocal-B-blues+childhood+country+devotional+k-pop+soundtrack+trance+world-music.txt",
                    0.66,
                ),
            ...
            train_urls_and_weights = [
                (
                    "hdfs://harunava/home/byte_data_seed_us/hdd_va/speech/data/shutterstock/all_url2idx_tag.txt",
                    1.0,
                )
            ]
            ...
            train_urls_and_weights = [
                (
                    "hdfs://harunava/home/byte_data_seed_us/hdd_va/speech/data/shutterstock/all_url2idx_tag_mcc.txt",
                    1.0,
                )
            ]
                    ...
                    "compose": [
                        ...
                        wds.map(
                            SemanticTokenLengthTransform(
                                sample_rate=sample_rate, audio_key="audio"
                            )
                        ),
        ...
        if val_split == "SSTK_EVAL_US":
            val_urls_and_weights = [
                (
                    "hdfs://harunava/home/byte_data_seed_us/hdd_va/speech/data/shutterstock/val_url2idx_tag.txt",
                    1.0,
                )
            ]
        ...
        super().__init__(
            ...
            collate_fn=partial(
                collate_fn, sample_rate=sample_rate, mixed_ratio=mixed_ratio
            ),
            do_shuffle=False,
        )
