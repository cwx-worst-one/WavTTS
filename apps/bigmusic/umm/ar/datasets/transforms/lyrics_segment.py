...
from apps.bigmusic.umm.ar.datasets.transforms.lyrics_section_alignment import (
    get_section_aligned_text,
    merge_intervals,
)
from apps.bigmusic.umm.ar.datasets.transforms.lyrics_segment_gt import (
    lyrics_to_section_segments,
)
from apps.bigmusic.umm.ar.datasets.transforms.audio import (
    FastNormalizeAudio,
    ReadMP3,
)
from apps.bigmusic.umm.ar.datasets.transforms.base import TransformBase
...
class LyricsSegmentTransforms(TransformBase):
    def __init__(
        ...
        if self.audio_format == "npy":
            ...
            read_mp3 = ReadMP3BytesIO(
                self.sample_rate, self.audio_format, fast=(self.audio_format == "mp3")
            )

        self.base_transform = Compose(
            [read_mp3, ToTensor(), SetAudioDimensions(), NormalizeAudioToFloat32()]
        )
        self.normalize_audio = FastNormalizeAudio()

    def process_segment(
        self, segment, audio_wavs, metadata, process_music_structure=False
    ):
        audio_duration = audio_wavs["target_audio"].shape[-1] / self.sample_rate
        if segment.end > audio_duration:
            return None
        cropped_segments = {
            name: crop_pad_audio_to_segment(segment, audio, self.sample_rate)
            for name, audio in audio_wavs.items()
        }
        if (
            "music_structure" in metadata and process_music_structure
        ):  # TODO: (AS) enable this in config
    ...
    def extract_audio_wavs(self, x: Dict[str, torch.Tensor]) -> Dict[str, torch.Tensor]:
        ...
        for name, audio_key in self.audio_keys.items():
            if audio_key in cached_wavs:
                ...
                try:
                    ...
                    raise Exception(
                        f"[MP3 decoding error] - {name} - {self.url2index} - {e}"
                    )
        ...
        if "target_audio" not in audio_wavs:
            assert (
                "style_audio" in audio_wavs and "vocal_audio" in audio_wavs
            ), "Must provide target audio or mulan and vocal audio"
            audio_wavs["target_audio"] = (
                audio_wavs["style_audio"] + audio_wavs["vocal_audio"]
            )
    ...
    def __call__(self, x: Dict[str, torch.Tensor]) -> Generator:
        ...
        is_invalid, reason = is_invalid_song(
            metadata, lyrics, confidence_threshold=self.min_song_confidence
        )
        if is_invalid:
        ...
        fixed_duration = (
            len(self.sample_duration) == 1
        )  # if only one duration is provided. Fix it to that duration
        ...
        if len(segments) == 0:
            self._update_stats(
                skipped=True,
                message="Filter low confident segments. 0 segments above threshold",
            )
            return
        try:
        ...
        if segment_count == 0:
            self._update_stats(
                skipped=True, message="Error processing segments. 0 segments processed"
            )
        else:
...
def crop_pad_to_seq_length(
    seq: torch.Tensor, target_seq_len, dtype=None, padding_value=0, start=0
):
...
def random_crop_pad_to_seq_length(
    seq: torch.Tensor, target_seq_len, dtype=None, padding_value=0
):
    *dims, input_seq_len = seq.shape
    start_idx = random.randint(0, max(0, input_seq_len - target_seq_len))
    return crop_pad_to_seq_length(
        seq,
...
class Segment:
    ...
    def from_dict(cls, json_dict):
        ...
        return Segment(start, end, text, confidence, words, phoneme)
    ...
    def __add__(self, other: "Segment", new_line_token=". "):
        return Segment(
            ...
            phoneme=self.phoneme + " <n> " + other.phoneme,
        )


def lyrics_to_segments(
    lyrics,
    target_durations=(20, 25, 30),
    new_line_token=". ",
    fixed_duration=False,
    min_confidence=0.75,
    shuffle_start=False,
    shuffle_lengths=False,
    include_intro_p=0.5,
):
    if not lyrics:
        return []

    # for parsing ground truth lyrics
    section_segments = lyrics_to_section_segments(
        lyrics, target_durations=target_durations
    )
    if section_segments:
    ...
    for i in range(start_idx, len(lyrics)):
        ...
        if (
            current_segment.text and current_segment.phoneme is None
        ):  # error with phoneme - usually due to non-tts supported languages - arabic, etc.
            segment = None
            continue
        if (
            current_segment.start is None
            or current_segment.start < 0
            or (segment and current_segment.start < segment.end)
        ):
            segment = None
            continue
        ...
        if segment and (current_segment.end - segment.start) > target_duration_length:
            if fixed_duration and "words" in current_diction:
                ...
                extended_segment, _ = _words_to_segment(
                    current_diction["words"], segment.start, target_end_time
                )
                if extended_segment:
                ...
                segment.end = max(
                    segment.end,
                    min(segment.start + target_duration_length, current_segment.start),
                )
        ...
        if current_segment and current_segment.duration > max_duration:
            ...
            for i in range(math.ceil(current_segment.duration / max_duration)):
                ...
                clipped_segment, cached_index = _words_to_segment(
                    current_diction["words"], start, end, cached_index
                )
                ...
                if fixed_duration and clipped_segment.duration <= max_duration:
                    ...
                    and clipped_segment.duration >= min_duration
                ):
                    segments.append(clipped_segment)
        ...
        if segment is None:
            ...
            previous_end_time = (
                0
                if i == 0
                else Segment.from_dict(lyrics[i - 1]).end + previous_end_offset
            )
            if (
                random.random() > include_intro_p
                and previous_end_time
                and segment.end - previous_end_time <= target_duration_length
            ):
...
def _words_to_segment(words, start_time, end_time, start_index=0):
    def is_word(word):
        return word.text not in punctuation_whitespace
    ...
    for idx, word in enumerate(words[start_index:]):
        ...
        if is_word(word_segment):
            if (
                word_segment.start >= 0 and word_segment.start < start_time
            ):  # skip words less than start time
                continue
        ...
        if segment:
            ...
            segment is None
            and word_segment.has_valid_time()
            and word_segment.start >= start_time
        ):
...
def is_valid_song_name(metadata):
    if "meta_song_title" not in metadata:
        return True
    ...
    if is_live(full_name):
        return False
    if is_instrumental(full_name):
        return False
