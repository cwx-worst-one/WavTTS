...
from apps.bigmusic.umm.ar.datasets.mir_data_util import ARTIST_ID_MAP_V2
from apps.bigmusic.umm.ar.datasets.tokenizers.phoneme import MAX_PHONE_LEN
from apps.bigmusic.umm.ar.datasets.mcc.mix import (
    INDEX,
    BaseTransforms,
    DataModule,
...
from apps.bigmusic.umm.ar.datasets.transforms.audio import FastNormalizeAudio
from apps.bigmusic.umm.ar.utils.format_utils import (
    normalize_text,
    rewrite_metadata,
    rewrite_playlist_labels,
    sa_music_tagging_to_style_text,
)
...
def collate_fn(
    batch: List[torch.Tensor], conditions="style_text,lyrics_tokens"
    ...
    for idx in range(len(batch)):
        ...
        construct = lambda key, default: (
            batch[idx][key].clone().detach()
            if key in batch[idx] and batch[idx][key] is not None
...
def format_utterances(utterances, time_in_sec=False, min_confidence=0.2):
    ...
    for i, u in enumerate(utterances):
        ...
        if utt_start is None:
            continue
        utt_start = math.floor(
            utt_start
        )  # Round down so we don't miss the first few phonemes
        utt_end = u.get("end_time", None)
        if utt_end is None:
            if i < (len(utterances) - 1):
                utt_end = max(
                    math.floor(utterances[i + 1].get("start_time", 0)), utt_start + 1
                )
            else:
                continue
        ...
        if time_in_sec:
            ...
            new_utterances.append(
                [
                    math.floor(utt_start / 1000),
                    math.ceil(utt_end / 1000),
                    u["text"],
                    phone,
                ]
            )
    ...
    for i in range(len(utterances) - 1):
        if (utterances[i][0] == utterances[i + 1][0]) and (
            utterances[i][1] == utterances[i + 1][1]
        ):
            continue
...
def extract_seg_from_utts(
    i, utterances, min_duration, max_duration, new_line_token, sec_start_idx=None
):
...
def remove_short_inst_phrases(song_slice: SongSlice) -> Optional[SongSlice]:
    ...
    phrases = [
        p
        for p in song_slice.phrases
        if p.has_utterance
        or (p.section_tag not in ["verse", "chorus", "bridge"] and p.duration >= 2)
    ]
...
def group_utterances_with_structure(
    ...
    def get_section_span(structure_tag: Dict) -> Tuple[int, int]:
        return math.floor(structure_tag["start_time"]), math.ceil(
            structure_tag["end_time"]
        )
        ...
        for start_idx in section_start_ind:
            ...
            while idx <= len(phrases_in_proc):
                ...
                if (phrase is None) or (
                    get_phrase_group_dur(group + [phrase]) > max_duration
                ):
                    if group:
    ...
    song_time_span = (
        math.floor(structure_tags[0]["start_time"]),
        math.ceil(structure_tags[-1]["end_time"]),
    )
...
def group_utterances(
    ...
    if infer_structure_tags:
        ...
        last_utt_end = -10
        MIN_NUM_CHAR = (
            3  # Minimum number of characters in an utterance to infer it's valid vocal.
        )
        MIN_SEP_WITHIN_SEC = (
            4  # Max separation of two utterance to infer there is a section break.
        )
        for i, utt in enumerate(utterances):
            if (utt[0] - last_utt_end >= MIN_SEP_WITHIN_SEC) and len(
                utt[2]
            ) > MIN_NUM_CHAR:
    ...
    if sec_start_idx:
        ...
        while i < len(utterances):
            i, new_seg = extract_seg_from_utts(
                i, utterances, min_duration, max_duration, new_line_token
            )
            if new_seg:
                segs.append(new_seg)
    ...
    def __call__(self, item: Dict[str, Any]) -> Generator:
        ...
        if lyrics_connfidence_avg is None:
            ...
            if lyrics_connfidence_avg < self.lyrics_confidence:
                self._update_stats(
                    skipped=True, message="Low confidence lyrics sta avg"
                )
                return
        ...
        if isinstance(self.tokenizer, BertTokenizer):
            self.tokenizer.add_special_tokens(
                {"additional_special_tokens": [new_line_token]}
            )
        ...
        if self.segment_method == "verse_chorus_section":
            song_slices = [
                ...
                if set(s.section_tag for s in song_slice.phrases).issubset(
        ...
        song_slices = [
            ss
            for ss in song_slices
            if ss.is_time_span_valid((self.min_duration, self.max_duration))
        ]
        ...
        if any(
            phrase.text and not phrase.phonemes
            for song_slice in song_slices
        ...
        if "music_tagging" in meta:
            style_text, unfamiliar_tags, is_sinking = sa_music_tagging_to_style_text(
                meta["music_tagging"], self.sinking_threshold
            )
            ...
            if unfamiliar_tags:
                self._update_stats(
                    skipped=False,
                    message=f"Detected tag(s) not in SA vocab: {unfamiliar_tags}",
                )
        ...
        for song_slice in song_slices:
            ...
            normalized_text = normalize_text(
                "\n".join([s.format_text() for s in song_slice.phrases]),
            ...
            if self.tokenizer == "tts_chinese_frontend_model":
                ...
                for _phrase in song_slice.phrases:
                    labels = (
                        [l for l in _phrase.phonemes.split("\n") if l]
                        if _phrase.phonemes
                        else None
                    )
                    if labels is None or (labels is not None and len(labels) > 0):
                        convert_result = convert_labels_to_text_id(
                            labels, _phrase.prefix_tags
                        )
                        if convert_result:
                ...
                text_tokens = self.tokenizer(
                    normalized_text, add_special_tokens=False, return_tensors="pt"
        ...
        if isinstance(dataset_names, list):
            ...
            if dataset_names == "SodaTest":
                ...
                dataset = IndexedWebDataset(
                    url2index=INDEX[region][dataset_names], **kwargs
                )
    ...
    def __init__(
        ...
        train_dataset = WebPipeline(
            MultiIterableDataset(
                datasets=self.wds_vocal_datasets + self.parquet_vocal_datasets,
...
class SftWebDataModule(DataModule):
    ...
    def __init__(
        ...
        for split, pd_id_weights in parquet_datasets.items():
            ...
            nodesplitter = (
                shardlists.single_node_only if split == "train" else return_self
            )
...
class LyricsSftWebDataModule(SftWebDataModule):
    def __init__(self, **kwargs):
        super().__init__(
            parquet_datasets={"train": [(1026, 1)], "test": [(1012, 1)]}, **kwargs
        )
...
class ArtistSftWebDataModule(SftWebDataModule):
    def __init__(self, **kwargs):
        super().__init__(
            parquet_datasets={"train": [(1473, 1)], "test": [(1012, 1)]}, **kwargs
        )
