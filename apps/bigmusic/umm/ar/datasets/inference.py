...
from apps.bigmusic.umm.ar.datasets.lyrics import (
    default_batch_fn,
    dictionary_collate,
    transform_dataset,
)
from apps.bigmusic.umm.ar.datasets.transforms.lyrics import (
...
from apps.bigmusic.umm.ar.datasets.transforms.structure import IntensityTransform
...
default_prompt_path = (
    Path(__file__).absolute().parent / "inference_prompts/default.json"
)
...
def inference_dataset_from_prompt(
    ...
    if "vocal_audio" in prompts:
        additional_transforms = (
            [voice_clone_transform(extra_params)]
            if extra_params.get("app_type") == "vclone"
            else []
        )
        prompts["vocal_audio"] = load_and_normalize_wavs(
            prompts["vocal_audio"], additional_transforms
        )
    ...
    if "structure" in prompts:
        prompts["structure"] = [
            None if x == "random" else json.loads(x) for x in prompts["structure"]
        ]
    ...
    if "semantic_tokens" in prompts:
        prompts["semantic_tokens"] = [
            torch.load(fp) for fp in prompts["semantic_tokens"]
        ]
    if run_combinations:
    ...
    if "lyrics_tokens" in conditions:
        ...
        if "style_text" in conditions and "style_text" not in prompts:
            ...
            if "metadata" in prompts:
                print(
                    "WARNING: style_text not provided. Using metadata to generate style prompt"
                )
    ...
    if "intensity" in conditions:
        segment_transforms.append(
            IntensityTransform(
                audio_key=(
                    "intensity_audio" if "intensity_audio" in prompts else "style_audio"
                ),
    ...
    batch_fn = default_batch_fn(
        batch_size, collation_fn=partial(dictionary_collate, remove_invalid=False)
    )
...
def load_and_normalize_wavs(wav_paths, additional_transforms=()):
    audio_transforms = Compose(
        [ToTensor(), SetAudioDimensions(), *additional_transforms]
    )
