...
from apps.bigmusic.umm.diffusion_tools.models.diffusion_model.utils import run_diffusion
from samantha.utils.utils import download_checkpoint
from apps.bigmusic.umm.ar.lightning.base_modules import BaseModule
from apps.bigmusic.umm.ar.lightning.embedding_modules import get_bestrq_umm_tokens
from apps.bigmusic.umm.ar.lightning.semantic_modules import (
    process_eos_indexes,
    truncate_wav_to_eos,
)
from apps.bigmusic.umm.ar.utils.model_initializer import run_2ar
...
class SemanticInferenceModule(pl.LightningModule):
    def __init__(self, semantic_cls_path, required_modules, extra_params=None):
        super().__init__()
        ...
        *module_paths, cls_name = semantic_cls_path.split(".")
        ...
        if not semantic_ckpt_path.exists():
            ...
            semantic_ckpt_path = download_checkpoint(
                self.extra_params.semantic_ckpt, f".module_cache/{semantic_dir}"
            )
    ...
    def setup(self, stage: str) -> None:
        if isinstance(self.semantic_module.model, gpt.GPTLMHeadModel) and (
            "32" in self.trainer.precision
        ):
            raise Exception(
                f"Invalid precision for cTIGA model {self.trainer.precision}. Please set --run_opts.precision 16"
            )
        if isinstance(self.semantic_module.model, LlamaPreTrainedModel) and (
            "16" in self.trainer.precision
        ):
            raise Exception(
        ...
        if self.extra_params.token2wav_type == "diffusion":
            ...
            self.decoding_params = DotDict(
                {**self.extra_params, **self.extra_params["diffusion_params"]}
            )
            ...
            self.decoding_params = DotDict(
                {**self.extra_params, **self.extra_params["ar_params"]}
            )
            ...
            self.decoding_params = DotDict(
                {**self.extra_params, **self.extra_params["soundstorm_params"]}
            )
        else:
        ...
        if self.extra_params.use_reranker:
            if self.extra_params.beam_size <= 1:
                print(
                    f"[WARNING] use_reranker=True but beam_size={self.extra_params.beam_size}"
                )
            required_modules.update(
                {"reranker": self.hparams.required_modules["reranker"]}
            )
    ...
    def load_required_modules(self, required_modules):
        ...
        if (
            self.extra_params.get("mulan_ckpt", None)
            and self.extra_params.mulan_ckpt != "infer_from_semantic_ckpt"
        ):
            self.semantic_module.hparams.required_modules["mulan"][
                "hpath"
            ] = self.extra_params.mulan_ckpt
    ...
    def predict_step(self, batch, batch_idx=0, dataloader_idx=0):
        ...
        semantic_samples, eos_index_list = process_eos_indexes(
            raw_semantic_samples, self.semantic_module, self.extra_params.sample_rate
        )
        # TODO (QQ) use semantic_samples embedding as context input for decoding fn
        if self.extra_params.get("mixv2", False):
            semantic_samples = self.requires["Stage3"].model.vq.embedding(
                semantic_samples
            )
        raw_wav_output = self.decoding_fn(
            self.requires, semantic_samples, self.decoding_params
        )
        ...
        if self.extra_params.use_reranker:
            ...
            raw_wav_output, eos_index_list, rewards_breakdown = self.requires[
                "reranker"
            ].rerank(raw_wav_output, eos_index_list, batch, self.extra_params)
...
class GTInferenceModule(pl.LightningModule):
    def __init__(self, required_modules, extra_params=None):
        ...
        if self.extra_params.token2wav_type == "diffusion":
            ...
            required_modules.update(self.hparams.required_modules["diffusion_modules"])
            self.decoding_params = DotDict(
                {**self.extra_params, **extra_params["diffusion_params"]}
            )
        elif self.extra_params.token2wav_type == "ar":
            self.decoding_fn = run_2ar
            required_modules.update(self.hparams.required_modules["ar_modules"])
            self.decoding_params = DotDict(
                {**self.extra_params, **extra_params["ar_params"]}
            )
        else:
            raise ValueError(f"Unhandled type: {self.extra_params.token2wav_type}")
    ...
    def predict_step(self, batch, batch_idx=0, dataloader_idx=0):
        batch["target_audio"] = batch[
            "style_audio"
        ]  # prepare_inputs expects target_audio key
        semantic_samples = self.encoding_fn(self.requires, batch["target_audio"])
        wavs = self.decoding_fn(
            self.requires, semantic_samples, self.decoding_params
        )  #
        return {"generated_audio": wavs, "generated_audio_tensor": wavs}
