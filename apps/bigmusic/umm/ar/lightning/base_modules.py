...
from apps.bigmusic.umm.ar.lightning.embedding_modules import BaseEmbedder, TokenEmbedder
...
class BaseModule(pl.LightningModule):
    ...
    def _shared_step(self, batch, update_mfu=False):
        ...
        accu_seq_25 = (
            x.argmax(dim=-1)[..., :25] == target_ids[..., :25]
        ).float().mean() * 100
        result_dict = {
            "loss": loss.item(),
            "accu": accu.item(),
    ...
    def on_validation_epoch_end(self):
        for dataloader_idx, results_list in self.val_outputs.items():
            ...
            log_dict = {
                f"val_{k}_{str(dataloader_idx)}": torch.tensor(v).mean().item()
...
class BaseContinuousEmbedModule(BaseModule):
    ...
    def prepare_target_inputs(self, batch):
        ...
        if "target_tokens_length" in batch:
            ...
            target_lengths = torch.full(
                (batch_size,), fill_value=seq_len, dtype=torch.long, device=self.device
            )
    ...
    def prepare_training_inputs(self, batch):
        ...
        target_ids = target_values["token_ids"][
            :, 1:
        ]  # offset targets by one to skip SOS prediction
    ...
    def sample_logits(self, i, logits, temp, mode, thresh=0.9, exclude_ids=None):
        return sample(
            logits, temp=temp, mode=mode, thresh=thresh, exclude_ids=exclude_ids
        )
    ...
    def predict(
        ...
        for i in pbar:
            ...
            if isinstance(self.model, gpt.GPTLMHeadModel):
                ...
                model_input["inputs_embeds"] = model_input["inputs_embeds"].to(
        ...
        for slice_beg, slice_end in slice_durations:
            ...
            for i in pbar:
                ...
                if isinstance(self.model, gpt.GPTLMHeadModel):
                    ...
                    logits = logits[:, -1:, :]  # only predicting on last logit.
                    predict_token = self.sample_logits(
                        i, logits, temperature, sample_mode, sample_thresh
                    )
