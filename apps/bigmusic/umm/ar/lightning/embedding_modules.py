...
from apps.bigmusic.umm.ar.datasets.mir_data_util import (
    NONE_LABEL,
...
from apps.bigmusic.umm.ar.lightning.ssl_frontend import (
...
def get_mulan_tokens(requires, x, data_type="music"):
    ...
    mulan_tokens, _ = requires["mulan_rvq_fn"](mulan_embeds, requires["mulan_centers"])
    return mulan_tokens
...
def get_bestrq_umm_tokens(requires, batch, chunk_size=None):
    ...
    if chunk_size is None or batch.shape[-1] <= chunk_size:
        ...
        vq_ids = lit_module.wav2token(
            batch_chunked.reshape(batch_size * num_chunks, 1, chunk_size)
        )
...
class TokenEmbedder(BaseEmbedder):
    ...
    def __init__(
        self, vocab_size, embedding_dim, add_sos=False, add_eos=False, **kwargs
    ):
        super().__init__()
    ...
    def tokenize(
        self, requires=None, batch=None, token_ids=None, with_sos=False, with_eos=False
    ):
...
class TagCategoricalEmbedder(TokenEmbedder):
    ...
    def sync_tags(self, batch_style_tags):
        ...
        if self.vocab_type != "auto":
            return
        if not self.training:
            return  # do not run on validation set - to prevent deadlocks
        if (
            ...
            new_tags = set(
                [tag for style_tags in batch_style_tags for tag in style_tags]
            )
            ...
            gathered_new_tags = list(
                sorted(
                    set(
                        [
                            tag
                            for tags in gathered_tags
                            if tags is not None
                            for tag in tags
                            if tag is not None
                        ]
    ...
    def get_tag_id(self, tag, dropout=0.0):
        if tag not in self.vocab2id:
            if (
                not self.training and len(tag.strip()) > 0
            ):  # use NONE_LABEL for empty label
                raise Exception(
                    f"Inference Error: Tag {tag} not found in vocab {self.vocab2id}. Please check vocab"
                )
            tag = NONE_LABEL
...
class MulanCategoricalEmbedder(BaseEmbedder):
    ...
    def get_tokens(self, requires, style_texts):
        ...
        for style_text in style_texts:
            ...
            if isinstance(style_text, str):
                separator = self.category_separator
                normalized_style_text = style_text.replace("，", separator).replace(
                    ",", separator
                )
        ...
        batch_tag_ids = pad_sequence(
            batch_tag_ids, batch_first=True, padding_value=self.get_tag_id(NONE_LABEL)
        )
        device = next(self.parameters()).device
        return torch.as_tensor(batch_tag_ids).to(device)
    ...
    def get_embeds(
        self, requires, input_audio_or_text, data_type=None, target_samples_length=None
    ):
        # Text
        if data_type == "text":
    ...
    def get_embeds(
        ...
        if data_type == "text":
            ...
            if not self.mulan_crop and not self.mulan_average:
                ...
                prefix_length = (
                    1 + (target_samples_length - self.min_audio_length) // shift_length
                )
        ...
        if not self.training and not self.mulan_crop and not self.mulan_average:
            assert target_samples_length is not None
            input_audio_or_text = random_crop_pad_to_seq_length(
                input_audio_or_text, target_samples_length
            )
        if input_audio_or_text.shape[-1] < self.min_audio_length:
            input_audio_or_text = crop_pad_to_seq_length(
                input_audio_or_text, self.min_audio_length
...
class WavToVecTokenEmbedder(TokenEmbedder):
    def __init__(
        self, vocab_size=1024, embedding_dim=1024, add_sos=False, add_eos=False
    ):
...
class BestRQMKIITokenEmbedder(TokenEmbedder):
    def __init__(
        self, vocab_size=65_536, embedding_dim=1024, add_sos=False, add_eos=False
    ):
        super().__init__(vocab_size, embedding_dim, add_sos, add_eos)
...
class SoundstreamTokenEmbedder(TokenEmbedder):
    ...
    def __init__(
        self, layer_range, codebook_size=1024, embedding_dim=1024, add_sos=False
    ):
...
class StructureEmbedder(nn.Module):
    def __init__(
        self, durations, embedding_dim, structure_labels, granularity_in_secs=0.5
    ):
        super().__init__()
        durations = durations if isinstance(durations, (list, tuple)) else [durations]
    ...
    def embed(self, batch_structure_labels, target_duration):
        device = next(self.parameters()).device
        structure_ids = (
            torch.full((len(batch_structure_labels), self.max_seq_len), self.pad_id)
            .long()
            .to(device)
        )
        ...
        for i, structure_labels in enumerate(batch_structure_labels):
            if (
                structure_labels is None
            ):  # This means no specification, just do whatever
        ...
        if self.logged < 5:
            print(
                f"target_duration: {target_duration}, target_seq_len: {target_seq_len}"
            )
            print(
                f"structure_labels: {batch_structure_labels}, structure_ids: {structure_ids}"
            )
            self.logged += 1
...
class IntensityEmbedder(nn.Module):
    ...
    def embed(self, batch_intensity_labels, target_duration):
        ...
        if self.logged < 5:
            print(
                f"target_duration: {target_duration}, intensity_labels: {batch_intensity_labels.shape}, intensity_ids: {intensity_ids.shape}"
            )
            print(
                f"intensity_labels: {batch_intensity_labels}, intensity_ids: {intensity_ids}"
            )
            self.logged += 1
    ...
    def embed(self, batch_beat_labels, target_duration):
        ...
        batch_beat_labels = [
            [
                (x[0], int(x[1]))
                for x in y
                if x[0] <= target_duration and int(x[1]) in self.beat2id
            ]
        ...
        beat_ids = (
            torch.full((batch_size, max_beat_length + 1), self.pad_id).long().to(device)
        )
        beat_timestamps = (
            torch.zeros((batch_size, max_beat_length + 1)).float().to(device)
        )
        ...
        if self.logged < 5:
            ...
            self.logged += 1
        return (
            self.embedder(beat_ids) * beat_timestamps.unsqueeze(2),
            beat_ids,
            beat_timestamps,
        )
