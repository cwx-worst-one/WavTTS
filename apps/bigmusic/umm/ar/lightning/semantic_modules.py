...
from apps.bigmusic.umm.ar.datasets.mir_data_util import convert_m1_tag_to_style_text
from apps.bigmusic.umm.ar.lightning.music_sft import get_m1_tags
from samantha.criterion.masked_loss import MaskedCrossEntropyV2
...
from apps.bigmusic.umm.ar.utils.mulan_tag import get_mulan_tags
...
class SemanticModule(BaseContinuousEmbedModule):
    def __init__(
        ...
        self.prepare_input_types = extra_params.get(
            "prepare_input_types", []
        )  # m1_tagger, mulan_tagger
        embedder_dict = {}
        ...
        for emb_type in extra_params.get("input_embedders", ["mulan", "lyrics_tokens"]):
            if emb_type == "mulan":
                ...
                embedder_dict[emb_type] = DurationEmbedder(
                    durations=extra_params["duration"], embedding_dim=hidden_size
                )
                ...
                max_duration = (
                    duration[-1] if isinstance(duration, (list, tuple)) else duration
                )
    ...
    def infer_target_duration(self, batch):
        if "duration" in batch:
            ...
            target_duration = (
                batch["target_audio"].shape[-1] // self.extra_params.sample_rate
            )
        else:
    ...
    def prepare_mulan_inputs(self, batch, mulan_embedder):
        ...
        if "style_text" in conditions:
            ...
            embeds = mulan_embedder.embed(
                self.requires, batch["style_text"], with_sos=True, data_type="text"
            )
    ...
    def prepare_lyrics_inputs(self, batch, lyrics_embedder):
        ...
        if "lyrics_tokens" in conditions:
            embeds = lyrics_embedder.embed(
                self.requires, batch["lyrics_tokens"].to(self.device), with_sos=True
            )
        else:
    ...
    def prepare_categorical_inputs(self, batch, categorical_embedder):
        ...
        embeds = categorical_embedder.embed(
            self.requires, batch["style_category"], with_sos=True
        )
        return embeds
    ...
    def prepare_speaker_inputs(self, batch, speaker_embedder):
        ...
        if "speaker_id" in conditions and "speaker_id" in batch:
            ...
            embeds = speaker_embedder.embed(
                self.requires, batch["speaker_id"].to(self.device), with_sos=True
            )
        else:
    ...
    def prepare_acc_audio_inputs(self, batch, acc_embedder: BestRQTokenEmbedder):
        ...
        if "acc_audio" in conditions:
            embeds = acc_embedder.embed(
                self.requires, batch["acc_audio"], with_sos=True
            )
        else:
    ...
    def prepare_vocal_audio_inputs(self, batch, vocal_embedder: BestRQTokenEmbedder):
        ...
        if "vocal_audio" in conditions:
            embeds = vocal_embedder.embed(
                self.requires, batch["vocal_audio"], with_sos=True
            )
        else:
    ...
    def prepare_beat_inputs(self, batch, beat_embedder):
        ...
        if "beat" not in batch:
            target_audio = (
                ...
                else batch["style_audio"]
            )
            batch["beat"] = self.requires["beat"].predict_step(
                {"target_audio": target_audio}, 0
            )
        assert batch_size == len(batch["beat"])
        target_duration = self.infer_target_duration(batch)
        embeds, beat_ids, beat_timestamps = beat_embedder.embed(
            batch["beat"], target_duration
        )
    ...
    def prepare_batch_inputs(self, batch):
        ...
        if "m1_tagger" in self.prepare_input_types:
            assert isinstance(
                self.target_embedder, BestRQTokenEmbedder
            ), "m1 requires target_hidden_states to predict categories"
            assert (
                self.target_embedder.hidden_states is not None
            ), "m1 requires target_hidden_states to predict categories"
            assert (
                "target_audio" in batch
            ), "m1 requires target_audio to predict categories"
    ...
    def aux_loss(self, batch, logits, last_hidden_state, target_length):
        ...
        for pred_type, pred_head in self.prediction_heads.items():
            ...
            if pred_type == "intensity":
                target_intensity = self.input_embedders["intensity"].quantize(
                    batch["intensity"]
                )
                ...
                pred_accu = (
                    intensity_logits.argmax(dim=-1) == target_intensity
                ).float().mean() * 100
                pred_dict = {
                ...
                # Offset by one
                st = st_idx - 1
                beat_embed = last_hidden_state[
                    :, st : st + target_beat_ids.shape[-1], :
                ]
                ...
                beat_ids_accu = (
                    beat_ids_logits.argmax(dim=-1) == target_beat_ids
                ).float().mean() * 100
                beat_timestamps_loss = (
                    (beat_timestamps - target_timestamps) ** 2
                ).mean()
    ...
    def _shared_step(self, batch, update_mfu=False):
        with self.profiler.profile(
            f"bigmusic.prepare_training_inputs{self.trainer.global_step}"
        ):
        ...
        accu_seq_25 = (
            target_logits.argmax(dim=-1)[..., :25] == target_ids[..., :25]
        ).float().mean() * 100
        result_dict = {
            "loss": loss.item(),
            "accu": accu.item(),
        ...
        # Auxiliary loss
        aux_loss, aux_dict = self.aux_loss(
            batch, logits, last_hidden_state, target_length
        )
        loss += aux_loss
    ...
    def _predict_intensity(
        ...
        for i in pbar:
            ...
            if isinstance(self.model, gpt.GPTLMHeadModel):
                ...
                inference_params.sequence_len_offset += model_input[
                    "inputs_embeds"
                ].size(1)
            else:
                model_output = self.model(
            ...
            target_logits = intensity_head(
                last_hidden_state[:, -1:]
            )  # only predict for last logit
            predict_token = self.sample_logits(
                i, target_logits, intensity_temperature, "top_p"
            )
            predict_token_emb = intensity_embedder.embedder(predict_token)
            model_input = {"inputs_embeds": predict_token_emb}
            previous_inputs_embeds = torch.cat(
                [previous_inputs_embeds, predict_token_emb], dim=1
            )
            output_tokens = (
...
class SemanticRLModule(SemanticModule):
    def __init__(
        ...
        super().__init__(
            model_cls=model_cls,
            criterion_cls=MaskedCrossEntropyV2,
            optimizer_cls=optimizer_cls,
            scheduler_cls=scheduler_cls,
            required_modules=required_modules,
            checkpointing=checkpointing,
    ...
    def _shared_step(self, batch, mode):
        ...
        sampled_semantic_tokens_processed, eos_index_list = process_eos_indexes(
            sampled_semantic_tokens, self, sample_rate=self.extra_params.sample_rate
        )
        # Compute rewards
        ...
        if len(eos_index_list) > 0:
            token2wav_rate = (
                self.extra_params.sample_rate // self.extra_params.semantic_frame_rate
            )
            seq_len = (
                eos_index_list // token2wav_rate + 1
            )  # need to add 1 to include <eos>
    ...
    def training_step(self, batch, batch_idx):
        ce_loss, accu, seq_loss, _, _, _, reward_breakdown, seq_probs, skip = (
            self._shared_step(batch=batch, mode="training")
        )
        stats = {
    ...
    def validation_step(self, batch, batch_idx, dataloader_idx=0):
        ...
        self.val_outputs[dataloader_idx].append(
            self._shared_step(batch=batch, mode="validation")
        )
    ...
    def on_validation_epoch_end(self):
        ...
        for dataloader_idx, outputs in self.val_outputs.items():
            ...
            for batch_idx, (
                ...
                stats[f"seq_probs/{prefix}_max_mean"] += seq_probs.max(
                    dim=-1
                ).values.mean()
                stats[f"seq_probs/{prefix}_max_std"] += seq_probs.max(
                    dim=-1
                ).values.std()
                for rw_type, rw in reward_breakdown.items():
                    stats[f"reward_{rw_type}/{prefix}_avg_mean"] += rw.mean(
                        dim=-1
                    ).mean()
                    stats[f"reward_{rw_type}/{prefix}_avg_std"] += rw.mean(dim=-1).std()
                    stats[f"reward_{rw_type}/{prefix}_intra_beam_std"] += rw.std(
                        dim=-1
                    ).mean()
                    stats[f"reward_{rw_type}/{prefix}_max_mean"] += rw.max(
                        dim=-1
                    ).values.mean()
                    stats[f"reward_{rw_type}/{prefix}_max_std"] += rw.max(
                        dim=-1
                    ).values.std()
    ...
    def get_reward(self, items):
        ...
        for rw_type, rw_weight in self.extra_params.rewards.items():
            ...
            if rw_type == "style_sim":
                conditions = self.infer_conditions(batch)
                rw_type = (
                    "style_text_sim" if "style_text" in conditions else "mulan_sim"
                )
    ...
    def _get_reward(self, items, reward_type):
        ...
        if reward_type == "mulan_sim":
            (mulan_sim, items["sampled_mulan_embeds"], items["target_mulan_embeds"]) = (
                mulan_audio_reward(
            ...
            if len(sampled_lyrics) != sampled_audio.size(0):
                ...
                print(
                    f"lyrics: len={len(sampled_lyrics)} (expected {sampled_audio.size(0)}), content={sampled_lyrics}"
                )
                return 0
            return wer_reward(
                sampled_lyrics,
            ...
            for i in range(sampled_audio.size(0)):
                ...
                # Final filter
                genres = [
                    g for g in genres if self.requires["chord_lms"][g] is not None
                ]
            ...
            return semantic_diversity_reward(
                items["sampled_semantic_tokens"], device=sampled_audio.device
            )
...
def process_eos_indexes(
    semantic_samples, semantic_module: SemanticModule, sample_rate=24000
):
