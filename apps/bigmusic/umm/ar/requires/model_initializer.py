...
from apps.bigmusic.umm.ar.lightning.ssl_frontend import SSLFrontend
...
def _init_cached_mulan(hpath, local_rank, cache_dir=None, version="149", prefix=""):
    if version in ["149"]:
        ...
        from .mulan.mulan_infer_filmgen import create_mulan_model, mulan_inference
...
def init_t5_encoder(hpath, local_rank, cache_dir=None):
    ...
    # loading our model weights
    model = (
        T5EncoderModel.from_pretrained("t5-small", cache_dir=cache_dir)
        .eval()
        .to(device)
    )
    return {"t5": model}
