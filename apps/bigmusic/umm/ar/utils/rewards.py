...
from apps.bigmusic.umm.ar.datasets.transforms.structure import (
    ChorusDetectionTransform,
    IntensityTransform,
)
from apps.bigmusic.umm.ar.utils.utils import dump_wav
from apps.bigmusic.umm.ar.utils.metrics_asr import edit_distance, remove_punc_case
...
def mulan_audio_reward(
    ...
    target_embeds_reshaped = target_embeds.repeat(1, beam).reshape(
        batch_size * beam, -1
    )
...
def mulan_text_reward(
    ...
    if target_embeds is None:
        target_embeds = mulan_infer_fn(
            model=mulan_model, text=target_text, device=device
        )
    batch_size, beam = _infer_batch_beam(sampled_embeds, target_embeds)
    # (batch_size, D) --> (batch_size * beam, D)
    target_embeds_reshaped = target_embeds.repeat(1, beam).reshape(
...
@torch.no_grad()
def wer_reward(
    sampled_lyrics, ref_lyrics, device  # (batch_size * beam,)  # (batch_size,)
):
...
def loudness_reward(
    ...
    target_loudness = (
        target_loudness.reshape(batch_size, 1)
        .repeat(1, beam)
        .reshape(batch_size * beam)
    )
...
@torch.no_grad()
def chord_reward(
    chord_model, chord_lms, sampled_audio, chord_lm_keys, sample_rate, device
):
    if sample_rate != chord_model._sample_rate:
        resampled_audio = resample(
            sampled_audio, orig_freq=sample_rate, new_freq=chord_model._sample_rate
        )
    else:
        resampled_audio = sampled_audio
    chord_labels = chord_model.predict_step(batch=(resampled_audio, None), batch_idx=0)
...
@torch.no_grad()
def structure_reward(structure_model, sampled_audio, sample_rate, device):
    def dedup(lst):
        lst_dedup = []
        for x in lst:
    ...
    all_structure_labels = structure_model.predict_step(
        batch=(resampled_audio,), batch_idx=0
    )
    ...
    for structure_labels in all_structure_labels:
        structure_labels = [
            ...
            if x["funct_name"] in {"intro", "verse", "chorus"}
        ]
...
@torch.no_grad()
def chorus_sim_reward(sampled_audio, ref_choruses, sample_rate, device, debug=False):
...
def intensity_sim_reward(
    ...
    hyp_intensity = [
        intensity_transform.get_intensity(audio) for audio in sampled_audio
    ]
...
def semantic_diversity_reward(umm_tokens, device):
    ...
    for i in range(len(umm_tokens)):
        semantic_diversity_rewards[i] = (
            float(len(umm_tokens[i].unique())) / umm_tokens.shape[-1]
        )
...
def semantic_diversity_sim_reward(hyp_umm_tokens, ref_umm_tokens, device):
    ...
    for i, hyp in enumerate(hyp_umm_tokens):
        ...
        semantic_diversity_sim_rewards[i] = 1 - multiplier * abs(
            hyp_diversity - ref_diversity
        )
