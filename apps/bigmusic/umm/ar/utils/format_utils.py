...
def sa_music_tagging_to_style_text(
    music_tagging: Dict, sinking_threshold: float
    ...
    tags = [
        (
            quality
            if item == "MusicLowQuality"
            else map_tag(parse_result(music_tagging[item]["result"]))
        )
        for item in order
    ]
    unfamiliar_tags = [tag for tag in tags if tag not in SA_CAT_VOCAB]
    return (
        "|".join([(tag if tag in SA_CAT_VOCAB else "") for tag in tags]),
        unfamiliar_tags,
        is_sinking,
    )
...
def normalize_text(text, enable_punctuation=False, lowercase=False):
    ...
    nlp_punctuation = nlp_punctuation.replace("[", "").replace(
        "]", ""
    )  # allow brackets for structure tags
    nlp_punctuation = nlp_punctuation.replace("<", "").replace(
        ">", ""
    ...
    for line in lines:
        ...
        normalized_lines.append(
            add_section_tag(
                section_tag,
                add_singer_tag(
                    singer_tag, normalize_text(rest_line, enable_punctuation, lowercase)
                ),
            )
        )
