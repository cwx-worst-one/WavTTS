...
class Wav2Lyrics:
    ...
    def __init__(self, model_name: str = "en_punc", device_id=None):
        ...
        with pypetrel.asr.OfflineRecognizer(num) as recognizer:
            ...
            while num_recognized < recognizing_idx or has_next:
                while recognizing_idx - num_recognized < num and has_next:
                    ...
                    wav_input.set_sample_rate(
                        self._sample_rate
                    )  # "target sample rate". Must be set to 16k. Model will resample anyways
...
def init_asr(hpath, local_rank, cache_dir=None):
    wav2lyrics_module = Wav2Lyrics(model_name=hpath, device_id=local_rank)
...
def asr_transcribe_lyrics(requires, wav_batch, sample_rate, sample_lengths=None):
    ...
    """
    return requires["asr"](
        wav_batch, sample_rate=sample_rate, sample_lengths=sample_lengths
    )["lyrics"]
