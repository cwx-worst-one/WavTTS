...
from apps.bigmusic.umm.ar.utils.utils import (
    format_name,
    generate_hash,
    save_wav,
    slugify,
...
def format_lyrics_and_style(style_text, lyrics=None):
    ...
    text_encoded = generate_hash(
        text_combined
    )  # this function was already there for MusicLM
    name_formatted = (
        text_formated[:32] + "_" + lyrics_formated[:96] + "_" + text_encoded[:4]
    )
...
def save_batch_outputs(
    ...
    for i, wav in enumerate(wavs):
        ...
        lyrics_normalized_str = (
            lyrics_normalized_text[ii]
            if "lyrics_tokens" in conditions and lyrics_normalized_text
            else None
        )
        style_text = prompts[ii] if "style_text" in conditions else None
        style_category = (
            style_categories[ii]
            if "style_category" in conditions and style_categories
            else None
        )
        structure = structures[ii] if "structure" in conditions else None
        if index is None:
            file_name = (
                f"{absolute_idx:03d}_{format_lyrics_and_style(style_text, lyrics_str)}"
            )
        else:
            file_name = index[ii]
        ...
        if save_semantic_tokens and semantic_tokens is not None:
            semantic_tokens_fp = os.path.join(
                wav_dir, f"{wav_file_name}.semantic_tokens.pt"
            )
...
class NormVolumeCallback(pl.Callback):
    def on_predict_end(
        ...
        for idx, generated_output_fp in enumerate(generated_output_fps):
            ...
            command = "ffmpeg-normalize '%s' -t %d -ext wav -ar %d -o '%s' -f" % (
                generated_output_fp,
                -16,
                sample_rate,
                generated_output_fp,
...
def format_video_text(metadata, max_width=50):
    ...
    if metadata.get("style_text") is not None:
        ...
        style_text = "\n".join(
            textwrap.wrap(style_text, max_width, break_long_words=False)
        )
...
def save_video(
    input_results_dir,
    output_video_dir,
    format_video_text_fn=default_format_video_text,
    ...
    if len(mp3_fps) > 0 and len(generated_output_fps) == 0:
        raise Exception(
            "Save video does not support mp3 outputs. Encoding to video causes artifacts"
        )
...
def run_average_metrics(output_dir):
    ...
    for idx, metadata_fp in enumerate(metadata_fps):
        ...
        category2wer[str(output_dir)].append(
            [wer, ins, subs, dels, mcs]
