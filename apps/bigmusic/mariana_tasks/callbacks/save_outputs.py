...
from hdfs_io import hopen, hput
...
class CacheSemanticTokensCallback(pl.Callback):
    ...
    def on_predict_batch_start(
        ...
        if len(all_semantic_tokens) == len(batch["index"]):
            batch["semantic_tokens"] = torch.stack(all_semantic_tokens).to(pl_module.device)
    ...
    def on_predict_batch_end(
        ...
        if pl_module.extra_params.get('cached_sementic_tokens_to_hdfs', False):
            ...
            hput(output_dir, pl_module.extra_params.hdfs_output_dir)
