...
from recipes.umm2.models.umm_fm import UMMModified_no_VQ as UMM
...
_umm_network_config = {
    ...
    "act_fn": "gelu",
    "mask_mel": False,
    "rope_enhance_pos": 18000,  #was 7500
    "add_ctc": False,
    "add_chroma": False,
    "add_mel": False,
    "add_pitch": False,
...
class MaskedAvgPool1d(nn.Module):
    ...
    def forward(self, x: torch.Tensor, mask: torch.Tensor) -> "Tuple[torch.Tensor, torch.Tensor]":
        # x: (B, T, D), mask: (B, T)
        if mask is None:
            pooled = nn.AdaptiveAvgPool1d(self.output_size)(x.transpose(1, 2)).transpose(1, 2)
            new_mask = torch.ones(pooled.shape[0], pooled.shape[1], device=pooled.device, dtype=torch.bool)
            return pooled, new_mask
        bool_mask = mask.dtype == torch.bool
        if bool_mask:
            mask = (~mask).float()
        x = x.masked_fill(mask.unsqueeze(-1) == 0, 0)
        pooled = x.sum(dim=1) / (mask.sum(dim=1, keepdim=True) + 1e-8)
        if self.output_size > 1:
            # This is not a trivial operation, so we don't support it for now.
            raise NotImplementedError("MaskedAvgPool1d with output_size > 1 is not supported.")
        new_mask = torch.ones(x.shape[0], 1, device=x.device, dtype=torch.bool)
        return pooled.unsqueeze(1), new_mask
...
class AudioAdapter(nn.Module):
    def __init__(self, downsample_size, input_size, output_size):
        ...
        if downsample_size > 1:
            ...
            self.downsampler = MaskedAvgPool1d(1)
...
def unified_repetition_cleaner(
    text: str,
    n_gram_sizes: list = [5, 4, 3, 2, 1],
    top_n_suspects: int = 5,
    max_consecutive_repeats: int = 3
) -> str:
    """
    一个健壮且统一的重复清洗器（V10 修复版）。
    1. 能处理单字符无缝重复 (e.g., "啊啊啊啊")。
    2. 能处理多字符中文短语无缝重复 (e.g., "该谈谈该谈谈")。
    3. 能处理带分隔符的短语重复 (e.g., "你好，你好", "la la la")。
    4. [修复] 修正了对连续单中文字符（如 "耶耶耶"）错误使用单词边界 \b 的问题。
    """
    # 模块1: 预处理连续的单字符重复 (逻辑正确，保持不变)
    single_char_pattern = re.compile(r"(.?)\1{" + str(max_consecutive_repeats) + r",}")
    def single_char_replacement(match):
        char = match.group(1)
        if char.strip() == "":
            return match.group(0)
        return char * max_consecutive_repeats
    cleaned_text = single_char_pattern.sub(single_char_replacement, text)

    # 模块2: 按 N-gram 处理复杂的短语重复
    for n in n_gram_sizes:
        try:
            words = re.findall(r"[\w'-]+|[\u4e00-\u9fa5]", cleaned_text.lower())
        except Exception:
            return cleaned_text
        ...
        suspects = [ngram for ngram, count in ngram_counts.most_common(top_n_suspects) if count > max_consecutive_repeats]

        for suspect in suspects:
            is_chinese_char_only = bool(re.fullmatch(r'[\u4e00-\u9fa5]+', suspect.replace(" ", "")))
            
            flexible_separator = r"[\s,.;:!?\"'()\[\]-]+"
            
            # ======================= [核心修复区域 V10] =======================
            # 新增一个判断条件，用于识别“单个中文字符”这一特殊情况
            is_single_chinese_char = (n == 1 and is_chinese_char_only)
            
            if is_chinese_char_only and n > 1:
                # 逻辑不变: 处理多字符中文短语，如 "你 好"
                chars_in_suspect = suspect.split()
                atomic_pattern = r"\s*".join([re.escape(c) for c in chars_in_suspect])
                separator = flexible_separator
            
            elif is_single_chinese_char:
                # [新增逻辑]: 处理单个中文字符，如 "耶"。
                # 关键：不使用单词边界 \b，因为它在 "耶耶耶" 内部无法匹配。
                atomic_pattern = re.escape(suspect)
                separator = flexible_separator
            
            else:
                # 原始逻辑: 处理英文、中英混合等，此时使用 \b 是正确的。
                # 例如，可以防止 "is" 匹配 "this" 中的 "is"。
                words_in_suspect = suspect.split()
                atomic_pattern = flexible_separator.join(
                    [r"\b" + re.escape(w) + r"\b" for w in words_in_suspect]
                )
                separator = flexible_separator
            # =================================================================
            ...
            pattern = re.compile(
                r"(" + atomic_pattern + r")" +
                r"((?:" + separator + atomic_pattern + r")+)",
                re.IGNORECASE
            )
            
            def create_replacement_func(ap):
                def replacement_func(match):
                    full_match_text = match.group(0)
                    num_repeats = len(list(re.finditer(ap, full_match_text, re.IGNORECASE)))

                    if num_repeats > max_consecutive_repeats:
                        first_unit_match = re.search(ap, full_match_text, re.IGNORECASE)
                        first_unit = first_unit_match.group(0)
                        
                        separator_match = re.search(separator, full_match_text[first_unit_match.end():])
                        if separator_match:
                            sep = separator_match.group(0)
                        else:
                            sep = " "
                            
                        return first_unit + (sep + first_unit) * (max_consecutive_repeats - 1)
                    else:
                        return match.group(0)
                return replacement_func

            cleaned_text = pattern.sub(create_replacement_func(atomic_pattern), cleaned_text)
...
class SemanticLlmModel(CruiseModule):
    def __init__(
        ...
        with init_on_device(torch.device('cpu')): # if DIST_ENV.local_rank == 0 else init_empty_weights():
            ...
            self.audio_adapter = AudioAdapter(
                                 downsample_size=umm_config.downsample_size,
    ...
    def update_process_group(self):
        ...
        if self.hybrid_shard_group_size >= DIST_ENV.world_size:
            ...
            if DIST_ENV.world_size > 1 and self.hybrid_shard_group_size > 1:
                ...
                timeout_seconds = int(os.environ.get("CRS_NCCL_TIMEOUT_SECOND", 3600))
                # create shard_group
    ...
    def calc_loss_acc(self, shift_hidden_states, shift_labels, loss_mask,require_eos_acc=False, eos_index_window=1):
        ...
        if self.hparams.network.get('tie_weight', False):
            loss, acc = self.gpt2.transformer.wte(
                    ...
                    use_flash_ce=self.hparams.network.get('use_flash_ce', False),
                )
        else:
            #[Deprecated] P6 version
            loss, acc = self.gpt2.lm_head(
                shift_hidden_states.view(-1, shift_hidden_states.size(-1)).to(torch.float32),
                return_cross_entropy_loss=True,
                labels=shift_labels.view(-1),
                use_flash_ce=self.hparams.network.get('use_flash_ce', False),
            )
    ...
    def prepare_training_inputs(self, batch):
        ...
        # logger.info(audio_proj.shape)
        # logger.info(audio_mask.sum(dim=1))
        ...
        # logger.info(f"audio length {round(batch['audio'].shape[-1]/24000, 1)} sec, {hidden_states.shape}") 
        # logger.info(f"audio_attention: {audio_mask.sum(dim=1)}")
        # logger.info(f"prefix_mask: {prefix_mask.sum(dim=1)}")
        # logger.info(f"suffix_mask: {suffix_mask.sum(dim=1)}")
        # logger.info(f"target_mask: {target_mask.sum(dim=1)}")
        # logger.info(f"Length: {first_true_indices}")
    ...
    def training_step(self, batch, batch_idx):
        outputs = {}
        is_problematic_on_this_rank = False
        ...
        try:
            ...
            if loss is not None and (torch.isnan(loss) or torch.isinf(loss)):
                is_problematic_on_this_rank = True
                logger.warning(f"!!!!!! NaN/Inf LOSS DETECTED at global_step: {self.trainer.global_step} on Rank {self.trainer.global_rank} !!!!!!")

        except Exception:
            is_problematic_on_this_rank = True
            logger.error(
                f"!!!!!! EXCEPTION during forward pass at global_step: {self.trainer.global_step} on Rank {self.trainer.global_rank} !!!!!!",
                exc_info=True
            )

        # Synchronize problem status across all ranks
        is_problematic_tensor = torch.tensor(float(is_problematic_on_this_rank), device=self.device)
        if torch.distributed.is_initialized():
            torch.distributed.all_reduce(is_problematic_tensor, op=torch.distributed.ReduceOp.MAX)

        # If any rank had a problem, all ranks will skip the batch.
        if is_problematic_tensor.item() > 0:
            rank = self.trainer.global_rank
            logger.warning(f"Skipping batch update at global_step {self.trainer.global_step} on rank {rank} due to an error on at least one rank.")
            
            # Save the batch for debugging, but only on the rank that actually had the issue.
            if is_problematic_on_this_rank:
                problem_batch_path = f'problem_batch_step_{self.trainer.global_step}_rank_{rank}.pt'
                try:
                    torch.save(batch, problem_batch_path)
                    logger.warning(f"Saved problematic batch from Rank {rank} to: {problem_batch_path}")
                except Exception as save_e:
                    logger.error(f"Failed to save problematic batch on Rank {rank}: {save_e}")

            # Explicitly delete the output from the failed forward pass to release the computation graph.
            # This is necessary to prevent OOM on the next step.
            del outputs

            # Clear CUDA cache to free up memory from the failed forward pass.
            torch.cuda.empty_cache()

            # Create a dummy loss to allow backward pass to proceed, resulting in zero gradients.
            trainable_params = [p for p in self.parameters() if p.requires_grad]
            if trainable_params:
                dummy_loss = sum(p.sum() for p in trainable_params) * 0.0
            else:
                # Fallback if no trainable params are found
                dummy_loss = torch.tensor(0.0, device=self.device, requires_grad=True)
            
            # Re-initialize the outputs dictionary and populate it with default values for the trainer.
            # This prevents both the UnboundLocalError and potential KeyErrors in the trainer's logging logic.
            lr = self.trainer.optimizers[0].param_groups[0]['lr'] * 1000 if self.training and self.trainer.optimizers else 0.0
            outputs = {
                "loss": dummy_loss,
                "acc": torch.tensor(0.0, device=self.device),
                "eos_acc": torch.tensor(0.0, device=self.device),
                "loss_tokens": torch.tensor(1.0, device=self.device), # Use 1 to avoid division by zero in meters
                "tokens": torch.tensor(1.0, device=self.device), # Use 1 to avoid division by zero in meters
                "loss_tokens(B)": torch.tensor(0.0, device=self.device),
                "consume_tokens(B)": torch.tensor(0.0, device=self.device),
                "flops": torch.tensor(0.0, device=self.device),
                "lr * 1e3": torch.tensor(lr, device=self.device),
            }
            return outputs
    ...
    def validation_step(self, batch, batch_idx):
        ...
        # infer_inputs = self.prepare_inference_inputs(batch)
        # outputs = self.gpt2(
        #         input_ids=None,
        #         is_inference=True, 
        #         inputs_embeds=infer_inputs['token_embeds'],
        #         inputs_embeds_mask=infer_inputs['attention_mask'], 
        ...
        # logger.info(f"output_ids: {output_ids.shape}")

        # for i, (uttid, prediction) in enumerate(zip(batch['uttid'], output_texts)):
        #     try:
        #         prediction = re.sub(r'\[Lyrics:\s*(.*?)\]', r'\1', str(prediction), flags=re.DOTALL)
        #         prediction = remove_last_unpaired_closing_bracket(prediction)
        #         pred = merge_consecurtive_sections(prediction)
                
        #         if 'mir' in batch:
        #             mir_lyrics = re.sub(r'\[Lyrics:\s*(.*?)\]', r'\1', batch['mir'][i], flags=re.DOTALL)
        #             refer = merge_consecurtive_sections(mir_lyrics)
        #             if random.random() < 0.0:
        #                 logger.info('\n\t' + prediction.replace('\n', '\n\t'))
        #                 logger.info('\n' + mir_lyrics)
        #             scores = evaluate_section(pred, refer, wer_th=0.3)
        #             for metric, value in scores.items():
        #                 if value is not None:
        #                     outputs[metric] = value
                            
        #     except Exception as e:
        #         logger.info(f"{uttid} Parse error: {e}")
    ...
    def prepare_inference_inputs(self, batch):
        ...
        total_length = torch.stack(total_length_list)
    ...
    def predict_step(self, batch, batch_idx=0):
        ...
        original_uttids = batch['uttid']
        rank_idx = torch.distributed.get_rank() if torch.distributed.is_initialized() else 0

        # === cache ===
        indices_to_keep = [
            i for i, uttid in enumerate(original_uttids)
            if (not (self.rank_result_file.parent / f"{uttid}_True.json").exists()
            and not (self.rank_result_file.parent / f"{uttid}_False.json").exists())
        ]

        if not indices_to_keep:
            logger.info(f"Rank {rank_idx}: All {len(original_uttids)} UTTIDs in this batch already exist. Skipping.")
            return

        if len(indices_to_keep) < len(original_uttids):
            skipped_count = len(original_uttids) - len(indices_to_keep)
            logger.info(f"Rank {rank_idx}: Skipping {skipped_count} existing UTTIDs. Processing {len(indices_to_keep)} new UTTIDs.")
        ...
        indices_to_keep_tensor = torch.tensor(indices_to_keep, device=device)

        for key, value in batch.items():
            if isinstance(value, (list, torch.Tensor)) and len(value) == len(original_uttids):
                if isinstance(value, torch.Tensor):
                    batch[key] = value.index_select(0, indices_to_keep_tensor)
                elif isinstance(value, list):
                    batch[key] = [value[i] for i in indices_to_keep]
        # === end cache ===
        ...
        # with retry
        if output_ids.shape[-1] == self.inference_config.max_new_tokens:
            logger.info(f"Rank {rank_idx}: UTTID {batch['uttid'][0]} generated {self.inference_config.max_new_tokens} tokens.")

            local_generation_config = deepcopy(self.generation_config)
            local_generation_config.repetition_penalty = 1.6  # may affect [inst]
            ...
            outputs = self.gpt2(
                    ...
                    inputs_embeds=input_embed,
                    inputs_embeds_mask=attention_mask, 
                    generation_config=local_generation_config)
        ...
        for i, (uttid, prediction) in enumerate(zip(batch['uttid'], batch['predict_results'])):

            prediction_raw = deepcopy(prediction)

            # remove repetition
            prediction = unified_repetition_cleaner(prediction_raw,     
                                                n_gram_sizes=list(range(1, 15)),
                                                top_n_suspects=20,
                                                max_consecutive_repeats=8)

            prediction = prediction.replace("男：", "").replace("女：", "").replace("伴；", "").replace("多：", "").replace("伴：", "")
            prediction = prediction.replace("[bridge]\n[", "[inst]\n[")
            if prediction.startswith("[verse]"):
                prediction = '[intro]\n' + prediction
            if '[outro]' not in prediction and not prediction.endswith(']'):
                prediction = prediction + '\n[outro]'
            
            prediction = zhconv.convert(prediction, 'zh-cn')

            logger.info(f"UTTID: {uttid}")
            logger.info("-" * 50)
            logger.info(f"Prediction: {prediction}")

            good_case = True
            if output_ids[i].shape[0] == self.inference_config.max_new_tokens:
                good_case = False

            # wrtie a json with uttid
            uttid_json_path = self.rank_result_file.parent / f"{uttid}_{good_case}.json"
          
            # Parse and format the prediction results nicely
            try:
                if 'mir' in batch:
                    prediction = re.sub(r'\[Lyrics:\s*(.*?)\]', r'\1', prediction, flags=re.DOTALL)
                    # prediction = remove_last_unpaired_closing_bracket(prediction)
                    mir_lyrics = re.sub(r'\[Lyrics:\s*(.*?)\]', r'\1', batch['mir'][i], flags=re.DOTALL)
                    if random.random() < 0.05: # sample cases for display
                        logger.info(f"UTTID: {uttid}")
                        logger.info('\n\t' + prediction.replace('\n', '\n\t'))
                        logger.info('\n' + mir_lyrics)
                    self.section_scores.append([uttid, {}, prediction, mir_lyrics])

                elif 'label' in batch:
                    label = batch['label'][i]
                    # logger.info(f"Label: \n{label}")

                with open(uttid_json_path, 'w', encoding='utf-8') as f:
                    json.dump({
                                'uttid': uttid,
                                'predict_results': prediction,
                                'lyrics': label,
            ...
            pickle.dump(self.section_scores, open(f'section_scores_{DIST_ENV.local_rank}.pkl', 'wb'))
    ...
    def on_predict_epoch_end(self, outputs, batch_idx=0):
        rank_idx = torch.distributed.get_rank() if torch.distributed.is_initialized() else 0
        logger.info(f"Rank {rank_idx}: Prediction completed.")
...
class SemanticLlmTrainer(AudioTrainer):
    ...
    #('evaluate', {'type': 'Weighted', 'args': ["lyrics_wer", "section_wer", "bound_precision", "bound_recall", "label_precision", "label_recall"]}),
    valid_meters = [
        ('loss', {'type': 'Weighted', 'args': ['loss', 'loss_tokens']}),
        ('acc', {'type': 'Weighted', 'args': ['acc', 'loss_tokens']}),
        ('lwer', {'type': 'Mean', 'args': ["lyrics_wer"]}),
        ('swer', {'type': 'Mean', 'args': ["section_wer"]}),
        ('eos_acc',{'type': 'Weighted', 'args': ['eos_acc', 'tokens']} ),
        ('loss_tokens(B)', {'type': 'Sum', 'args': ['loss_tokens(B)']}),
        ('consume_tokens(B)', {'type': 'Sum', 'args': ['consume_tokens(B)']}),
        ('flops', {'type': 'Flops', 'args': ['flops']}),
