...
class TokenBuffer:
    def __init__(
        ...
        self.step_out_blank_version = step_out_blank_version
        self.buffer: List[int] = []
        self.max_repeat_times = max_repeat_times
        self.eos_id = eos_id
        self.verbose = verbose
        process_duplicate_fns = {
            "v1": self.process_token_v1,
            "v2": self.process_token_v2,
            "v3": self.process_token_v3,
        }
        if step_out_blank_version not in process_duplicate_fns:
            raise ValueError(
                f"only support step_out_blank_version={process_duplicate_fns.keys()}"
            )
        self.process_duplicate_fn = process_duplicate_fns[step_out_blank_version]
    ...
    def process_token(self, step, predict_token, sampler, **sampler_kwargs):
        return self.process_duplicate_fn(step, predict_token, sampler, **sampler_kwargs)

    def process_token_v1(self, step, predict_token, sampler, **sampler_kwargs):
        self.put(predict_token)
        ...
        while self.is_duplicate():
            count = 0
            while (
                count < self.max_retry_times and self.is_duplicate()
            ) or predict_token == self.eos_id:
                if self.verbose:
                    logger.warning(
    ...
    def process_token_v2(self, step, predict_token, sampler, **sampler_kwargs):

        logits: Optional[torch.Tensor] = sampler_kwargs.pop("logits", None)
        assert logits is not None, "logits must be provided"

        while self.buffer.count(predict_token.item()) > self.max_repeat_times - 1:
            logits[:, predict_token] = -float("Inf")
            predict_token = sampler(
                logits=logits, temp=self.init_temperature, **sampler_kwargs
            )
            predict_token = predict_token[:, None]
            if predict_token.item() == self.eos_id:
                break

        self.put(predict_token.item())
        return predict_token

    def process_token_v3(self, step, predict_token, sampler, **sampler_kwargs):
