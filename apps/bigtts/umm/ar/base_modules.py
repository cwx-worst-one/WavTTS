...
class BaseContinuousEmbedModule(BaseModule):
    ...
    def predict(
        ...
        if not isinstance(self.model, gpt.GPTLMHeadModel):
            for i in pbar:
                pbar.set_description(f"{tqdm_name} [0 - {num_tokens}], mode: {sample_mode}, temp: {temperature}")
            ...
            for i in pbar:
                ...
                predict_token = self.sample_logits(i, logits.float(), temperature, sample_mode)
