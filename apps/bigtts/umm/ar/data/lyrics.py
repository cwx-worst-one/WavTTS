...
class LyricsTokenTransform():
    ...
    def __call__(self, item):
        ...
        try:
            lyrics_tokens = pad_crop(torch.tensor(lyrics_tokens), padding_length + len(lyrics_tokens), torch.int, padding_value=self.pad_id)
        except:
            lyrics_tokens = phones
            lyrics_tokens = pad_crop(torch.tensor(lyrics_tokens), padding_length + len(lyrics_tokens), torch.int, padding_value=self.pad_id)
        phones = pad_crop(torch.tensor(phones), padding_length + len(phones), torch.int, padding_value=self.pad_id)
        tones = pad_crop(torch.tensor(tones), padding_length + len(tones), torch.int, padding_value=self.pad_id)
        wordsegs = pad_crop(torch.tensor(wordsegs), padding_length + len(wordsegs), torch.int, padding_value=self.pad_id)
        prompt_text_lens = torch.tensor(prompt_text_lens)
        # return { **item, 'lyrics_tokens': lyrics_tokens, 'lyrics_normalized_text': lyrics_text }
        return { **item, 'lyrics_tokens': lyrics_tokens, 'lyrics_normalized_text': lyrics_text,
                'phones': phones, 'tones': tones, 'wordsegs': wordsegs,
    ...
    def init_sami_tokenizer(cls, lyrics_max_seq_len, test_wer, **kwargs):
        with rank_zero_first():
            phone2id = PhoneToId()
            phone_tone_wordseg_dict = torch.load('apps/bigtts/umm/ar/data/fronted_v3_2_dict.pyt')
            sami_tokenizer = SAMITokenizer(phone2id, phone_tone_wordseg_dict, test_wer)
        return LyricsTokenTransform(lyrics_tokenizer=sami_tokenizer,
                                    pad_id=0,
...
class SAMITokenizer():
    ...
    def __call__(self, label_path):
        if '|' not in label_path: # no prompt
            ...
            text_id_phones_tones_infer = self.phone2id.convert_tacolab_to_text_id_infer(infer_labels)
            text_id_infer, _, _, _, _ = text_id_phones_tones_infer
            phones, tones, word_segs = text_id_infer[0], text_id_infer[1], text_id_infer[2]
            ret_dict = {}
        ...
        text_id_phones_tones_prompt = self.phone2id.convert_tacolab_to_text_id_infer(prompt_labels)
        text_id_prompt, _, _, _, _ = text_id_phones_tones_prompt
        phones, tones, word_segs = text_id_prompt[0], text_id_prompt[1], text_id_prompt[2]
        ...
        text_id_phones_tones_infer = self.phone2id.convert_tacolab_to_text_id_infer(infer_labels)
        text_id_infer, _, _, _, _ = text_id_phones_tones_infer
        phones_infer, tones_infer, word_segs_infer = text_id_infer[0], text_id_infer[1], text_id_infer[2]
