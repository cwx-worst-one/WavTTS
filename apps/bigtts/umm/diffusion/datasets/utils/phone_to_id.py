...
from .frontend import phone_to_int, tone_to_int, wordseg_to_int, lang_to_int
...
class PhoneToId():
    ...
    def convert_v3_to_v1(self, tacolab):
        ...
        for x in tacolab:
            ...
            if len(x_split) == 7:
                ...
                print("convert_v3_to_v1: Wrong tacolab", x_split)
                return None
    ...
    def convert_tacolab_to_text_id(self, tacolab):
        try:
            ...
            return np.stack([phone_ids, tone_ids, wordseg_ids, lang_ids]), phones, tones, wordsegs, alignments
    ...
    def convert_tacolab_to_text_id_infer(self, tacolab):
        # try:
            ...
            if lang == 'zh':
                ...
                for i in range(len(tacolab)):
                    ...
                    if len(x_split) == 7:
                        ...
                        print("convert_tacolab_to_text_id_infer:Wrong tacolab", x_split)
                        return None
                    assert phone in self.phone_to_int, f"{phone} not in phone set"
                    assert tone in self.tone_to_int, f"{tone} not in tone set"
                    phone_ids.append(self.phone_to_int[phone])
            ...
            phone_ids, tone_ids = np.array(phone_ids), np.array(tone_ids)
            wordseg_ids = np.array(wordseg_ids)
            return np.stack([phone_ids, tone_ids, wordseg_ids, lang_ids]), phones, tones, wordsegs
