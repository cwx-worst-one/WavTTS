...
class VAET2SLangSpkModule(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        for i in range(bsz):
            targets_dense.append(
                F.pad(
                    bns[i, : bn_lens[i], :],
                    (0, 0, text_spk_lens[i], seq_len - (bn_lens[i] + text_spk_lens[i])),
                    "constant",
                    0,
                )
            )
    ...
    def inference_from_text_streaming(self, batch, bpe_tokenizer):
        ...
        for split in range(len(text_lenses)):
            ...
            infer_phone = (
                torch.from_numpy(phones[split]).long().unsqueeze(0).to(bns.device)
            )
            infer_tone = (
                torch.from_numpy(tones[split]).long().unsqueeze(0).to(bns.device)
            )
            ...
            bpe_seq = (
                torch.from_numpy(
                    np.asarray(
                        bpe_tokenizer(text, truncation=True, max_length=8000).input_ids
                    )
                )
                .unsqueeze(0)
                .to(bns.device)
            ...
            frontend_inputs = {"phone": phone, "tone": tone, "phonetone": None}
            b = bns.shape[0]
            ...
            z_outputs_all.append(z_outputs)
            # if z_outputs_all is None:
            #     z_outputs_all = z_outputs
            # else:
            #     z_outputs_all = torch.cat([z_outputs_all, z_outputs], dim=1)

            semantic_outputs_all.append(semantic_outputs)
            # if semantic_outputs_all is None:
            #     semantic_outputs_all = semantic_outputs
            # else:
    ...
    @torch.no_grad()
    def inference_from_text_byt5(self, batch, tokenizer):
        bns = batch["bn"]
        text_lens, bn_lens = batch["text_lens"], batch["bn_lens"]
        lang_seq, infer_lang_id = batch["lang_seq"], batch["infer_lang_id"]
        spk_seq, infer_spk_id = batch["spk_seq"], batch["infer_spk_id"]
            ...
            for i in tqdm(range(max_step)):
                if i == 0:
                    model_outputs = self.model(
                        ...
                        lang_seqs=lang_seq,
                        spk_seqs=spk_seq,
                        byt5_embeds=batch.get("byt5", None),
                        byt5_lens=batch.get("byt5_lens", None),
                        tag_ids=batch.get("tag_id", None),
                    )
                    input_len = text_lens[0] + bn_lens[0]
                    if self.use_spk_id and self.spk_type == "concat":
    ...
    predict_byt5 = inference_from_text_byt5
