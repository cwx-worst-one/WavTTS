...
from apps.bigtts.wvae.ar.infer.lit_infer import BigTTSWVAEInfer
...
class BigTTSWVAEInferLangSpk(BigTTSWVAEInfer):
    def __init__(
        ...
        text2id_path="apps/bigtts/wvae/ar/resources/metaid_to_textid.json",
    ...
    def predict_step(self, batch: Any, batch_idx: int, dataloader_idx: int = 0) -> Any:
        ...
        sil_interval_by_sample = torch.from_numpy(
            np.zeros(int(self.sil_interval * 24000))
        ).to(device)
        ...
        for i in range(len(infer_texts)):
            ...
            z_outputs, _ = self.ar_model.predict_byt5(sample, None)
            ...
            if generated_wav_all is None:
                ...
                generated_wav_all = torch.cat(
                    [generated_wav_all, sil_interval_by_sample, generated_wav], axis=0
                )
        ...
        if self.infer_mode == "offline":
            ...
            generated_wav *= 32767
            write(output_path, 24000, generated_wav.cpu().numpy().astype(np.int16))
        else:
    ...
    def encode(self, sample):
        ...
        if not self.use_prompt:
            ...
            #     wav.squeeze(1), 2048, 80, 24000, 300, 1200, 0.0, None
            # )
            # _, m, logs = self.wvae_encoder(wav, spec, mel)
        ...
        # get text_id
        if (not self.use_prompt) or (self.use_ref_enc):
            if self.use_offline_tacolab:
                infer_utt = uttid
                infer_tacolab_path = os.path.join(
                    self.infer_tacolab_dir, infer_utt + ".lab"
                )
            ...
            if self.save_tacolab:
                save_infer_tacolab_dir = os.path.join(
                    self.hparams.output_dir, "../infer_tacolab"
                )
                os.makedirs(save_infer_tacolab_dir, exist_ok=True)
                infer_tacolab_path = os.path.join(
                    save_infer_tacolab_dir, uttid + ".lab"
                )
            ...
            if self.use_offline_tacolab:
                ...
                infer_utt = uttid
                prompt_tacolab_path = os.path.join(
                    self.prompt_tacolab_dir, prompt_utt + ".lab"
                )
                infer_tacolab_path = os.path.join(
                    self.infer_tacolab_dir, infer_utt + ".lab"
                )
                if not os.path.exists(prompt_tacolab_path) or not os.path.exists(
                    infer_tacolab_path
                ):
                    print(
                        "prompt_tacolab_path or infer_tacolab_path not exists, skip",
                        prompt_tacolab_path,
                        infer_tacolab_path,
                    )
                    return None
            ...
            if self.save_tacolab:
                save_prompt_tacolab_dir = os.path.join(
                    self.hparams.output_dir, "../prompt_tacolab"
                )
                os.makedirs(save_prompt_tacolab_dir, exist_ok=True)
                prompt_tacolab_path = os.path.join(
                    save_prompt_tacolab_dir, uttid + ".lab"
                )
            ...
            prompt_tacolab_list = list(
                filter(lambda x: x != "", prompt_tacolab.split("\n"))
            )
            infer_tacolab_list = list(
                filter(lambda x: x != "", infer_tacolab.split("\n"))
            )
            if not self.use_sp:
                prompt_tacolab_list = self.remove_replace_sp(prompt_tacolab_list, uttid)
                if prompt_tacolab_list is None:
                    return None
                infer_tacolab_list = self.remove_replace_sp(infer_tacolab_list, uttid)
                if infer_tacolab_list is None:
                    return None
            prompt_text_id_phones_tones = self.convert_tacolab_to_text_id(
                prompt_tacolab_list
            )
            infer_text_id_phones_tones = self.convert_tacolab_to_text_id(
                infer_tacolab_list
            )

            if (
                prompt_text_id_phones_tones is None
                or infer_text_id_phones_tones is None
            ):
                logger.warning(f"{uttid} convert_tacolab_to_text_id failed ...")
                return None
            else:
                if self.input_type == "2dim":
                    (
                        ...
                        prompt_phones,
                        prompt_tones,
                    ) = prompt_text_id_phones_tones
                    (
                        infer_text_id,
                        infer_phones,
                        infer_tones,
                    ) = infer_text_id_phones_tones
                    ...
                    text_id = np.concatenate([prompt_text_id, infer_text_id], axis=-1)
                    phonetone_ids = np.concatenate(
                        [prompt_phonetone_ids, infer_phonetone_ids], axis=-1
                    )
                    phones = prompt_phones + infer_phones
                    tones = prompt_tones + infer_tones
        ...
        if self.input_type == "1dim":
            ...
            data_dict["phonetone"] = (
                torch.from_numpy(data_dict["phonetone"]).long().unsqueeze(0).to(device)
            )
        ...
        if self.use_spk_id:
            ...
            infer_spk_key = (
                self.infer_spk_name if self.infer_spk_name is not None else spk
            )
        ...
        data_dict["phone"] = (
            torch.from_numpy(text_id[0, :]).long().unsqueeze(0).to(device)
        )
        data_dict["tone"] = (
            torch.from_numpy(text_id[1, :]).long().unsqueeze(0).to(device)
        )
        ...
        if self.use_lang_id:
            data_dict["lang_seq"] = (
                torch.tensor(lang_seq).long().to(device).unsqueeze(0)
            )
        ...
        if self.use_bpe:
            data_dict["bpe_seq"] = (
                torch.from_numpy(
                    np.asarray(
                        self.bpe_tokenizer(
                            infer_text, truncation=True, max_length=self.max_length
                        ).input_ids
                    )
                )
                .unsqueeze(0)
            ...
            with torch.no_grad():
                data_dict["byt5"] = self.text_encoder(data_dict["bpe_seq"])[0]
                data_dict["byt5_lens"] = (
                    torch.tensor([data_dict["byt5"].shape[1]]).long().to(device)
                )
            data_dict["bpe_lens"] = (
                torch.tensor([data_dict["bpe_seq"].shape[1]]).long().to(device)
            )
        else:
            data_dict["bpe_seq"] = None

        data_dict["tag_id"] = (
            torch.from_numpy(np.asarray([int(self.tag_id)])).long().to(device)
        )

        return data_dict

    def generate_tacolabels_engine(self, text_str):
        lang_key = self.get_lang_by_text_infer(text_str)
        if lang_key == None:
