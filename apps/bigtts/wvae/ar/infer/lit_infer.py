...
def model_loader(name, ckpt_path):
    ...
    if name == "VAET2SLangSpkModule":
        return VAET2SLangSpkModule.load_from_checkpoint(
            checkpoint_path=ckpt_path
        ).eval()
        # return VAET2SLangSpkModule.load_from_checkpoint(checkpoint_path=ckpt_path).to("cuda:0").eval()
    elif name == "VAET2SLangSpkModule_align":
        ref = torch.load(
            "/home/tiger/epoch=00-step=135000-kl_loss=0.33.ckpt", map_location="cpu"
        )
        hyper_parameters = ref["hyper_parameters"]
        del ref
        return VAET2SLangSpkModule.load_from_checkpoint(
            checkpoint_path=ckpt_path, **hyper_parameters
        ).eval()
    ...
    # elif name == "VAET2SLangSpkSerAliV3Module":
    #     return VAET2SLangSpkSerAliV3Module.load_from_checkpoint(checkpoint_path=ckpt_path).eval()
    # elif name == "VAET2SLangSpkSerAliV4Module":
    #     return VAET2SLangSpkSerAliV4Module.load_from_checkpoint(checkpoint_path=ckpt_path).eval()
    else:
...
class BigTTSWVAEInfer(LightningModule):
    def __init__(
        ...
        assert (
            (text2id_version == "v1" and tacolab_version == "oldv1")
        ...
        self.text2id = TextToTacolabID(
            text2id_path,
            text2id_version=text2id_version,
            use_sy=use_sy,
            tacolab_version=tacolab_version,
        )

        self.tokenizer = PhoneTokenizerWithAudioTokens(
            phone_token_num=phone_tokens_num,
            speaker_token_num=speaker_tokens_num,
        ...
        if self.use_bpe:
            ...
            if tokenizer_type == "flan-T5-large":
                ...
                self.text_encoder = T5EncoderModel.from_pretrained(bpe_dir)
    ...
    def predict_step(self, batch: Any, batch_idx: int, dataloader_idx: int = 0) -> Any:
        ...
        if self.infer_mode == "offline":
            ...
            if self.scale_generated_wav:
                generated_wav *= min(0.99, prompt_wav_max) / max(
                    0.01, np.max(np.abs(generated_wav))
                )
            ...
            if self.save_prompt:
                save_wav(
                    np.concatenate((prompt_wav, generated_wav), axis=0),
                    f"{output_dir}/prompt-{utt_ids[0]}.wav",
                    24000,
                )
        else:
            return generated_wav
    ...
    def encode(self, sample):
        ...
        if self.use_lang_id:
            wav_id = np.concatenate([[lang_id], wav_id])
