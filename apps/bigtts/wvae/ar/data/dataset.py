...
class PhoneTokenizerWithAudioTokens:
    def __init__(
        self, phone_token_num, speaker_token_num, bpe_tokens_num=0, lang_tokens_num=0
    ) -> None:
    ...
    def tokenize(self, inputs, input_key):
        if input_key == "inputs":  # text_id
            ...
            return (
                inputs
                + 1
                + self.phone_token_num
                + self.bpe_tokens_num
                + self.speaker_token_num
            )
        else:
            return None
    ...
    def get_text_wavid(self, idx):
        if self.inference:
            ...
            if len(x) == 3:
                ...
                # # 临时关prompt
                # wav_id = wav_id[:1]
                # prompt_text_id = np.concatenate([prompt_text_id[:1], prompt_text_id[-1:]])

                text_id = np.load(text_id_path)
                text_id = np.concatenate(
                    (prompt_text_id[:-1], np.array([2]), text_id[1:])
                )
...
class PhoneTokenizerWithAudioTokensSpk:
    def __init__(self, phone_token_num, audio_token_num, spk_num) -> None:
        self.audio_token_num = audio_token_num
        self.phone_token_num = phone_token_num
        self.spk_num = spk_num
        self.vocab_size = (
            audio_token_num + phone_token_num + 3 + spk_num
        )  # <s> </s>, <sep>, <pad>
        self.pad = 0
        self.bos = self.vocab_size - 1 - spk_num
        self.eos = self.vocab_size - 1 - spk_num
        self.sep = self.vocab_size - 2 - spk_num

    def tokenize(self, inputs, input_key):
        # pad, [text_id], [wav_id], sep, bos/eos, [spk_id]
        if input_key == "inputs":  # text_id
            return inputs + 1
        elif input_key == "targets":  # wav_id
            return inputs + 1 + self.phone_token_num
        elif input_key == "spks":
            return inputs + self.phone_token_num + self.audio_token_num + 2
        else:
            return None


class GPT2TTSSpkDataset(Dataset):
    def __init__(
        self,
        path,
        hp=None,
        ...
        self.hp = DotDict(hp)
        self.metas = self.get_metadata(path)
        self.tokenizer = PhoneTokenizerWithAudioTokensSpk(
            self.hp.phone_tokens_num, self.hp.audio_tokens_num, self.hp.spk_num
        )
        self.return_full_seq = return_full_seq
        self.inference = inference
        if dynamic_batch_size:
    ...
    def get_text_wav_spkid(self, idx):
        if self.inference:
            x = self.metas[idx].split("|")
            if len(x) == 4:
                wav_id_path, prompt_text_id_path, text_id_path = x[0], x[1], x[2]
            ...
            x = self.metas[idx].split("|")
            wav_id_path, text_id_path, spk = x[0], x[1], x[3]
            uttid = os.path.splitext(os.path.basename(text_id_path))[0]
            wav_id = np.load(wav_id_path)
            text_id = np.load(text_id_path)
        ...
        text_id = self.tokenizer.tokenize(text_id, "inputs")
        wav_id = self.tokenizer.tokenize(wav_id, "targets")
        spk_id = self.tokenizer.tokenize(spk_id, "spks")
        return text_id, wav_id, uttid, spk_id

    def __len__(self):
        return len(self.metas)

    def __getitem__(self, idx):
        text_id, wav_id, uttid, spk_id = self.get_text_wav_spkid(idx)
        # get len
        wav_len, num_rvqs = wav_id.shape[0], wav_id.shape[1]
        text_len = text_id.shape[0]
        ...
        if self.inference:
            ...
            # get position embedding
            pos_id = np.asarray(
                list(range(text_len + 2)) + list(range(wav_len + 1 + 1))
            )
            seq_sen_id = np.asarray([1] * (text_len + 2) + [2] * (wav_len + 1 + 1))
        ...
        if self.return_full_seq:
            ...
            if self.inference:
                ...
                full_seq = np.concatenate(
                    [bos, full_text_seq, sep, full_spk_seq, wav_id, eos], axis=0
                )
        else:
            full_seq = None
