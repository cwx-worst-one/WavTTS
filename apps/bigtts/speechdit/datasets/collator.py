...
class Collator1(CollatorBase):
    def __init__(self,
            bn_config=None,
            bn_padding=0,
    ):

        self.feat_hz = bn_config["sampling_rate"] // bn_config["hop_size"]
    ...
    def __call__(self, batches):
        ...
        min_prompt_len = min([b['prompt_bn'].shape[1] for b in batches])
        ret_dict["prompt_bn"] = collate_2d(
            [b['prompt_bn'] for b in batches],
            pad_idx=self.feat_padding,
            max_len=max_prompt_len)
        crop_len = int(max(self.feat_hz, min_prompt_len))
        start_point = 0
        ret_dict[f"prompt_bn"] = ret_dict["prompt_bn"][:, :, start_point:start_point+crop_len]
