            ...
            if isinstance(module, ConvModule):
                ...
                if original_conv_name + '.weight' not in state_dict_r2d:
                    ...
                    if shape_2d != shape_3d[:2] + shape_3d[3:]:
                        ...
                        self._inflate_conv_params(module.conv, state_dict_r2d, original_conv_name,
                                                  inflated_param_names)
                ...
                if original_bn_name + '.weight' not in state_dict_r2d:
                    ...
                    self._inflate_bn_params(module.bn, state_dict_r2d, original_bn_name,
                                            inflated_param_names)
    ...
    def __init__(self,
        ...
        for i in range(len(self.stage_blocks)):
            ...
            if lateral and i != self.num_stages - 1:
                ...
                setattr(
                    self, lateral_name,
                    ConvModule(self.inplanes // self.channel_ratio,
                               self.inplanes * 2 // self.channel_ratio,
                               kernel_size=(fusion_kernel, 1, 1),
                               stride=(self.speed_ratio, 1, 1),
                               padding=((fusion_kernel - 1) // 2, 0, 0),
    ...
    def _inflate_conv_params(self, conv3d, state_dict_2d, module_name_2d, inflated_param_names):
        ...
        if new_shape[1] != old_shape[1]:
            ...
            conv2d_weight = torch.cat(
                (conv2d_weight, torch.zeros(pad_shape).type_as(conv2d_weight).to(
        ...
        if stride != 1 or in_channels != out_channels * BottleNeck.expansion:
            self.shortcut = nn.Sequential(
                nn.Conv2d(in_channels,
                          ...
                          stride=stride,
                          kernel_size=1,
                          bias=False), nn.BatchNorm2d(out_channels * BottleNeck.expansion))
...
class ResNet(nn.Module):
    ...
    def __init__(self, block, num_block, num_classes=100, truncate_sec=4):
        ...
        self.conv1 = nn.Sequential(nn.Conv2d(1, 64, kernel_size=3, padding=1, bias=False),
...
class ActionEncoder(nn.Module):
    ...
    def forward(self, frames):
        ...
        frames = frames.permute(0, 2, 1, 3, 4)  # [b, d, t, h, w]
        video_feat = self.video_encoder(frames)  # input [b, d, t, h, w]
...
if __name__ == "__main__":
    ...
    frames = torch.rand([2, 8, 3, 224, 224])

    video_feats = action_encoder(frames)

    print(video_feats.shape)  # [b, t, 512]
