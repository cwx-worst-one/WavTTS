...
def _get_sparse_fn(size, device):
    ...
    if name not in sparse_fns:
        ...
        qk_matmul = sparse_matmul(layout,
                                  block,
                                  mode="sdd",
                                  trans_a=False,
                                  trans_b=True,
        ...
        wv_matmul = sparse_matmul(layout,
                                  block,
                                  mode="dsd",
                                  trans_a=False,
                                  trans_b=False,
    ...
    def forward(self, x, position_ids):
        if x.dim() == 3:
            ...
            if position_ids.size(0) != 1:
                position_ids = (position_ids.unsqueeze(1).repeat(1, h, 1).view([b * h, -1])
                               )  # [b, t] -> [b*h, t]
...
class Attention(nn.Module):
    ...
    def forward(
        ...
        if not self.sparse or not self.training:
            ...
            scores = softmax_fn(scores, scale=1 / math.sqrt(self.head_dim), is_causal=True)
            scores = self.attn_dropout(scores)
...
class LLaMa(nn.Module):
    ...
    def __init__(self, params: ModelArgs, provider="default"):
        super().__init__()
        self.params = params
        ...
        self.freqs_cis = precompute_freqs_cis(self.params.dim // self.params.n_heads,
                                              self.params.max_seq_len * 2)

    def init_weight_and_load_state(self, state_dict_path=None):
    ...
    def _init_weights(self, module: nn.Module) -> None:
        if isinstance(module, nn.Linear):
            torch.nn.init.normal_(module.weight, mean=0.0, std=0.02 / math.sqrt(2 * self.n_layers))
    ...
    def create_layers(self, params, provider):
        if provider == "ctiga":
            ...
            ctiga_config.cross_attn = getattr(params, "cross_attn", False)
        ...
        if self.provider == "ctiga":
            ...
            for layer in self.layers:
                if self.params.checkpointing:
                    ...
                    h = torch.utils.checkpoint.checkpoint(create_custom_forward(layer), h,
...
def checkpoint_converter(origin_ckpt_path, ctiga_ckpt_path, only_weights=False):
    ...
    def map_to_hf(origin_state_dict, params):
        ...
        for i in range(n_layer):
            ...
            qw = (qw.reshape(n_head, head_dim // 2, 2,
                             hidden_size).transpose(1, 2).reshape(hidden_size, hidden_size))
            hf_state_dict[f"model.layers.{i}.self_attn.q_proj.weight"] = qw

            kw = hf_state_dict.pop(f"{attn_layer}.wk.weight")
            kw = (kw.reshape(n_head, head_dim // 2, 2,
                             hidden_size).transpose(1, 2).reshape(hidden_size, hidden_size))
            hf_state_dict[f"model.layers.{i}.self_attn.k_proj.weight"] = kw

            hf_state_dict[f"model.layers.{i}.self_attn.v_proj.weight"] = hf_state_dict.pop(
                f"{attn_layer}.wv.weight")
            hf_state_dict[f"model.layers.{i}.self_attn.o_proj.weight"] = hf_state_dict.pop(
                f"{attn_layer}.wo.weight")
            ...
            hf_state_dict[f"model.layers.{i}.input_layernorm.weight"] = hf_state_dict.pop(
                f"{input_norm}.weight")
            hf_state_dict[f"model.layers.{i}.post_attention_layernorm.weight"] = hf_state_dict.pop(
                f"{ffn_norm}.weight")
    ...
    def map_to_ctiga(hf_state_dict, params):
        ...
        def permute(w):
            return (w.view(n_head, 2, hidden_size // n_head // 2,
                           hidden_size).transpose(1, 2).reshape(hidden_size, hidden_size))
        ...
        if only_weights:
            ...
            ctiga_state_dict["layers.ln_f.weight"] = ctiga_state_dict.pop("model.norm.weight")
            ctiga_state_dict["output.weight"] = ctiga_state_dict.pop("model.output.weight")
            ctiga_state_dict["prenet.weight"] = ctiga_state_dict.pop("model.prenet.weight")
            ctiga_state_dict["h_output.weight"] = ctiga_state_dict.pop("model.h_output.weight")
        else:
            ctiga_state_dict["model.layers.ln_f.weight"] = ctiga_state_dict.pop("model.norm.weight")
        from_prefix = "model.layers.{}.{}"
        to_prefix = ("layers.layers.{}.{}" if only_weights else "model.layers.layers.{}.{}")
        for layer_idx in range(n_layer):
            # RMS-Norm
            # input_layernorm -> norm1
            ctiga_state_dict[
                f"{to_prefix.format(layer_idx, 'norm1')}.weight"] = ctiga_state_dict.pop(
                    f"{from_prefix.format(layer_idx, 'input_layernorm')}.weight")
            # post_attention_layernorm -> norm2
            ctiga_state_dict[
                f"{to_prefix.format(layer_idx, 'norm2')}.weight"] = ctiga_state_dict.pop(
            ...
            q, k, v, o = [
                ctiga_state_dict.pop(f"{from_}.{k}_proj.weight") for k in ["q", "k", "v", "o"]
            ]
            q = permute(q)
            k = permute(k)
            ...
            ctiga_state_dict[f"{to_}.fc1.weight"] = torch.cat(
                [ctiga_state_dict.pop(f"{from_}.{k}_proj.weight") for k in ["up", "gate"]],
                dim=0,
            )
