...
parser.add_argument(
    ...
    "--model",
    type=str,
    help="The model config name under src/wavtts/configs, used only when --model_cfg is not provided.",
)
parser.add_argument(
    "-mc",
    "--model_cfg",
...
if "voices" in config:
    for voice in config["voices"]:
        ...
        if "infer/examples/" in voice_ref_audio:
            config["voices"][voice]["ref_audio"] = str(files("wavtts").joinpath(f"{voice_ref_audio}"))
...
model_cfg = OmegaConf.load(
    args.model_cfg or config.get("model_cfg", str(files("wavtts").joinpath(f"configs/{model}.yaml")))
)
