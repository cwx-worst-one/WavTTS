...
Example data processing scripts, and you may tailor your own one along with a Dataset class in `src/wavtts/model/dataset.py`.
...
# .yaml files are under src/wavtts/configs directory
accelerate launch src/wavtts/train/train.py --config-name F5TTS_v1_Base.yaml

# possible to overwrite accelerate and hydra config
accelerate launch --mixed_precision=fp16 src/wavtts/train/train.py --config-name F5TTS_v1_Base.yaml ++datasets.batch_size_per_gpu=19200
```

### 2. Finetuning practice
Discussion board for Finetuning [#57](https://github.com/SWivid/F5-TTS/discussions/57).

Gradio UI training/finetuning with `src/wavtts/train/finetune_gradio.py` see [#143](https://github.com/SWivid/F5-TTS/discussions/143).
