...
if [ ${DEBUG_MODE} = True ]; then
    python -m debugpy --listen ${MASTER_ADDR}:${DEBUG_PORT} --wait-for-client src/wavtts/train/train.py \
...
if [ ${DEBUG_MODE} = False ]; then
    accelerate launch --main_process_port ${MASTER_PORT} --num_processes ${num_processes} --num_machines ${num_machines} --mixed_precision ${mixed_precision} --dynamo_backend no src/wavtts/train/train.py \
        --config-name "${CONFIG_NAME}.yaml" \
        ++hydra.run.dir=${OUTDIR} \
        ++ckpts.save_dir="${OUTDIR}/ckpts" \
        ++ckpts.exp_name=${EXP_NAME} \
...
# bash src/wavtts/train/debug_train.sh
