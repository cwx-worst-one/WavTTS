...
from f5_tts.model.utils import is_package_available, lens_to_mask
...
class HubertFeatureLoss(nn.Module):
    def __init__(
        ...
        print(f"Loading SSL model and feature extractor from: {model_path}")
        self.feature_extractor = AutoFeatureExtractor.from_pretrained(model_path)
        self.ssl_model = AutoModel.from_pretrained(model_path)
    ...
    def forward(self, x_pred: torch.Tensor, x_true: torch.Tensor, time_weight: torch.Tensor | None = None):
        ...
        x_pred = self.feature_extractor(x_pred.float(), sampling_rate=self.target_sample_rate, return_tensors="pt").input_values.squeeze(0).to(x_pred.device)
        x_true = self.feature_extractor(x_true.float(), sampling_rate=self.target_sample_rate, return_tensors="pt").input_values.squeeze(0).to(x_true.device)

        with torch.no_grad():
            outputs_true = self.ssl_model(input_values=x_true, output_hidden_states=True)
        
        outputs_pred = self.ssl_model(input_values=x_pred, output_hidden_states=True)

        if self.layer_index == -1:
            feats_true = outputs_true.last_hidden_state
            feats_pred = outputs_pred.last_hidden_state
        else:
            if 0 <= self.layer_index < len(outputs_true.hidden_states):
                feats_true = outputs_true.hidden_states[self.layer_index]
                feats_pred = outputs_pred.hidden_states[self.layer_index]
            else:
                print(f"Warning: Layer index {self.layer_index} out of bounds. Using last_hidden_state.")
                feats_true = outputs_true.last_hidden_state
                feats_pred = outputs_pred.last_hidden_state

        min_len = min(feats_pred.shape[1], feats_true.shape[1])
        feats_pred = feats_pred[:, :min_len, :]
