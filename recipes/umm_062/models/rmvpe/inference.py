...
from recipes.umm_062.models.rmvpe.model import E2E0, E2E
from recipes.umm_062.models.rmvpe.spec import MelSpectrogram 
from recipes.umm_062.models.rmvpe.utils import to_local_average_f0, to_viterbi_f0
...
class RMVPE:
    ...
    def batch_infer(self, audios, sample_rate, thred=0.03, use_viterbi=False):
        if sample_rate == 16000:
            ...
            if key_str not in self.resample_kernel:
                self.resample_kernel[key_str] = Resample(sample_rate, 16000, lowpass_filter_width=128)
            self.resample_kernel[key_str] = self.resample_kernel[key_str].to(audios.device)
