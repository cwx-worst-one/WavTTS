...
from recipes.umm_062.models.rmvpe import RMVPE
from recipes.umm_062.vocoder.ctiga_llama import CtigaLLaMa
from recipes.umm_062.vocoder.llama_v100 import ModelArgs
...
def f0_normalize(f0, norm_pitch=True):
    _f0 = f0.clone()
    f0 = torch.log1p(f0)
    if not norm_pitch:
        return f0
...
class EMAEmbedding(nn.Module):
    ...
    @torch.no_grad()
    def remap_weight(self, thres=None):
        ...
        if thres is None:
            one_of_three_index = min(len(indices) - len(indices) // 3, len(indices) - 1)
            thres = max(0.01, topk_res.values[one_of_three_index])
        # remap
        new_cs = torch.zeros_like(cs.data)
        new_avg_w = torch.zeros_like(avg_w.data)
        new_w = torch.zeros_like(w.data)
        cnt = 0
        print("Thres: ", thres)
        for j, ind in enumerate(indices):
            ...
            if cs_tmp <= thres:
                print(cs_tmp, j)
                new_w[j + 1] = w[indices[cnt]].data
                new_avg_w[j + 1] = (w[indices[cnt]] * cs[indices[cnt]] / 2).data
                new_cs[j + 1] = (cs[indices[cnt]] / 2).data
                cnt += 1
                cnt = cnt % len(indices)
            else:
                new_w[j + 1] = w[ind].data
                new_avg_w[j + 1] = avg_w[ind].data
                new_cs[j + 1] = cs[ind].data
        new_cs[0] = cs[0].data
        print("Thres: ", thres)
...
class Stage1(Base):
    def __init__(self, config):
        ...
        gpt_config = ModelArgs(
            ...
            n_layers=config.num_pre_layers + config.num_post_layers,
            n_heads=config.num_attention_heads,
            out_dim=config.hidden_size,
            attn_pdrop=0.1
        )
        self.gpt = CtigaLLaMa(gpt_config)

    def forward(self, input_dict):
        masked_feature = input_dict["masked_mel"]
        masked_indices = input_dict["masked_indices"]

        hidden_states = self.audio_encoder(masked_feature)
...
class Stage2(Base):
    def __init__(self, config):
        ...
        self.gpt = CtigaLLaMa(gpt_config)
        self.mel_head = Conv2dUpsampling(config.hidden_size, config.n_mels, use_bn=True)
        ...
        if config.add_chroma:
            ...
            self.chroma_head = Conv2dUpsampling(config.hidden_size, config.n_chroma, use_bn=True)
    ...
    def forward(self, input_dict):
        ...
        hidden_states = self.gpt(audio_feature)
    ...
    def preprocessing(self, x):
        ...
        if self.config.get("add_pitch", False):
            ...
            f0 = f0[:, :-1]
            vuv = get_vuv(f0)
            f0 = f0_normalize(f0, norm_pitch=self.config.get("norm_pitch", True))
...
class USMStage3(USMStage2):
    def __init__(self, config):
        ...
        if config.get("fix_layers", False):
