...
def init_stage3(hpath, local_rank, cache_dir=None):
    ...
    with local_zero_first():
        if hpath.startswith("hdfs://"):
            ...
            local_path = hpath
        model = Stage3.load_from_checkpoint(local_path, map_location=device).eval()
        return {"Stage3": model}


def init_mkii(hpath, local_rank, cache_dir=None):
    from recipes.umm_062.modules.lit_module import MKIIVQ
...
def init_unified_decoder(hpath, local_rank, cache_dir=None):
    from recipes.umm_062.modules.lit_module import UnifiedDecoder
