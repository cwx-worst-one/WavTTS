...
def mhubert_infer(semantic_model, centers, wavs, wav_lens, device):
    # (wav_len - 80) // 320
    feat_length = hubert_cal_conv_out_dim(wav_lens)
    feat_mask = make_pad_mask(feat_length)

    outputs, mask = semantic_model(wavs.to(device), wav_lens.to(device), feat_mask.to(device))
    codes = compute_codes(outputs, centers, device)
    return codes, feat_length


def compute_codes(dataset, centers, device):
    b, t, d = dataset.size()
    dataset = dataset.contiguous().view([b*t, d])
