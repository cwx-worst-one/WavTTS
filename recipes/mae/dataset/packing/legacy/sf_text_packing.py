...
def pack_text_process(hdfs_output_dir):
    with open("short_from_and_playlist.pkl", "rb") as f:
    ...
    while True:
        ...
        with tarfile.open("meta/" + os.path.basename(tar_file), "w") as tar:
            for file in file_list:
                ...
                if music_id not in df_dict:
                    meta = {"data_source": "unknown", "music_id": music_id}
                else:
                    meta = df_dict[music_id]
                stream = io.BytesIO()
        ...
        if hdfs_output_dir is not None:
            cmd = f"hdfs dfs -put meta/{os.path.basename(tar_file)} "
...
if __name__ == "__main__":
    ...
    cmd = (
        "hdfs dfs -ls /home/byte_speech_sv/"
        f"mulan/short_form_uio/inst_shards/shards_{drive_id}_*.tar"
    )
    print(cmd)
