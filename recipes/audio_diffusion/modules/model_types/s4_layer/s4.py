...
Ref: https://github.com/HazyResearch/state-spaces
"""
...
try:  # Try CUDA extension
    from .cauchy_cuda_kernel.cauchy import cauchy_mult

    has_cauchy_extension = True
except Exception:
    log.warn(
        "CUDA extension for cauchy multiplication not found. Install by going to"
        " extensions/cauchy/ and running `python setup.py install`. This should speed"
...
try:  # Try pykeops
    ...
    if not has_cauchy_extension:
        log.error(
            "Falling back on slow Cauchy kernel. Install at least one of pykeops or the"
...
class TransposedLinear(nn.Module):
    ...
    def __init__(self, d_input, d_output, bias=True):
        ...
        # nn.init.kaiming_uniform_(self.weight, nonlinearity='linear') # should be equiv
...
def krylov(L, A, b, c=None, return_power=False):
    ...
    """
    # TODO There is an edge case if L=1 where output doesn't get broadcasted,
    # which might be an issue if caller is expecting broadcasting semantics...
    # can deal with it if it arises
...
def power(L, A, v=None):
    ...
    res = torch.eye(A.shape[-1]).to(A)  # , dtype=A.dtype, device=A.device)
...
def transition(measure, N, **measure_args):
    ...
    measure: the type of measure
      ...
      lagt, tlagt - previous versions of (tilted) Laguerre with slightly different
      normalization
    """
    # Laguerre (translated)
    if measure == "lagt":
...
def bilinear(dt, A, B=None):
    ...
    A_backwards = eye - dt[:, None, None] / 2 * A
    A_forwards = eye + dt[:, None, None] / 2 * A
...
class SSKernelNPLR(nn.Module):
    ...
    corresponding to a discretized state space, where A is Normal + Low Rank (NPLR)
    ...
    def _omega(self, L, dtype, device, cache=True):
        """Calculate (and cache) FFT nodes and their "unprocessed" them with the
        bilinear transform. This should be called everytime the internal length self.L
        changes"""
        omega = torch.tensor(
    ...
    def __init__(
        ...
        length_correction: multiply C by (I - dA^L) - can be turned off when L is large
        for slight speedup at initialization (only relevant when N large as well)

        Note: tensor shape N here denotes half the true state size, because of conjugate
        symmetry
        """
    ...
    def forward(self, state=None, rate=1.0, L=None):
        ...
        # The idea is that this kernel's length (in continuous units) is self.L, while
        # we are asked to provide a kernel of length L at (relative) sampling rate rate
        # If either are not passed in, assume we're not asked to change the scale of our
        #  kernel
        ...
        if state is not None:
            ...
            # Can do this without expanding (maybe minor speedup using conj symmetry in
            # theory), but it's easier to read this way
        ...
        B_cat = torch.cat([B, P], dim=-3)  # (s+1+r, H, N)
        C_cat = torch.cat([C, Q], dim=-3)  # (c+r, H, N)

        # Incorporate B and C batch dimensions
        v = B_cat.unsqueeze(-3) * C_cat.unsqueeze(-4)  # (s+1+r, c+r, H, N)
    ...
    def _step_state_linear(self, u=None, state=None):
        ...
        Unfortunately, as currently implemented it's about 2x slower because it calls
        several sequential operations. Perhaps a fused CUDA kernel implementation would
        be much faster
        ...
        if (
            ...
            # TODO worth setting up a contract_expression in default_state if we want to
            # use this at inference time for stepping
    ...
    def _step_state(self, u, state):
        """Must be called after self.default_state() is used to construct an initial
        state!"""
    ...
    def setup_step(self, mode="dense"):
        ...
        dA_L = power(self.L, self.dA)
        eye = torch.eye(self.dA.size(-1)).to(dA_L)
        C = _conj(_r2c(self.C))  # (H C N)

        dC = torch.linalg.solve(eye - dA_L.transpose(-1, -2), C.unsqueeze(-1)).squeeze(
            -1
        )
        self.dC = dC
    ...
    def step(self, u, state):
        """Must have called self.setup_step() and created state with
        self.default_state() before calling this"""
...
class HippoSSKernel(nn.Module):
    ...
    def __init__(
        ...
        length_correction=True,  # Multiply by I-A|^L after initialization;
        hurwitz=False,
        tie_state=False,  # Tie parameters of HiPPO ODE across the H features
        precision=1,  # 1 (single) or 2 (double) for the kernel
        resample=False,  # If given inputs of different lengths, adjust the sampling rat
        verbose=False,
    ):
        super().__init__()
        self.N = N
...
class S4(nn.Module):
    def __init__(
        ...
        d_model,
        d_state=64,
        l_max=1,  # Maximum length of sequence.
    ...
    def step(self, u, state):
        """Step one time step as a recurrent model. Intended to be used during val.
