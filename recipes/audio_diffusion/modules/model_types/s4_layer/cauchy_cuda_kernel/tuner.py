...
def set_up_tuning_temp_dir(params: dict, source_files, extension_dir, verbose=True):
    if verbose:
        print("params: ", params)
    # TD [2021-10-22]: tempfile.mkdtemp sometimes create dir name with '_' in it,
    # thus messing up the extension name.
    # temp_dir =
    # Path(tempfile.mkdtemp(prefix="temp_", dir=Path.cwd().parent)).absolute()
...
class KernelTuner:
    ...
    def tune(self):
        ...
        # Compile in parallel (for speed), then install sequentially to
        # ensure correctness
