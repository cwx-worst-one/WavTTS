...
def test_cauchy_mult_symmetric(N, L):
    ...
    out_torch = cauchy_mult_torch(
        v.cdouble(), z.cdouble(), w.cdouble(), symmetric=True
    ).cfloat()
    ...
    print(
        f"Keops out relative error: max {relerr_out_keops.amax().item():.6f}, mean"
        f" {relerr_out_keops.mean().item():6f}"
    )
    print(
        f"out relative error: max {relerr_out.amax().item():.6f}, mean"
        f" {relerr_out.mean().item():.6f}"
    )
    print(
        f"Keops dv relative error: max {relerr_dv_keops.amax().item():.6f}, mean"
        f" {relerr_dv_keops.mean().item():6f}"
    )
    print(
        f"dv relative error: max {relerr_dv.amax().item():.6f}, mean"
        f" {relerr_dv.mean().item():.6f}"
    )
    print(
        f"Keops dw relative error: max {relerr_dw_keops.amax().item():.6f}, mean"
        f" {relerr_dw_keops.mean().item():6f}"
    )
    print(
        f"dw relative error: max {relerr_dw.amax().item():.6f}, mean"
        f" {relerr_dw.mean().item():.6f}"
    )
