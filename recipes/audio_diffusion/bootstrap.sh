...
# Install SoundStream
export PYTHONPATH=/mnt/bn/audio-diffusion/pretrained_models/bytegen/models:$PYTHONPATH
...
if [[ -z "${FAD_LABEL_DIR}" ]]; then
  echo "FAD_LABEL_DIR is undefined, setting to default"
  export FAD_LABEL_DIR="/mnt/bn/audio-diffusion/data/Full/VOL 1/Drum Rolls"
fi

export ARNOLD_OUTPUT="${PROJECT_ROOT}"/logs/$ARNOLD_TASK_ID/trials/$ARNOLD_TRIAL_ID
