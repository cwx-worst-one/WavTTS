...
ckpt_path = (
    "/mnt/bn/audio-diffusion/logs/281298/trials/2332380/diffusion/"
    "DAG_700M_new_mulan/checkpoints/last.ckpt"
)
config = "conf/latent_diffusion_reboot/DAG_700M_resso_mix_mulan149.yaml"
device = "cuda"
...
audio, cond = next(iter(pl_datamodule.val_dataloader()))

num_parallel_examples = 10
...
with torch.no_grad():
    result = pl_module.generate_samples(
        initial_latents, text_cond, guidance_scale=7.0, num_steps=100
    )
