...
class SoundStorm(pl.LightningModule):
    ...
    def __init__(
        ...
        self.audio_embedding = nn.ModuleList(
            [
                nn.Embedding(self.audio_model.codebook_size + 1, self.hparams.n_embd)
        ...
        self.heads = nn.ModuleList(
            [
                nn.Linear(n_embd, self.out_dim, bias=False)
                for _ in range(self.n_quantizers)
            ]
        )
    ...
    def _init_weights(self, module: nn.Module) -> None:
        ...
        if isinstance(module, nn.Linear):
            ...
            torch.nn.init.normal_(
                module.weight,
                mean=0.0,
                std=self.config.initializer_range / math.sqrt(2 * self.hparams.n_layer),
            )
    ...
    def step(self, batch, return_loss: bool = True):
        ...
        semantic_tokens, audio_tokens, audio = self.prepare_inputs(batch)
        masked_audio_tokens, _, rand_qs = self.hparams.masking_scheme(
            audio_tokens, self.mask_token_id
        )
        logits = self.forward(semantic_tokens, masked_audio_tokens, rand_qs)

        if return_loss:
            return self.loss(logits, audio_tokens, masked_audio_tokens, rand_qs)
        return logits
