...
class Featurizer(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        self.weights = nn.Parameter(torch.zeros(50)).to(upstream_device)
        self.output_dim = -1 # unused
        self.downsample_rate = -1 # unused
    ...
    def forward(
        ...
        feats_lens = [feats.shape[-2] for feats in paired_features]
        max_lens = max(feats_lens)
        paired_features = [F.pad(feat, (0,0,0,max_lens-feat_len)) for feat, feat_len in zip(paired_features, feats_lens)]
        feature = torch.stack(paired_features, 1) # [L, B, T, D]
        feature = [feature[i] for i in range(feature.shape[0])]
        feature = self._weighted_sum(feature) # [B, T, D]
        feature = [f[:l] for f,l in zip(feature, feats_lens)]
        return feature
        # feature = self._select_feature(paired_features)
        # if isinstance(feature, (list, tuple)):
