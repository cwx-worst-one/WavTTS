...
class LLaMa(nn.Module):
    def __init__(
        ...
        assert provider in ("default", "ctiga", "flashattn")
    ...
    def forward(
        ...
        h = self.forward_layers(
            h, start_pos=start_pos, seqlen=seqlen, 
    ...
    def create_layers(self, params, provider):
        if provider == "ctiga":
            ...
            return layers
        elif provider == "flashattn":
            from samantha.models.ctiga.gpt_flashattn import GPTModel as GPTModelFlash
            ...
            layers = GPTModelFlash(ctiga_config)
            del layers.embeddings.word_embeddings
            return layers
        else:
    ...
    def forward_layers(self, h, start_pos, seqlen, 
            inference_params=None,
            cond=None, attention_mask=None):
        if self.provider in ["ctiga", "flashattn"]:
            return self.layers(inputs_embeds=h, inference_params=inference_params, cond=cond,
                               attention_mask=attention_mask)
        else:
