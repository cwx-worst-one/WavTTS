...
from recipes.valle.datasets.text_converter import  TextToTacolabID
...
def main(args):
    device = args.device
    if args.ctiga:
        ar_model, nar_model, vqgan =  prepare_models_ctiga_llama(args, device)
    else:
    ...
    tokenizer = PhoneTokenizerWithAudioTokens(args.phone_token_num, args.audio_token_num)
    symbol_sets = ['.', ',', '?', '!', '，', '。', '？', '！']
    ...
    for line in open(args.meta_file):
        ...
        seqs, pos_ids, seq_sen_ids = ar_model.inference_from_text(
            (seqs, seq_lens, pos_ids, seq_sen_ids, utts), tokenizer,
            temperature=args.temperature,
            thres=args.thres)
            # (seqs, seq_lens, pos_ids, seq_sen_ids, utts), tokenizer, thres=args.thres, temp=args.temp, mode=args.mode)
        b, t = seqs.shape
        
        # seqs[:, 0] = seqs[:, 0] - 8000 + 200
        # seqs[:, text_len - 1:] = seqs[:, text_len - 1:] - 8000 + 200
        # init_full_seqs[:, text_len[0] :, :] = init_full_seqs[:, text_len[0] :, :] - 8000 + 200
        ...
        mask2 = (text_len + unmask_len).unsqueeze(1) > torch.arange(t, device=device).unsqueeze(0)  # [b, 1] > [1, t] = [b, t]
        ...
        # tokenizer2 = PhoneTokenizerWithAudioTokens(200, args.audio_token_num)
        ...
        save_wav(wav_gen, os.path.join(out_dir, "gen_with_prompt", "prompt-{}.wav".format(utts[0])),sr=24000)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--phone_token_num", type=int, default=8000)
