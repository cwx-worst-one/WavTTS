...
def test_webdataset_audio_encode_decode(n_test_samples: int = 5):
    ...
    for batch_idx, batch in enumerate(train_loader):
        ...
        with torch.no_grad():
            audio_emb = pl_module.audio_model(audio)
            dec_audio = pl_module.audio_model.decode(audio_emb)

            audio_emb[:, 2:, :] = 0
            dec_coarse_only_audio = pl_module.audio_model.decode(audio_emb)

            torchaudio.save(
                f"{batch_idx}-original---test_webdataset_audio_encode_decode.mp3",
                audio[0].cpu(),
                sample_rate=sample_rate,
            )
            torchaudio.save(
                f"{batch_idx}-decoded---test_webdataset_audio_encode_decode.mp3",
                dec_audio[0].cpu(),
                sample_rate=sample_rate,
            )

            torchaudio.save(
                f"{batch_idx}-decoded-coarse-only---test_webdataset_audio_encode_decode.mp3",  # noqa
