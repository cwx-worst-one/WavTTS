...
from recipes.musiclm.lightning.modules import SemanticEmbedARModule
...
class BaseModule(pl.LightningModule):
    ...
    def get_best_rq_embeds(self, x):
        best_rq_embeds = self.requires["semantic"].get_latent(
            x,
            self.extra_params.semantic_layer_idx
        )
...
class InferenceEmbedModule(BaseModule):
    def __init__(
        self,
        required_modules,
        extra_params=None,
        ...
        self.save_hyperparameters()
        self.extra_params = DotDict(extra_params)
        self.semantic_module = SemanticEmbedARModule.load_from_checkpoint(self.extra_params.semantic_ckpt).eval()
        self.coarse_module = CoarseCrossAttnModule.load_from_checkpoint(self.extra_params.coarse_ckpt).eval()
        self.fine_module = FineModule.load_from_checkpoint(self.extra_params.fine_ckpt).eval()
        self.requires = {}
        self.load_required_modules()
        self.semantic_module.requires = self.requires
...
class InferenceDiffusionModule(BaseModule):
    def __init__(
        self,
        required_modules,
        extra_params=None,
    ...
    def _predict_step(self, batch, round):
        ...
        mulan_embeds = torch.cat(text_embs, dim=0)
        bs = mulan_embeds.size(0)
        semantic_samples = self.semantic_module.predict(mulan_embeds, self.extra_params)
        coarse_samples = self.coarse_module.predict(semantic_samples, self.extra_params)
        fine_samples = self.fine_module.predict(coarse_samples, self.extra_params)
...
class InferenceGTModule(BaseModule):
    def __init__(
        self,
        required_modules,
        extra_params=None,
    ...
    def load_required_modules(self):
        if self.gt_infer_mode in ["mulan->semantic->coarse->fine", "semantic->coarse->fine", "coarse->fine"]:
            ...
            if self.gt_infer_mode in ["mulan->semantic->coarse->fine"]:
                ...
                if semantic_type == "best_rq":
                    self._load_required_module("semantic_cmvn")
    ...
    def _predict_step(self, batch, batch_idx, round):
        ...
        if self.extra_params.gt_infer_mode == "mulan->semantic->coarse->fine":
            ...
            print(f"[NUC] {sum([len(tokens.unique()) for tokens in semantic_tokens]) / bs}")
            coarse_samples = self.coarse_module.predict(semantic_tokens, self.extra_params)
...
class InferenceGTCrossAttnModule(BaseModule):
    def __init__(
        self,
        required_modules,
        extra_params=None,
