...
class BestRQV1(pl.LightningModule):
    def __init__(
        ...
        self.len_masking_raw = int(self.extra_params.sample_rate * self.extra_params.mask_hop)
        len_masking_token = (
    ...
    def _shared_step(self, batch):
        ...
        with torch.cuda.amp.autocast(enabled=True):
            ...
            hidden_state = self.model["backend"](encoded_masked_feature)["last_hidden_state"]
...
class BestRQ(pl.LightningModule):
    def __init__(
        ...
        if self.extra_params.get("input_norm", True):
            self.model["ln"] = nn.LayerNorm(
                self.extra_params.n_mels * pow(self.extra_params.kernel, len(self.extra_params.conv_dim)),
        ...
        if self.extra_params.feature_mean_path is None:
            ...
            feature_mean = torch.load(
                self.extra_params.feature_mean_path
            )
        ...
        if self.extra_params.feature_std_path is None:
            ...
            feature_std = torch.load(
                self.extra_params.feature_std_path
            )
    ...
    def masking(self, x):
        ...
        # mask with random values
        masking_noise = (
            torch.randn(len(time_domain_masked_indices), dtype=x.dtype, device=device) * 0.1
        )  # 0 mean 0.1 std
        mx[tuple(time_domain_masked_indices.t())] = masking_noise
        return mx, token_domain_masked_indices
...
class Inference(pl.LightningModule):
    ...
    def predict_step(self, batch, batch_idx):
        ...
        if self.extra_params.latent_type == "melspec":
            ...
            embeds = rearrange(self.module._subsample(feature), "b t d -> (b t) d").cpu()
        else:
