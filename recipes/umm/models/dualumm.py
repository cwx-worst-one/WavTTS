...
from recipes.umm.models.umm_mkii import ClusteredVectorQuantizer, FiniteScalarQuantizer, LookupFreeQuantizer, \
    EMAVectorQuantizerEntropy, Transpose, WNConv1d, get_vuv
...
class ConvStacksWithDownUpSampling(nn.Module):
    def __init__(self, hidden_size, input_dim, output_dim,
                 downsampling=1, upsampling=1, num_layers=4, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.proj_in = nn.Conv1d(input_dim, hidden_size, 3, padding=1)
        self.conv_stacks1 = ConvBlocks(
            hidden_size, hidden_size, None,
            5, layers_in_block=2, num_layers=num_layers,
            norm_type='ln', dropout=0,
            post_net_kernel=3, is_BTC=False)
        self.conv_stacks2 = ConvBlocks(
            hidden_size, output_dim, None,
            5, layers_in_block=2, num_layers=num_layers,
            norm_type='ln', dropout=0,
            post_net_kernel=3, is_BTC=False)
    ...
    def forward(self, x, nonpadding=None):
        ...
        x = x.transpose(1, 2)
        x = self.proj_in(x) * F.interpolate(nonpadding, size=x.shape[-1], mode='nearest')
        x = self.conv_stacks1(x, F.interpolate(nonpadding, size=x.shape[-1], mode='nearest'))
        if self.downsampling > 1:
            x = F.avg_pool1d(x, self.downsampling)
        if self.upsampling > 1:
            x = F.interpolate(x, scale_factor=self.upsampling, mode='nearest')
        x = self.conv_stacks2(x, F.interpolate(nonpadding, size=x.shape[-1], mode='nearest'))
        x = x.transpose(1, 2)
        return x
...
class MultiRefTimbreEncoder(nn.Module):
    ...
    def forward(self, x_q, mels):
        ...
        K, V = h_mels[..., :self.fg_spk_enc_hidden], h_mels[..., self.fg_spk_enc_hidden:]
        attn_mask = (nonpadding_q[..., 0][:, :, None] > 0) & (nonpadding_mel[..., 0][:, None] > 0)
...
class DualUMMv2(nn.Module):
    ...
    def init_inst_branch(self, config):
        ...
        self.audio_encoder_inst = ConvStacksWithDownUpSampling(
            ...
            downsampling=self.ds, upsampling=self.us)
        self.mel_head_inst = ConvStacksWithDownUpSampling(
            head_hidden_size, config.hidden_size, config.n_mels_tgt,
            downsampling=self.us, upsampling=self.ds)
        self.chroma_head_inst = ConvStacksWithDownUpSampling(
            head_hidden_size, config.hidden_size, config.n_chroma,
            downsampling=self.us, upsampling=self.ds)
    ...
    def init_vocal_branch(self, config):
        ...
        self.f0_vuv_head_vocal = ConvStacksWithDownUpSampling(
            head_hidden_size, config.hidden_size, 2,
            downsampling=self.us, upsampling=self.ds)
        self.asr_aux_decoder_vocal = TransformerDecoder(
            config.vocab_size, config.hidden_size,
            4, config.hidden_size * 4, 4)
    ...
    def init_full_branch(self, config):
        head_hidden_size = self.conv_hidden_size
        self.decoder_in_proj = nn.Conv1d(config.hidden_size * 2, config.hidden_size, 3, 1, 1)
    ...
    def forward_vq(self, hidden_states, suffix):
        ...
        if self.config.get("vq_proj_noise", 0) > 0:
            noise_scale = (self.config.vq_proj_noise - cnt).clamp(0) / \
    ...
    def forward_inst_branch(self, input_dict):
        ...
        hidden_states_aftervq_inst = hidden_states_inst = self.forward_decoder(hidden_states_inst, self.decoder_inst)
    ...
    def forward_vocal_branch(self, input_dict):
        ...
        hidden_states_vocal_f, vq_ids_vocal_f, vq_loss_vocal_f = self.forward_vq(hidden_states_vocal_f, 'vocal')
        ...
        hidden_states_vocal_b, vq_ids_vocal_b, vq_loss_vocal_b = self.forward_vq(hidden_states_vocal_b, 'vocal')
        ...
        hidden_states_vocal_f = self.forward_decoder(hidden_states_vocal_f, self.decoder_vocal, feature_vocal_ref_b,
                                                     self.timbre_enc_vocal)
        hidden_states_vocal_b = self.forward_decoder(hidden_states_vocal_b, self.decoder_vocal, feature_vocal_ref_f,
                                                     self.timbre_enc_vocal)
        hidden_states_vocal = torch.cat([hidden_states_vocal_f, hidden_states_vocal_b], 1)

        # mel out
        mel_out_vocal_f = self.mel_head_vocal(hidden_states_vocal_f, nonpadding_f)[:, :T_f]
        mel_out_vocal_b = self.mel_head_vocal(hidden_states_vocal_b, nonpadding_b)[:, :T_b]
        mel_out_vocal = torch.cat([mel_out_vocal_f, mel_out_vocal_b], 1)

        # f0 out
        f0_vuv_out_vocal_f = self.f0_vuv_head_vocal(hidden_states_vocal_f, nonpadding_f)[:, :T_f]
        f0_vuv_out_vocal_b = self.f0_vuv_head_vocal(hidden_states_vocal_b, nonpadding_b)[:, :T_b]
        ...
        text_out = self.asr_aux_decoder_vocal(hidden_states_vocal, None, text_ids_pad, (text_ids_pad > 0).sum(-1))[0]
        ...
        output_dict = {
            ...
            'hidden_states_aftervq_vocal': (hidden_states_vocal_f, hidden_states_vocal_b),
            'feature_vocal_ref': (feature_vocal_ref_f, feature_vocal_ref_b),
        }
        return output_dict

    def forward_full_branch(self, input_dict, h_aftervq_vocal, h_aftervq_inst, h_vocal_ref):
        ...
        hidden_states_full_f = torch.cat([hidden_states_vocal_f, h_aftervq_inst[:, :T_f_hs]], -1)
        hidden_states_full_b = torch.cat([hidden_states_vocal_b, h_aftervq_inst[:, T_f_hs:]], -1)
        feature_vocal_ref_f, feature_vocal_ref_b = h_vocal_ref
        hidden_states_full_f = self.decoder_in_proj(hidden_states_full_f.transpose(1, 2)).transpose(1, 2)
        hidden_states_full_b = self.decoder_in_proj(hidden_states_full_b.transpose(1, 2)).transpose(1, 2)
        hidden_states_full_f = self.forward_decoder(
            hidden_states_full_f, self.decoder_full, feature_vocal_ref_b, self.timbre_enc_full)
        hidden_states_full_b = self.forward_decoder(
            hidden_states_full_b, self.decoder_full, feature_vocal_ref_f, self.timbre_enc_full)
        ...
        # f0 out
        f0_vuv_out_full_f = self.f0_vuv_head_full(hidden_states_full_f, nonpadding_f)[:, :T_f]
        f0_vuv_out_full_b = self.f0_vuv_head_full(hidden_states_full_b, nonpadding_b)[:, :T_b]
        f0_vuv_out_full = torch.cat([f0_vuv_out_full_f, f0_vuv_out_full_b], 1)

        # for LAS loss
        text_ids = input_dict["text_ids"]
        text_ids_pad = F.pad(text_ids, [1, -1], value=1)
        text_out = self.asr_aux_decoder_full(hidden_states_full, None, text_ids_pad, (text_ids_pad > 0).sum(-1))[0]
    ...
    def token2mel(self, token, wav_ref=None, token_type='vocal'):
        if token_type == 'full':
            ...
            h = self.forward_decoder(h, self.decoder_full, mel_ref, self.timbre_enc_full)
