...
class PretrainOutput(ModelOutput):
    ...
    quant_state: Optional[Tuple[torch.FloatTensor]] = None
...
class Wav2Vec2ConformerEncoder(nn.Module):
    ...
    def forward(
        ...
        for i, layer in enumerate(self.layers):
            ...
            if vq is not None and self.config.vq_layer_idx == i:
                quant_encoder_out, quant_idx, quant_loss = vq(hidden_states)
                hidden_states = quant_encoder_out
                quant_state = (quant_encoder_out, quant_idx, quant_loss)
    ...
    def forward_from_vq(self, token_embeddings):
        ...
        for i, layer in enumerate(self.layers):
            ...
            layer_outputs = layer(
                hidden_states, relative_position_embeddings=position_embeddings
            )
            hidden_states = layer_outputs[0]
        hidden_states = self.layer_norm(hidden_states)
        return hidden_states
...
class Wav2Vec2ConformerPreTrainedModel(PreTrainedModel):
    ...
    def _set_gradient_checkpointing(self, module, value=False):
        if isinstance(
            module, (Wav2Vec2ConformerEncoder, Wav2Vec2ConformerFeatureEncoder)
        ):
