...
def wav2allbn(model, wav, use_layer=None):
    if wav.dim() == 3:
        wav = wav.squeeze(dim=1)
    hidden_states_list = model.extract_features(wav)
    if use_layer is not None:
        hidden_states_list = [hidden_states_list[i] for i in use_layer]
...
def evaluate(dataloader, umm_model):
    ...
    for i, (audio, target_path) in enumerate(tqdm(dataloader)):
        audio = audio.to("cuda")
...
if __name__ == "__main__":
    ...
    ckpt_path = "/mnt/bd/litang-lq-fairseq/samantha_mk4/checkpoints/step=090000.ckpt"  #modify
    ...
    dataset_list = ['LibriSpeech']

    for dataset_name in dataset_list:
        root_dir = os.path.join('/mnt/bn/cyz-lq-nas/s3prl_datasets', dataset_name)
        target_dir = os.path.join('/mnt/bn/cyz-lq-nas/s3prl_gendir/umm/fuse_umm_p48_90k', dataset_name)
