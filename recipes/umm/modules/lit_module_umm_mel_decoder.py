...
from recipes.umm.models.umm_mel_decoder_helpers import (FrozenUMMTokenizer, compute_l1_loss, compute_ssim_loss, compute_LSGAN_loss, GriffinLimHandler)
...
"""
    ...
    only encodes up to 12khz causing all my results using this class to be incorrect!
"""
...
class UMMMelDecoderTrainingTaskStereoGriffinLim(pl.LightningModule):
    """
    Lightning module for training a a mel decoder based on a pretrained Stage3 UMM model.
    Supports Stereo Signals and inversion back to waveform using Griffin-Lim.
    Unlike Mono setup, there is extra logic to handle the tokenizer trained in mono as well as how to manage 2 channel downsampling and upsampling
    """
    def __init__(
        self,
