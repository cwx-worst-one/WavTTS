...
class SemanticModule(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        with self.profiler.profile(
            ...
            with torch.autocast(device_type="cuda", enabled=False):
                if self.use_offline_token:
                    input_tokens, token_lens, split_point, text_token_len, audio_token_len = batch
                    input_tokens = input_tokens.long()
                    token_lens = token_lens.long()
                    split_point = split_point.long()
                    text_token_len = text_token_len.long()
                    audio_token_len = audio_token_len.long()
        ...
        offset_mask = mask[:, loss_offset + 1:org_len]
        ...
        accu = (
            100 * ((logits[:, loss_offset:org_len - 1, :].argmax(dim=-1)
                   == input_tokens[:, loss_offset + 1:org_len]) * offset_mask).sum() /
            offset_mask.sum())


        temp_prompt_unit_mask = self.get_mask_from_lengths(split_point, org_len)[:, loss_offset+1:org_len]

        prompt_unit_accu = (
            100 * ((logits[:, loss_offset:org_len - 1, :].argmax(dim=-1)
                   == input_tokens[:, loss_offset + 1:org_len]) * offset_mask \
                   * temp_prompt_unit_mask).sum() /
            (offset_mask*temp_prompt_unit_mask).sum())

        temp_text_mask = self.get_mask_from_lengths(text_token_len+split_point+1, org_len)[:, loss_offset+1:org_len]
        temp_text_mask = temp_text_mask * (~temp_prompt_unit_mask) # 包含了sep_u2t，无所谓了

        text_accu = (
            100 * ((logits[:, loss_offset:org_len - 1, :].argmax(dim=-1)
                   == input_tokens[:, loss_offset + 1:org_len]) * offset_mask \
                   * temp_text_mask).sum() /
            (offset_mask*temp_text_mask).sum())

        temp_gen_unit_mask = ~self.get_mask_from_lengths(text_token_len+split_point+2, org_len)[:, loss_offset+1:org_len]

        gen_unit_accu = (
            100 * ((logits[:, loss_offset:org_len - 1, :].argmax(dim=-1)
                   == input_tokens[:, loss_offset + 1:org_len]) * offset_mask \
                   * temp_gen_unit_mask).sum() /
            (offset_mask*temp_gen_unit_mask).sum())

        # print(temp_prompt_unit_mask[1])
        # print((offset_mask*temp_prompt_unit_mask)[1])
        # print((offset_mask*temp_text_mask)[1])
        # print((offset_mask*temp_gen_unit_mask)[1])
        # print('**'*100)

        self.log_dict({
            "tr_loss": loss.item(),
            "accu": accu.item(),
            "prompt_unit_accu": prompt_unit_accu.item(),
            "text_accu": text_accu.item(),
            "gen_unit_accu": gen_unit_accu.item()
        },
                      prog_bar=True,
                      sync_dist=True)
        return loss
