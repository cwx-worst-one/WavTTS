...
from ...requires.huggingface_hubert.huggingface_hubert_infer import hubert_large_24l_km1024_tokenization
...
class SemanticModule(pl.LightningModule):
    ...
    def __init__(
        ...
        for name, param in self.model.named_parameters():
            if name == 'transformer.wte.weight' or name == 'transformer.wpe.weight':
                # 对权重进行修改
                print('Modify: ', name)
                torch.nn.init.kaiming_uniform_(param.data, a=2.236)
    ...
    def training_step(self, batch, batch_idx):
        ...
        token_lens = []
        merge_tokens = []
        for b in range(input_tokens.shape[0]):
            cur_input_tokens = input_tokens[b]
            # if b == 0:
            #     print('before: ', cur_input_tokens)
            #     print(cur_input_tokens.shape)
            ...
            # if b == 0:
            #     print('after: ', cur_merge_tokens)
            #     print(cur_merge_tokens.shape)
            if cur_len < org_len:
                cur_merge_tokens = F.pad(cur_merge_tokens, [0, org_len-cur_len])
            else:
                cur_merge_tokens = cur_merge_tokens[:org_len]
                cur_len = org_len
            merge_tokens.append(cur_merge_tokens)
            token_lens.append(cur_len)
            
        
        input_tokens = torch.stack(merge_tokens)
        token_lens = torch.Tensor(token_lens).long().to(input_tokens.device)
    ...
    def get_hubert_token(self, wavs, wav_lens):
        return hubert_large_24l_km1024_tokenization(
