...
def bshall_hubert_tokenization(bshall_hubert_model, wavs, centers, device):
    b, t = wavs.size()
...
def speech_continuation(semantic_model, prompt_tokens, token_size, device):
    slice_range = []
    beg = 0
    while True:
        end = beg + 10 * 50
        if end >= args.duration * 50:
            end = args.duration * 50
            beg = end - (10 * 50)
    ...
    sep_u2t = torch.Tensor([[token_size - 3]]).to(device).long()
    sep_t2u = torch.Tensor([[token_size - 2]]).to(device).long()
    eos = torch.Tensor([[token_size - 1]]).to(device).long()
    ...
    for cur_beg, cur_end in slice_range:
        ...
        input_tokens = torch.cat([input_tokens, sep_u2t], dim=1)
...
def main():
    ...
    prompt_point_num = 5*50

    # batch_wavs = []
    for _token_file in os.listdir(args.input_dir):
        token_file =os.path.join(args.input_dir, _token_file)
        if token_file.endswith(".npy"):
            prompt_tokens = torch.from_numpy(np.load(token_file)).long().to(args.device)
        else:
            continue
        ...
        semantic_samples = speech_continuation(semantic_model, prompt_tokens, args.token_size, args.device)

        save_path = os.path.join(args.output_dir, _token_file)

        semantic_samples = semantic_samples.squeeze(0).cpu().numpy().astype(np.int32)

        beg_point = 0
        end_point = 0
        sep_t2u = args.token_size - 2
        eos = args.token_size - 1

        for i in range(len(semantic_samples)):
            if semantic_samples[i] == sep_t2u:
                beg_point = i+1 
            if semantic_samples[i] == eos:
                end_point = i 
                break
        
        semantic_samples = semantic_samples[beg_point: end_point]

        if os.path.exists(save_path):
            os.remove(save_path)
        np.save(save_path, semantic_samples, allow_pickle=False)
        print(semantic_samples)
...
if __name__ == "__main__":
    ...
    # Checkpoint paths
    ckpts = {}
    from recipes.speech_qa.lit_modules.audiogpt.lit_audiogpt import SemanticModule
