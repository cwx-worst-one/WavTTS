...
if [ ! -e last.ckpt ]
then
    bash recipes/speech_qa/bootstrap.sh fit --config recipes/speech_qa/conf/coarse_v2_3ar_sparse.yaml --run_opts.hdfs_path=$1
else
    bash recipes/speech_qa/bootstrap.sh fit --config recipes/speech_qa/conf/coarse_v2_3ar_sparse.yaml --run_opts.hdfs_path=$1 --ckpt_path=./last.ckpt
fi
