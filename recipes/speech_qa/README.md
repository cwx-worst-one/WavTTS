...
**note**: work-directory should be `samantha`
```shell
# training with original huggingface
bash recipes/speech_qa/bootstrap.sh fit --config recipes/speech_qa/conf/coarse.yaml --trainer.benchmark=True

# with fp16 precision
bash recipes/speech_qa/bootstrap.sh fit --config recipes/speech_qa/conf/coarse.yaml --run_opts.precision=16

# with activation checkpointing
bash recipes/speech_qa/bootstrap.sh fit --config recipes/speech_qa/conf/coarse.yaml --run_opts.checkpointing=True

# with flash attention
bash recipes/speech_qa/bootstrap.sh fit --config recipes/speech_qa/conf/coarse.yaml --run_opts.flash_attn=flash_attn

# with flash attention cuda (cuda ext must @A100 and with fp16 precision)
bash recipes/speech_qa/bootstrap.sh fit --config recipes/speech_qa/conf/coarse.yaml --run_opts.flash_attn=flash_attn_cuda --run_opts.precision=16
```
...
```yaml
    required_modules:
        mulan:
            ...
            - !name:recipes.speech_qa.requires.model_initializer.init_mulan
              cache_dir: !ref <run_opts[cache_dir]>
        w2v:
            - hdfs://haruna/home/byte_speech_sv/user/litang/mulan/2023-02-24_audio_emb_asset4/
            - !name:recipes.speech_qa.requires.model_initializer.init_w2v
              cache_dir: !ref <run_opts[cache_dir]>
        sound_stream:
            - hdfs://haruna/home/byte_speech_sv/user/wangxin.colin/pretrained/sound_stream_1000k/
            - !name:recipes.speech_qa.requires.model_initializer.init_sound_stream
              cache_dir: !ref <run_opts[cache_dir]>
```

### Datasets
```text
recipes/speech_qa/datasets
├── __init__.py
├── dataset.py
├── wds.py
└── wds_packer.py
