...
class Attention(nn.Module):
    def __init__(self, dim, heads = 8, dim_head = 64, dropout = 0., use_flash_attn=False):
        super().__init__()
        if use_flash_attn:
        ...
        if self.use_flash_attn:
            self.flash_attention = FlashAttention(attention_dropout=dropout)
    ...
    def forward(self, x, rotary_emb=None, masked_n_unmasked=None):
        ...
        if rotary_emb is not None:
            if masked_n_unmasked is not None: 
                ...
                # temp solution
                original_len = masked_n_unmasked[0].shape[1] + masked_n_unmasked[1].shape[1]
                b, h, _, d = q.shape
                expand_q = torch.zeros((b, h, original_len, d), device=q.device, dtype=q.dtype)
                expand_k = torch.zeros((b, h, original_len, d), device=k.device, dtype=k.dtype)
       
                batch_range = torch.arange(b, device = q.device)[:, None]
                expand_q.permute(0, 2, 1, 3)[batch_range, masked_n_unmasked[1]] = q.permute(0, 2, 1, 3)
                expand_k.permute(0, 2, 1, 3)[batch_range, masked_n_unmasked[1]] = k.permute(0, 2, 1, 3)

                q = rotary_emb.rotate_queries_or_keys(expand_q).permute(0, 2, 1, 3)[batch_range, masked_n_unmasked[1]].permute(0, 2, 1, 3)
                k = rotary_emb.rotate_queries_or_keys(expand_k).permute(0, 2, 1, 3)[batch_range, masked_n_unmasked[1]].permute(0, 2, 1, 3)
        ...
        if self.use_flash_attn:
            qkv = rearrange(torch.stack([q,k,v], dim=2), 'b h c n d -> b n c h d')
            out, _ = self.flash_attention(qkv)
...
class Transformer(nn.Module):
    def __init__(self, dim, depth, heads, dim_head, mlp_dim, dropout = 0., checkpointing=True, use_flash_attn=False):
        super().__init__()
        self.layers = nn.ModuleList([])
        for _ in range(depth):
            self.layers.append(nn.ModuleDict({
                'norm_0': RMSNorm(dim),
                'attn': Attention(dim, heads = heads, dim_head = dim_head, dropout = dropout, use_flash_attn=use_flash_attn),
                'norm_1': RMSNorm(dim),
    ...
    def _forward_checkpointing(self, x, rotary_emb=None, masked_n_unmasked=None):
        for layer in self.layers:
            ...
            if rotary_emb is not None:
                if masked_n_unmasked is not None:
                    x = checkpoint(layer['attn'], norm_x, rotary_emb, masked_n_unmasked, use_reentrant=False) + x
                else:
                    x = checkpoint(layer['attn'], norm_x, rotary_emb, use_reentrant=False) + x
            else:
    ...
    def _forward(self, x, rotary_emb=None, masked_n_unmasked=None):
        for layer in self.layers:
            if rotary_emb is not None: 
                if masked_n_unmasked is not None:
                    x = layer['attn'](layer['norm_0'](x), rotary_emb, masked_n_unmasked) + x
                else:
...
class LogMel(nn.Module):
    ...
    def forward(self, audio):
        with torch.autocast(device_type="cuda", enabled=False):
            ...
            mel_spec = self.feat_extract["mel"](audio.float())[..., :-1] # remove last frame
...
class MuT(nn.Module):
    def __init__(self, 
        *, 
        ...
        self.logmel_frontend = {
            "logmel": LogMel(sample_rate=sample_rate)
        }
        ...
        self.to_patch_embedding = nn.ModuleDict(
            {
                'to_patch': Rearrange('b c (h p1) (w p2) -> b (h w) (p1 p2 c)', p1 = patch_height, p2 = patch_width),
        ...
        self.transformer = Transformer(dim, depth, heads, dim_head, mlp_dim, dropout, checkpointing, use_flash_attn=use_flash_attn)
