...
from recipes.speech_qa.requires.mulan.mut import MuT, RMSNorm
...
class MuTWrapper(nn.Module):
    def __init__(self, emb_dim: int = 128):
        ...
        mut.mlp_head = nn.Sequential(
            RMSNorm(1280),
            nn.Linear(1280, emb_dim)
        )
        self.mut = mut
...
def get_music_encoder(music_encoder="ast", emb_dim=128):
    ...
    if music_encoder == "ast":
        return MusicEncoder("MIT/ast-finetuned-audioset-10-10-0.4593", emb_dim, sample_rate)
...
class LitMuLanModule(pl.LightningModule):
    def __init__(
        self, music_encoder, text_encoder, emb_dim, spec_aug, lr, weight_decay, temperature
    ):
        super().__init__()
...
def mulan_inference(
    ...
    assert (text is not None) ^ (music is not None), "text inputs and music input can only select one"
