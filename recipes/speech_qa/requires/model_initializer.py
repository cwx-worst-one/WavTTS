...
from .mulan.mulan_infer import create_mulan_model
...
def init_hubert(hpath, local_rank, cache_dir=None):
    if cache_dir is not None:
        os.makedirs(cache_dir, exist_ok=True)

    device = torch.device(f"cuda:{local_rank}")
    h_semantic, h_centroids = (
        f"{hpath}/mhubert_v1_1024_semantic.jit.pt",
        f"{hpath}/centroids_epoch_55.npy",
    )

    l_semantic, l_centroids = (
        f"{cache_dir}/mhubert_v1_1024_semantic.jit.pt",
        f"{cache_dir}/centroids_epoch_55.npy",
    )
    if hpath.startswith("hdfs://"):
        with local_zero_first():
            if not os.path.exists(l_centroids):
...
def init_hubert_large_24l_km1024_tokenization(hpath, local_rank, cache_dir=None):
    if cache_dir is not None:
        os.makedirs(cache_dir, exist_ok=True)

    device = torch.device(f"cuda:{local_rank}")
    h_centroids = f"{hpath}/centroids_epoch_428.npy"

    l_centroids = f"{cache_dir}/centroids_epoch_428.npy"

    # semantic = torch.hub.load("bshall/hubert:main", "hubert_discrete").to(device).eval()
    # processor = Wav2Vec2FeatureExtractor.from_pretrained('facebook/hubert-large-ll60k')
    semantic = HubertModel.from_pretrained('facebook/hubert-large-ll60k').to(device).eval()
...
def init_bshall_hubert(hpath, local_rank, cache_dir=None):
    if cache_dir is not None:
        os.makedirs(cache_dir, exist_ok=True)

    device = torch.device(f"cuda:{local_rank}")
    h_centroids = f"{hpath}/bshall_kmeans100.npy"

    l_centroids = f"{cache_dir}/bshall_kmeans100.npy"
