...
class LanguageModel(GPT2PreTrainedModel):
    ...
    def fa_forward(
        self, input_ids=None, inputs_embeds=None, return_dict=None, labels=None, inference_params=None,
    ) -> Union[Tuple, CausalLMOutputWithCrossAttentions]:

        hidden_states = self.transformer(
            input_ids=input_ids, inputs_embeds=inputs_embeds, inference_params=inference_params,
        )
        # Set device for model parallelism
        if self.model_parallel:
            torch.cuda.set_device(self.transformer.first_device)
