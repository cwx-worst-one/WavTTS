...
def create_mask(l_len, location, stride=0, loc_offset=0, unmask_len=0):
    ...
    xy = x - y
    # location mask
    location_mask = torch.logical_and(
        xy.abs() <= location + loc_offset, xy.abs() >= loc_offset
    )
...
def semantic_30s_layout_fn(train_len=768, num_heads=16, block=32):
    tq = torch.arange(train_len // block).unsqueeze(1)
    tk = torch.arange(train_len // block).unsqueeze(0)
    mask = (tq - tk) >= 0
    mask = mask.unsqueeze(0).repeat(num_heads, 1, 1)
...
layout_fn_dict = {
    ...
    "semantic_30s_layout": semantic_30s_layout_fn,
...
class FixedAbsolutePositionEmbedding(nn.Module):
    def __init__(self, max_pos, hidden_size, pos_type="rope"):
        super().__init__()
    ...
    def forward_fixed(self, x, position_ids):
        with autocast(enabled=False):
            return x.float() + nn.functional.embedding(position_ids, self.wpe)

    def forward_rope(self, x, position_ids):
        with autocast(enabled=False):
            embeddings = nn.functional.embedding(position_ids, self.wpe) # [b, t, d]
...
class GPT2Attention(nn.Module):
    def __init__(self, config, is_cross_attention=False, layer_idx=None):
        ...
        c_attn_bias = config_get(config, "c_attn_bias", True)
        if self.is_cross_attention:
            self.c_attn = Conv1D(2 * self.embed_dim, self.embed_dim, bias=c_attn_bias)
            self.q_attn = Conv1D(self.embed_dim, self.embed_dim, bias=c_attn_bias)
        else:
            self.c_attn = Conv1D(3 * self.embed_dim, self.embed_dim, bias=c_attn_bias)
        ...
        if self.pos_type == "rope_attn":
            self.wpe = FixedAbsolutePositionEmbedding(max_positions, self.head_dim, self.pos_type)
            self.register_buffer("pos_ids", torch.arange(max_positions).unsqueeze(0))

        layout_fn = layout_fn_dict[config.layout_fn]
        layout, block = layout_fn(train_len=config.train_len, num_heads=config.n_head)
    ...
    def _get_sparse_fn(self, device):
        ...
        self.softmax_fn = sparse_softmax(
            self.layout, self.block, device, is_dense=False
        )
    ...
    def _sparse_attn(self, query, key, value, attention_mask=None, head_mask=None):
        assert (attention_mask is None) and (
            head_mask is None
        ...
        # softmax_fn must uses float32
        with torch.autocast(device_type="cuda", enabled=False):
            attn_weights = self.softmax_fn(attn_weights, scale=scale, is_causal=True)
    ...
    def forward(
        ...
        if self.pos_type == "rope_attn":
            ...
            query = self.wpe(query, self.pos_ids[:, past_len: past_len + query_length])
            key = self.wpe(key, self.pos_ids[:, past_len: past_len + key_length])
...
class GPT2Block(nn.Module):
    def __init__(self, config, layer_idx=None):
        ...
        if norm_fn == "layer_norm":
            ...
            raise NotImplementedError("Norm function must be \"layer_norm\" or \"rms_norm\", but get {}".format(norm_fn))
...
class GPT2Model(GPT2PreTrainedModel):
    ...
    def __init__(self, config):
        ...
        if self.pos_type == "learnable":
            ...
            self.wpe = FixedAbsolutePositionEmbedding(config.max_position_embeddings, self.embed_dim, self.pos_type)
        elif self.pos_type == "rope_attn":
            self.wpe = None
        else:
            raise NotImplementedError("pos_type must be learnable, fixed or rope")
