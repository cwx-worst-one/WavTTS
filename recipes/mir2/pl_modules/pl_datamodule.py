    ...
    def prepare_data(self):
        ...
        if self.audio_dir:
            for f in os.listdir(self.audio_dir):
                f_path = Path(f)
                # if sf.check_format(f_path.suffix.split(".")[-1].upper()):
                if f_path.suffix.split(".")[-1].lower() not in ["wav", "mp3", "flac"]:
                    continue
...
class BaseDataModule(AudioInferenceDataModule):
    def __init__(self, train_dataloader, val_dataloader, test_dataloader, sampling_rate=24000, mono=True, **kwargs):
