...
from recipes.datasets.librilight.librilight import SAMPLE_RATE, LibriLightWebDataModule
...
def test_librilight_datamodule():
    ...
    for batch_idx, batch in enumerate(tqdm(train_loader)):
        ...
        for a_idx, a in enumerate(audio):
            torchaudio.save(
                f"librilight-batch-{batch_idx}-item-{a_idx}-test.mp3",
                a,
...
def test_mel_spectrogram(bucket_size):
    ...
    for a_idx, a in enumerate(audio):
        torchaudio.save(
            f"librilight-bucketsize-{bucket_size}-item-{a_idx}.mp3",
            a,
            pl_datamodule.sample_rate,
        )
        ...
        for win_length in [400]:
            for hop_length in [160]:
                model_input_transform = SpeechTransform(
                    sample_rate=pl_datamodule.sample_rate,
                    n_mels=n_mels,
                    n_fft=n_fft,
                    win_length=win_length,
                    hop_length=hop_length,
                    f_min=0,
                    f_max=pl_datamodule.sample_rate // 2,
                )
                ...
                plt.tight_layout()
                plt.savefig(
                    f"librilight-bucketsize-{bucket_size}-mel-{n_mels}-{n_fft}-{win_length}-{hop_length}.pdf"
                )
...
def test_librilight_datamodule_mel_transform():
    ...
    for batch_idx, batch in enumerate(tqdm(train_loader)):
        ...
        plt.tight_layout()
        plt.savefig(
            f"librilight-batch-{batch_idx}-mel-{n_mels}-{n_fft}-{win_length}-{hop_length}.pdf"
        )
        assert mel.shape[0] == batch_size
        assert mel.shape[1] == 1
        assert mel.shape[2] == n_mels
