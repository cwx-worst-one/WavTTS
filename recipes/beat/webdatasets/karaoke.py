...
class KaraokeDataset(AbstractWebDatasetGeneratorBasedBuilder):
    ...
    def _split_data(self, data_dir):
        keys = []
        for k in os.listdir(os.path.join(data_dir, "data", "karaoke_truth_beats")):
            k = k.split(".txt")[0]
    ...
    def _load_example(self, key):
        ...
        return {
            "dataset.txt": KaraokeDataset.DATASET_NAME,
            "__key__": key,
            "beats.pickle": beats,
            "audio.npy": np_audio.astype(KaraokeDataset.AUDIO_NUMPY_DTYPE),
        }
