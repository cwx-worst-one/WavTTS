...
_HDFS_DIRS = {
    ...
    "SheetDoctor/audio_h5/karavocal_audio.h5",
    "beat_labels": "hdfs://harunava/home/byte_speech_sv/data/"
    "SheetDoctor/labels/karavocal_truth_beats.zip",
}


class KaraokeVocalDataset(AbstractWebDatasetGeneratorBasedBuilder):
    """
    Karaoke Dataset
    """

    SAMPLING_RATE = 16000
    DATASET_NAME = "karaoke_vocal_beat"
    ...
    def _split_data(self, data_dir):
        keys = []
        for k in os.listdir(os.path.join(data_dir, "karavocal_truth_beats")):
            k = k.split(".txt")[0]
    ...
    def _load_example(self, key):
        ...
        with open(
            os.path.join(
                self.label_dir, "karavocal_truth_beats", key.replace("$$", ".") + ".txt"
            )
        ) as f:
            beats = f.readlines()
        ...
        return {
            "dataset.txt": KaraokeVocalDataset.DATASET_NAME,
            "__key__": key,
            "beats.pickle": beats,
            "audio.npy": np_audio.astype(KaraokeVocalDataset.AUDIO_NUMPY_DTYPE),
        }
