...
class MusicFM25HzCQT(nn.Module):
    """ 
    MusicFMCQT
    ...
    def __init__(
        ...
        super(MusicFM25HzCQT, self).__init__()
        ...
        self.quantizer_cqt = RandomProjectionQuantizer(84 * 4, codebook_dim, codebook_size)  # mel spec

        # feature extractor
        self.preprocessor_cqt = CQT1992v2(sr=24000, hop_length=hop_length)

        # two residual convolution layers + one projection layer
        self.conv = Conv2dSubsampling(1, conv_dim, encoder_dim, strides=[2, 2], n_bands=84)
    ...
    def get_predictions(self, x):
        # preprocessing
        x = self.preprocessing(x, features=["cqt"])
        x = self.normalize(x)
        
        # encoding
        logits, hidden_emb = self.encoder(x["cqt"])
