...
class MusicFM100Hz(nn.Module):
    ...
    def __init__(
        ...
        super(MusicFM100Hz, self).__init__()
        ...
        self.quantizer_melspec = RandomProjectionQuantizer(n_mels * 1, codebook_dim, codebook_size)  # mel spec

        # feature extractor
        self.preprocessor_melspec = MelSTFT(n_fft=n_fft)

        # two residual convolution layers + one projection layer
        self.conv = Conv2dSubsampling(1, conv_dim, encoder_dim, strides=[1, 1], n_bands=n_mels)

        # Conformer
        if is_flash:
            from recipes.musicfm.modules.flash_conformer import Wav2Vec2ConformerEncoder, Wav2Vec2ConformerConfig
        else:
    ...
    def rearrange(self, x):
        """ rearrange the batch to flatten every step """
        for key in x.keys():
            if key == "chromagram":
                x[key] = rearrange(x[key], "b f t -> b t f")
            else:
                x[key] = rearrange(x[key], "b f (t s) -> b t (s f)", s=1)
        return x
    ...
    def get_predictions(self, x):
        # preprocessing
        x = self.preprocessing(x, features=["melspec"])
        x = self.normalize(x)
        
        # encoding
        logits, hidden_emb = self.encoder(x["melspec"])
