...
def prob_to_tag(tag_pred, tag_map):
    ...
    for tag_type in tag_map:
        if tag_pred[tag_type].shape[0] == 0:
            continue
        prob = torch.sigmoid(tag_pred[tag_type]).squeeze(0).detach().cpu().numpy()
...
if __name__ == "__main__":
    ...
    yaml_path = "recipes/umm2/conf/inference_stage4_token_tag.yaml"
    ...
    # audio, _ = load_audio("inp071.generated.wav", sample_rate=hparams['audio']['sampling_rate'], downmix_to_mono=True)
    import torchaudio
    audio, sr = torchaudio.load("inp071.generated.wav")
    if sr != 24000:
        audio = torchaudio.functional.resample(audio, sr, 24000)
    audio = audio[0].unsqueeze(0).cuda()   # [B, 1, T]
