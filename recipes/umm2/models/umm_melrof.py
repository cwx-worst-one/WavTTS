...
class Attention(nn.Module):
    ...
    def forward(self, x, rotary_emb=None):
        ...
        if rotary_emb is not None:
            ...
            qkv = rotary_emb(qkv, None, 0, None, None, None, None, None)
...
class BandSplit(nn.Module):
    ...
    def forward(self, x):
        ...
        for subspec_idx, mel_band_transform in zip(self.subspec_idxs, self.mel_band_transforms):
            ...
            # if using unfold, self.channel_num *= time_pool_length 
            outs.append(sub_x)
...
class MLP_proj(nn.Module):
    def __init__(
        ...
        for _ in range(mel_bands):
            self.mlp_projs.append(nn.Sequential(
                collections.OrderedDict([
                    ("norm", RMSNorm(in_dim)),
                    ("ff", nn.Linear(in_dim, num_feature)),
                ])))

    def forward(self, x): # x: (b, t, k, n)
        outs = []
        for i, mlp_proj in enumerate(self.mlp_projs):
            sub_x = mlp_proj(x[:, :, i, :])
            outs.append(sub_x)
        x = torch.stack(outs, dim=2)    # (b, t, k, num_feature)
        return x
