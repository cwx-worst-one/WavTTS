...
class ModelInputTransform(nn.Module):
    ...
    def load_from_checkpoint(self, fp: str, verify: bool = True):
        ...
        if os.path.exists(fp):
            print(f"load_state_dict from {fp}")
...
def generate_data():
    ...
    max_duration = 50  # seconds (will force one sample to be exactly this)
    sample_rate = 24000

    # Generate random durations between 40-50 seconds, but ensure one is exactly 50s
    durations = torch.FloatTensor(batch_size - 1).uniform_(min_duration, max_duration)
    durations = torch.cat([durations, torch.tensor([max_duration])])  # Force last sample to be 50s
    durations = durations[torch.randperm(batch_size)]  # Shuffle to random position

    waveform_lengths = (durations * sample_rate).long()

    # Create random waveforms with appropriate lengths
    max_length = waveform_lengths.max()  # Will be exactly 50 * sample_rate
    waveforms = torch.randn(batch_size, 1, max_length)  # (b, 1, T)

    # Zero out the padded portions for shorter waveforms
    for i in range(batch_size):
        waveforms[i, :, waveform_lengths[i]:] = 0

    # Verify max duration is exactly 50s
    assert torch.any(waveform_lengths == max_duration * sample_rate), "Max duration not 50s!"
    print(f"Waveform lengths (seconds): {waveform_lengths / sample_rate}")
    print(f"Max duration: {waveform_lengths.max() / sample_rate:.2f}s (should be 50.00s)")
