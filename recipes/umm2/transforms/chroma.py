...
from recipes.umm2.transforms.hpss import HPSS, Onset_Strength
...
class HPSS_Chroma_Onset(torch.nn.Module):
    def __init__(
        self,
        sample_rate: int,
        n_fft: int,
        *,
        ...
        self.power_to_db = AmplitudeToDB(stype="power")  # power
        self.hpss = HPSS(
            kernel_size=31, 
            power=2.0, 
            hard=False, 
    ...
    def forward(self, waveform: torch.Tensor) -> torch.Tensor:
        ...
        """
        spectrogram = self.spectrogram(waveform)
        spectrogram = rearrange(spectrogram, "b f t -> b 1 f t")
        harm, perc = self.hpss(spectrogram)

        # harmonic part -> chromagram (12-d temporal)
        harm = rearrange(harm, "b c f t -> (b c) f t")
        harm_chroma = self.chroma_scale(harm)

        # percussive part -> onset_strength (1-d temporal)
        perc = rearrange(perc, "b c f t -> (b c) f t")
        perc = self.power_to_db(perc)
        perc_mel = self.mel_scale(perc)
        perc_onset = self.onset_strength(perc_mel)
        
        return harm_chroma, perc_onset


if __name__ == "__main__":
    dummy_input = torch.randn(4, 24000 * 8)
    transform = HPSS_Chroma_Onset(
        sample_rate=24000,
        n_fft=2048,
        win_length=2048,
        hop_length=240,
    ...
    harm, perc = transform(dummy_input)
    print(harm.shape, perc.shape)
    # torch.Size([4, 12, 801]) torch.Size([4, 801])