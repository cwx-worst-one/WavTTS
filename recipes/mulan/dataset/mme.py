...
URL2IDX_Map = {
    ...
    "nonvocal": {
        "simshutter": "/mnt/bn/audio-diffusion/data/shutterstock2m_sim/url2idx_gpt.txt",
...
class MMEDataset(IterableDataset):
    def __init__(self, name="mcc600k", audio_type="vocal", mode="train", seq_len=500, tok_path=None, **kwargs):
    ...
    def _process_text(self, data):
        if self.name == "mcc3.5m":
            ...
            return data
        elif self.name == "simshutter":
            meta = data["__index_data__"]
            text = meta["gpt_text"]
            if text.startswith("Unknown"):
                text = ""
            data["text"] = self.clean_text(text)
            data["music_id"] = int(meta["meta_song_id"])
            return data
...
if __name__ == "__main__":
    ...
    for audio_type in ["nonvocal"]:
        for name in ["simshutter"]:
            ...
            # if name == "mcc3.5m" and audio_type == "vocal":
                ...
                import webdataset as wds
                loader = wds.WebLoader(d, batch_size=2, collate_fn=collate_fn)
                start = 0
                for item in loader:
                    print(start)
                    start += 1
