...
class SSTKDataset(IterableDataset):
    def __init__(self, name="sstk", mode="train", text_pick = "random", **kwargs):
    ...
    def _process_text(self, data):
        ...
        if self.name == "sstk":
            ...
            if self.text_pick == "random":
                selected_fields = random.sample(text_fields, k=random.randint(1, 2))
                selected_text = " ".join([" ".join(t.split(',')) for t in selected_fields if not isEmpty(t)])
                # selected_text = '. '.join(filter(None, selected_fields))
                data["text"] = selected_text
                #print("random pick text field")
            else:
                # data["text"] = ". ".join([t for t in text_fields if t])
                data["text"] = " ".join([" ".join(t.split(',')) for t in text_fields if not isEmpty(t)])
                #print("random all text field")
