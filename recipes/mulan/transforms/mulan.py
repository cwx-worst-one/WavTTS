...
from recipes.mulan.transforms.audio import (
    NormalizeAudio,
...
from recipes.mulan.dataset.utils import fix_hash
...
class MCCTransforms(TransformBase):
    ...
    def is_metadata_good(self, metadata: Dict[str, Any]) -> bool:
        ...
        # Apply if text is there or not
        if self.for_mulan and not metadata.get("gpt_texts", False) and not metadata.get("external_texts", False):
            return False
        return True
    ...
    def __call__(self, x: Dict[str, Any]) -> Generator:
        ...
        for wid in window_ids:
            ...
            text = ""
            if self.for_mulan:
                text = x["metadata.json"]["gpt_texts"]
            output = {
                "audio": cropped_audio,
...
class YMVransforms(TransformBase):
    def __init__(
        self,
        n_samples: int,
        sample_rate: int,
    ...
    def is_metadata_good(self, metadata: Dict[str, Any]) -> bool:
        # check if necessary texts are there
        # print(f"1: {metadata.get('search_query_youtube', False)}\n")
        # print(f"2: {metadata.get('video_description_youtube', False)}\n\n\n")

        if (not metadata.get('search_query_youtube', False)) or (not metadata.get('video_description_youtube', False)):
            return False
    ...
    def __call__(self, x: Dict[str, Any]) -> Generator:
        ...
        # TODO get several 10s music segments    
        try:
            audio = self.base_transform(io.BytesIO(x[self.audio_key]))
        except Exception as e:
            print(f"[MP3 decoding error] {e}")
        ...
        # tokenize texts
        music_id = fix_hash(x["__key__"])
        search_query = x["__index_data__"]['search_query_youtube']
        video_des = x["__index_data__"]['video_description_youtube']
        text = search_query + " " + video_des
        ...
        num_windows = 1 + (audio.size(1) - self.n_samples) // self.crop_step_size
        window_ids = list(range(num_windows))
        left_window_ids = list()
        for wid in window_ids:
            if wid * self.crop_step_size >= 3 * self.n_samples:
                left_window_ids.append(wid)
        if len(left_window_ids) == 0:
            left_window_ids = window_ids

        random.shuffle(left_window_ids)
        # Return up to max_num_crops
        max_num_crops = self.max_num_crops
        if max_num_crops is None:
            max_num_crops = max(1, audio.size(1) // self.n_samples)
