...
from recipes.voicebox.diffusion_modules.ECAPA_TDNN.model import ECAPA_TDNN_GN
from recipes.voicebox.diffusion_modules.util_layers import RMSNorm
from recipes.voicebox.diffusion_modules.loss import MaskedMAELoss, MaskedMSELoss
...
from recipes.voicebox.diffusion_modules.a_unet.blocks import NumberEmbedder, Repeat
from recipes.voicebox.diffusion_modules.dpm_solver_pytorch import NoiseScheduleVP, model_wrapper, DPM_Solver
...
class LlamaDiffusion(nn.Module):
    def __init__(self, hp):
        ...
        if self.use_prompt:
            ...
            self.local_cond_project = nn.Linear(
                ...
                hp.local_cond_dim, bias=self.bias)
        else:
            self.local_cond_project = nn.Linear(
                hp.token_hidden_dim,
                hp.local_cond_dim, bias=self.bias)
    ...
    def _forward(self, x, local_cond, text_embed, timesteps,
        ...
        if  x.shape[0] != local_cond.shape[0]:
            x = x.repeat([2,1,1])
        residual = x # 1,800,64 / # 2,800,64, 4, 800, 256
        time_emb = self.time_embedding(timesteps) # 2, 800, 256
        time_emb = time_emb.unsqueeze(1).expand(-1, local_cond.shape[1], -1)
        ...
        if self.target_type == "velocity":
            ...
            raise NotImplementedError
        return pred # 2, 800, 64

    def ddim_sample(self, timesteps, local_cond, text_embed, 
            text_cfg_w=1.0, 
            eta=0.0):
        t = timesteps
        batch_size, device, frm_len = local_cond.size(0), local_cond.device, local_cond.size(1)
        if text_cfg_w != 1:
            uncondition_batch_size = batch_size // 2
        else:
            uncondition_batch_size = batch_size
        x = torch.randn([uncondition_batch_size, frm_len, self.hp.out_channels], device=device)  # 1, 800, 64
        ...
        sigmas = repeat(sigmas, "i -> i b", b=uncondition_batch_size)
        sigmas_batch = extend_dim(sigmas, dim=x.ndim+1)
        alphas, betas = self.get_alpha_beta(sigmas_batch)

        for i in tqdm(range(t)):
            if self.target_type == "velocity":
                if text_cfg_w != 1:
                    v_pred, v_pred_uncond = self._forward(x, local_cond, text_embed, 
                            timesteps=sigmas[i].expand(batch_size, -1)).chunk(2)
                    v_pred = text_cfg_w * v_pred + (1 - text_cfg_w) * v_pred_uncond # 1, 800, 64, 
                else:
                ...
                x_pred = self._forward(x, local_cond, text_embed, timesteps=sigmas[i],
                        alphas=alphas[i], betas=betas[i]) 
    ...
    def inference(self, inputs, timesteps=20, sampler="ddim", 
        ...
        if sampler == "ddim":
            x = self.ddim_sample(timesteps, local_cond, text_embed, 
...
if __name__ == "__main__":
    ...
    from recipes.voicebox.modules.loss import MaskedMAELoss, MaskedMSELoss
