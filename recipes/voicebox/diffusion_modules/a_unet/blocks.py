...
class Attention(nn.Module):
    def __init__(
        ...
        **kwargs
        ...
        if not self.use_ctiga:
            ...
            self.ctiga_attention = FlashCrossAttentionV2(False, head_features ** (-0.5))
            self.to_out =  nn.Linear(in_features=mid_features, out_features=features, bias=False)
    ...
    def forward(self, x: Tensor, mask=None) -> Tensor:
        ...
        if not self.use_ctiga:
            ...
            if mask is not None:
                return skip + self.attention(q, k, v, mask.unsqueeze(2)*mask.unsqueeze(1))
            else:
            ...
            if mask is None:
                ...
                out = self.ctiga_attention(
                    ...
                    rearrange(kv, "b n (two h d) -> b n two h d", two=2, h=self.head_features),
                    None,
                    None,
                    None,
                    None,
                ...
                return skip + self.to_out(rearrange(out,"b n h d -> b n (h d)"))
            else:
                bs = x.shape[0]
                x, indices_q, cu_seqlen_q, max_seqlen_q = unpad_input(x, mask)
                context, indices_k, cu_seqlen_k, max_seqlen_k = unpad_input(context, mask)
                q, kv = (self.to_q(x), self.to_kv(context))
                out = self.ctiga_attention(
                    rearrange(q, "n (h d) -> n h d", h=self.head_features),
                    rearrange(kv, "n (two h d) -> n two h d", two=2, h=self.head_features),
                    None,
                    cu_seqlen_q,
                    max_seqlen_q,
                    cu_seqlen_k,
                ...
                out = pad_input(self.to_out(rearrange(out,"t h d -> t (h d)")), indices_q, bs, max_seqlen_q)
                skip += out
                return skip
...
class CrossAttention(nn.Module):
    def __init__(
        ...
        **kwargs
        ...
        if not self.use_ctiga:
            ...
            self.to_k = nn.Linear(
                in_features=context_features, out_features=mid_features, bias=False
            )
            self.to_v = nn.Linear(
                in_features=context_features, out_features=mid_features, bias=False
            )
    ...
    def forward(self, x: Tensor, context: Optional[Tensor], mask=None, mask2=None) -> Tensor:
        assert_message = "You must provide a context when using context_features"
        assert not self.context_features or exists(context), assert_message
        skip = x
        # Use context if provided
        ...
        if not self.use_ctiga:
            ...
            if mask is None and mask2 is None:
                ...
                out = self.ctiga_attention(
                    ...
                    rearrange(kv, "b t (two h d) -> b t two h d", two=2, h=self.head_features),
                    None,
                    None,
                    None,
                    None,
                ...
                x, indices_q, cu_seqlen_q, max_seqlen_q = unpad_input(x, mask)
                context, indices_k, cu_seqlen_k, max_seqlen_k = unpad_input(context, mask2)
                x, context = self.norm(x), self.context_norm(context)
                q, kv = (self.to_q(x), self.to_kv(context))
                out = self.ctiga_attention(
                    rearrange(q, "t (h d) -> t h d", h=self.head_features),
                    rearrange(kv, "t (two h d) -> t two h d", two=2, h=self.head_features),
                    None,
                    cu_seqlen_q,
                    max_seqlen_q,
                    cu_seqlen_k,
