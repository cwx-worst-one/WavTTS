...
from recipes.voicebox.utils.infer_utils import load_torch_script
...
def init_vocoder_from_ts(dec_ts_path, local_rank, cache_dir):
    with local_zero_first():
        return {
                "vocoder": load_torch_script(dec_ts_path, local_rank, cache_dir), 
            }
...
class Wave():
    def __init__(self, encoder, decoder, version=3.1, hop_size=300, win_size=1200) -> None:
    ...
    def encode(self, wav):
        if self.version == 3.1 or self.version == 2.1:  # 3.1 和 2.1 的输入是 [wav, spec, mel_spec]
            wav = wav.unsqueeze(1).float()
            wav = F.pad(wav, (0, (wav.size(-1) // 600 + 1) * 600 - wav.size(-1), 0, 0, 0, 0), value=0.)
            spec = spectrogram_torch(wav.squeeze(1), 2048, 24000, self.hop_size, self.win_size)
            mel_spec = mel_spectrogram_torch(y=wav.squeeze(1),
                                         n_fft=2048,
                                         num_mels=80,
                                         sampling_rate=24000,
    ...
    def decode(self, bn):
        ...
        wav = self.decoder(z.transpose(1,2)).squeeze(1)
        return wav
    
    def decoder_from_z(self, z):
        dec1 = self.decoder(z.transpose(1,2))
        wav = dec1.squeeze(1)
        return wav
