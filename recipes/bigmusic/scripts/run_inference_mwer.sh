...
python3 -m samantha.main predict --config recipes/bigmusic/conf/inference_semantic_30s.yaml \
    ...
    --extra_params.inference_conditions style_text,lyrics_tokens,random_style_rewrite \
    --extra_params.prompt_path $PROMPT_PATH \
    --extra_params.max_items null --run_opts.batch_size 32 \
    --extra_params.semantic_ckpt /mnt/bn/lyrics-to-song/ashaw/logs/bigmusic/semantic_vocal/semantic_model_mulan/semantic_700m_30s_8gpu/checkpoints/last.ckpt \
...
python3 -m samantha.main predict --config recipes/bigmusic/conf/inference_semantic_30s.yaml \
    ...
    --extra_params.lyrics_max_seq_len 400 \
    --extra_params.semantic_ckpt /mnt/bn/audio-diffusion/jt/model_archive/vocalmusic/semantic_model_mulan/max400_30s_semantic_rewrite_GenreMoodGender_l24_h24_b8_g64_2em4/checkpoints/last-v1.ckpt \
    --extra_params.token2wav_type diffusion
