...
# earlier coarse ckps
python3 -m samantha.main predict --config recipes/bigmusic/conf/inference_gt.yaml --extra_params.output_dir $OUTPUT_DIR/$FOLDER_NAME/coarse_700m_195k \
    --extra_params.inference_conditions style_audio --extra_params.prompt_path $PROMPT_PATH \
    --extra_params.max_items null --run_opts.batch_size 24 \
    --extra_params.token2wav_type ar

# later coarse ckpt
python3 -m samantha.main predict --config recipes/bigmusic/conf/inference_gt.yaml --extra_params.output_dir $OUTPUT_DIR/$FOLDER_NAME/coarse_700m_295k \
    --extra_params.inference_conditions style_audio --extra_params.prompt_path $PROMPT_PATH \
    --extra_params.max_items null --run_opts.batch_size 24 \
    --extra_params.token2wav_type ar \
