...
CUDA_VISIBLE_DEVICES=0 python3 -m samantha.main predict --config recipes/bigmusic/conf/inference_semantic_30s_vocalapp.yaml \
    ...
    --extra_params.inference_conditions style_text,vocal_audio \
    --extra_params.prompt_path $PROMPT_PATH \
    --extra_params.voice_clone_duration -1 \
    --extra_params.app_type singsong \
    --extra_params.token2wav_type diffusion \
    --extra_params.semantic_ckpt /mnt/bn/audio-diffusion/semantci_label_134/semantic_model_mulan_text_07B/singsong_mulan_BGM_v1/checkpoints/step=008000-val_accu_0=15.18.ckpt
