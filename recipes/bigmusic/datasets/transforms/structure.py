...
class ChorusDetectionTransform:
    ...
    def find_key_points(self, volume, mode, start, end):
        if mode == "onset":
            ...
            sort_fn = lambda cands: map(lambda x: x[0], sorted(cands, key=lambda x: -x[1]))
        elif mode == "offset":
        ...
        for onset in onset_group:
            for offset in offset_group:
                ...
                if self.is_loud(volume, threshold, onset, offset):
                    segment = ("chorus", self.index_to_time(onset), self.index_to_time(offset))
    ...
    def find_chorus(self, audio):
        ...
        offsets = self.find_key_points(volume, "offset", self.min_duration, len(volume) + 1)
