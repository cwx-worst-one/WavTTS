...
class ARVSampler(nn.Module):
    ...
    def sample_loop(
        ...
        sigma = 1.
        repaint = 1 if first else 10
        repaint_noise = torch.randn_like(current)
        for i in progress_bar:
            for j in range(repaint):
                ...
                if first or j == (repaint - 1):
                    sigma = sigma - angle_schedule[i]
                current = np.cos(omega) * current - np.sin(omega) * v_pred
                print(angle_schedule[i], sigma, omega, np.cos(omega), np.sin(omega))
                if not first and j < (repaint - 1):
                    # current = np.cos(omega) * current + np.sin(omega) * v_pred
                    # current = np.cos(omega) * current + np.sin(omega) * repaint_noise
                    current =  np.cos(sigma) * current + np.sin(sigma) * torch.randn_like(current)
    ...
    def forward(
        ...
        tmp_emb = torch.zeros(b, c, 1250 + diffusion_hop_size *(num_chunks - 1), device=self.device)
        avg_cnt = torch.zeros(b, c, 1250 + diffusion_hop_size *(num_chunks - 1), device=self.device)
        ...
        for i in range(num_chunks):
            ...
            pred_emb = self.sample_loop(
                ...
                current=current_emb,
                prev_noise=prev_noise[..., 0 + (i*diffusion_hop_size):1250 + (i*diffusion_hop_size)],
                num_steps=num_steps,
            ...
            tmp_emb[..., 0 + (i*diffusion_hop_size):1250 + (i*diffusion_hop_size)] += pred_emb
            avg_cnt[..., 0 + (i*diffusion_hop_size):1250 + (i*diffusion_hop_size)] += 1
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '--semantic_model_cls',
        type=str,
        default='mix' # mix or normal
    )
    parser.add_argument(
        '--semantic_model_path',
        type=str,
        default='/mnt/bn/audio-diffusion/jt/model_archive/varlen_20_30_bs20_multi_03B_6w/step=140000-val_accu_0=18.91.ckpt'
    )
    parser.add_argument(
        '--diffusion_model_path_2_0',
        type=str,
        default='/mnt/bn/audio-diffusion/wtl/diffusion/model_14_fix_rot/checkpoints/last.ckpt'
    )
    parser.add_argument(
    ...
    if args.semantic_model_cls == 'normal':
        semantic_module_cls = SemanticModule
    elif args.semantic_model_cls == 'mix':
    ...
    lyrics_tokenizer = LyricsTokenTransform.init_espeak_tokenizer(lyrics_max_seq_len=args.lyrics_max_seq_len, truncate_long_lyrics=True)
    ...
    prompts = []
    import csv
    csv_file = '/mnt/bn/audio-diffusion/data/mixture_prompts/multi-tag-30s-short.csv'
    ...
    with torch.no_grad():
        for i in range(0, len(prompts), args.batch_size):
            ...
            # repeat condition so we can generate multiple samples
            ...
            if args.semantic_model_cls == 'mix':
            ...
            if args.semantic_model_cls == 'mix':
                for i, (wav_g, eos) in enumerate(zip(wavs_g, eos_index)):
                    ...
                    file_name = _prompts[int(i//args.samples_per_prompt)][:100]
                    torchaudio.save(
                        f'{args.output_dir_path}/{i%args.samples_per_prompt}_{file_name}.wav',
                        wav_g.cpu(),
                        24000,
                    )
            else:
                for i, wav_g in enumerate(wavs_g):
