...
from recipes.musicfm.modules.data_transform import (
    AudioTransformsForStat,
    PlaylistAudioTransformsForStat,
)
...
class Preprocessor:
    def __init__(
        self, batch_size=16, num_workers=4, hop_length=240, n_fft=2048, total_samples=0
    ):
        ...
        web_dataset = WebDataset(
            "pipe:hdfs dfs -cat /home/byte_speech_sv/mulan/playlist_v5_uio/audio_shards/shards_{0000..2596}.tar"
        )
        data_transform = PlaylistAudioTransformsForStat(30, 24000, hop_length, n_fft)
        ...
        train_dataset = (
            web_dataset.shuffle(100)
            .decode()
            .compose(buffer_processor.train_buffer_preprocessor)
            .to_tuple("spec mel_spec cqt mfcc chromagram")
            .batched(batch_size)
        )
    ...
    def update(self, x, feature_name):
        ...
        self.stat["%s_mean" % feature_name] += (
            delta * num_samples / self.stat["%s_cnt" % feature_name]
        )

        # update std
        new_std = (
            ((x - new_mean) * (x - self.stat["%s_mean" % feature_name])).sum().numpy()
        )
    ...
    def save_features(self):
        ...
        print(stat)
        with open(
            "/mnt/bn/audio-diffusion/pretrained_models/musicfm/playlist_classic_stats.json",
            "w",
        ) as file:
            json.dump(stat, file)
