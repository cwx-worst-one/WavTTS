...
def get_triton_activation_index(activation: Optional[Activation]) -> int:
    return (
        {Activation.ReLU: 1, Activation.LeakyReLU: 2, Activation.GeLU: 3}[activation]
...
def relu_grad(x):
    ...
    # in that it does not require the input to retrospectively compute its
    # gradient here the input is the downstream gradient, and we return the
    # upstream gradient directly
