...
from .k_fused_matmul_bw import fused_matmul_backward
from .k_fused_matmul_fw import fused_matmul
...
class _fused_linear_triton(torch.autograd.Function):
    ...
    def backward(
        ...
        """
        Compute the derivative with respect to x, other tensors were not
        trainable inputs.
        """
...
class FusedLinear(nn.Module):
    """
    Handle a linear transform, like torch.nn.Linear_, and a given activation,
    in a single kernel. The whole transform: is :math:`y = activation(xA^T + b)`

    This is typically significantly faster than PyTorch while using fp16 and
    non-sigmoid activations.
