...
import pytest
import torch
from byteformers.convert.checkpoints.convert_llama import (
    calc_rotary_inv_freq,
    convert_llama_state_dict,
)
from byteformers.models import Llama, LlamaConfig, LlamaModel

from tests.runif import RunIf
from tests.utils import ids_tensor, torch_device
...
def test_llama_kv_cache(model_tester: LlamaModelTester, orig_llama) -> None:
    ...
    sampled_ids_cache = sample_with_kv_cache(
        sampled_ids_cache, start_idx=0, kv_cache=kv_cache
    )
    ...
    torch.testing.assert_close(sampled_ids, sampled_ids_cache_prefix)
