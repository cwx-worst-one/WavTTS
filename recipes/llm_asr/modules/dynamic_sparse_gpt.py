...
def default_layout_fn(train_len, num_heads=16, block=32):
...
def sparse_layout_fn(train_len, num_heads=16, block=32):
...
class GPT2Attention(nn.Module):
    ...
    def __init__(self, config, is_cross_attention=False, layer_idx=None):
        ...
        self.c_proj = Conv1D(self.embed_dim, self.embed_dim, bias=False)
        ...
        # pre-compute sparsity layout and fns
        if config_get(config,"sparse_attn",False):
            self.block_sparse = True
            self.layer_out_dict = {}
            self.block = 32
            self.train_len_min = config_get(config, "train_len_min", 128)
            self.train_len_max = config_get(config, "train_len_max", 2048)
            for train_len in torch.arange(self.train_len_min, self.train_len_max+32, 32):
                mask, block = default_layout_fn(train_len)
                self.layer_out_dict[f"{train_len}"] = (mask, block)
        else:
            self.block_sparse = False

    def _get_sparse_fn(self, layout, block, device):
        ...
        self.softmax_fn = sparse_softmax(layout,
                                         block,
                                         device,
                                         is_dense=False)
    ...
    def _sparse_attn(self,
        ...
        q_len = query.shape[2]
        layout, block = self.layer_out_dict[f"{q_len}"]
        self._get_sparse_fn(layout, block, device=query.device)
    ...
    def _attn(self, query, key, value, attention_mask=None, head_mask=None):
        if self.training and self.block_sparse:
            dtype = value.dtype
...
class GPT2MLP(nn.Module):
    ...
    def __init__(self, intermediate_size, config):
        ...
        self.c_proj = Conv1D(embed_dim, intermediate_size, bias=False)
...
class GPT2LMHeadSparseModel(GPT2PreTrainedModel):
    ...
    def parallelize(self, device_map=None):
        self.device_map = (get_device_map(len(self.transformer.h),
