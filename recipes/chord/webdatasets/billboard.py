...
_HDFS_DIRS = {
    ...
    "SheetDoctor/labels/billboard_truth_chord.zip",
}
...
class BillboardDataset(AbstractWebDatasetGeneratorBasedBuilder):
    ...
    SAMPLING_RATE = 16000
    DATASET_NAME = "billboard_chord"
    AUDIO_NUMPY_DTYPE = np.float32
    DATASET_ROOT_DIR = os.path.join('/mnt/bn/mir-tasks/', DATASET_NAME)
    ...
    def _split_data(self, data_dir):
        keys = []
        for k in os.listdir(os.path.join(data_dir, "billboard_truth_chord")):
            k = k.split(".txt")[0]
    ...
    def _load_example(self, key):
        # read annotations
        with open(os.path.join(self.label_dir, "billboard_truth_chord", key + ".txt")) as f:
