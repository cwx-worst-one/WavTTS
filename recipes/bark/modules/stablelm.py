...
from transformers.modeling_utils import deepspeed_config, is_deepspeed_zero3_enabled
...
class GPTNeoXAttention(nn.Module):
    ...
    def forward(
        ...
        # Compute attention
        if not self.training or not self.sparse:
            attn_output, attn_weights = self._attn(query, key, value, attention_mask, head_mask)
        else:
            attn_output, attn_weights = self._sparse_attn(query, key, value, attention_mask, head_mask)
    ...
    def _sparse_attn(self, xq, keys, values, attention_mask=None, head_mask=None):
        ...
        output = wv_matmul(scores.to(values.dtype), values) # [b, head, tq, d]
        output = output[:, :, 0:size[1], :]
...
class GPTNeoXForCausalLM(GPTNeoXPreTrainedModel):
    ...
    def _get_resized_embeddings(
        ...
        new_embeddings.weight.data.normal_(mean=0.0, std=old_embeddings.weight.data.std())
    ...
    def _get_resized_lm_head(
        ...
        new_lm_head.weight.data.normal_(mean=0.0, std=old_lm_head.weight.data.std())
...
model.resize_token_embeddings(len(tokenizer) + 8192) # tokenizer + semantic
'''
