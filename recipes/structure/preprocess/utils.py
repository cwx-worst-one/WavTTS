...
def intervals_to_activation(
    ...
    lls = np.round(np.array(intervals) * hop_per_sec).astype(np.int)
    # sec to sample
    end_idx = np.round(song_duration * hop_per_sec).astype(np.int)
    timeweights = {}
    for label in lls:
        start, end = label
...
def boundary_activation(
    ...
    lls = np.round(np.array(times) * hop_per_sec).astype(np.int)
    # sec to sample
    end_idx = np.round(song_duration * hop_per_sec).astype(np.int)
    timeweights = {}
    for label in lls:
