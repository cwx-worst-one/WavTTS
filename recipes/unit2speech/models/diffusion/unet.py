...
class CrossAttention(nn.Module):
    ...
    def forward(self, x, context=None, mask=None):
        ...
        #print(x.shape, context.shape, flush=True)
        #print(context.shape, self.context_dim, flush=True)
        k = self.to_k(context)
        v = self.to_v(context)
...
class UNetModel(nn.Module):
    ...
    def __init__(
        ...
        self.from_1d_to_2d = from_1d_to_2d
        ...
        for level, mult in enumerate(channel_mult):
            for _ in range(num_res_blocks):
                ...
                if ds in attention_resolutions:
                    ...
                    #print(ds, context_dim, flush=True)
                    layers.append(
                        AttentionBlock(
                            ch,
                            use_checkpoint=use_checkpoint,
    ...
    def forward(self, x, timesteps=None, context=None, clip_embed=None, image_embed=None, super_res_embed=None, y=None,**kwargs):
        ...
        if self.num_classes is not None:
            ...
            emb = emb + self.label_emb(y.float())

        if image_embed is not None:
            x = th.cat([x, image_embed], dim=1)

        h = x.type(self.dtype)
        #print('h:', h.shape, flush=True)
        l = 1
        for module in self.input_blocks:
            #if context is not None:
            #    print(context.shape, flush=True)
            #print(l, 'h:', h.shape, flush=True)
            l += 1
