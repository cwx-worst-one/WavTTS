...
class DPRTNet(nn.Module):
    def __init__(self,
        ...
        self.prompt_map = ECAPA_TDNN_GN(80, self.feature_dim, self.feature_dim)
        self.prompt_norm = RMSNorm(self.feature_dim)
    ...
    def forward(self, x, timesteps=None, context=None, prompt=None, cfg=False):
        ...
        seq_emb = self.context_embed(context.long().detach())
        seq_emb = self.context_upsampler(seq_emb.transpose(1, 2).contiguous(), seq_length)
        seq_emb = self.context_norm(seq_emb)
        #p_emb = self.context_embed(prompt.long().detach()).transpose(1, 2).contiguous()
        mel_p = self.mel_transform(prompt)
        p_emb = self.prompt_norm(self.prompt_map(mel_p).view(batch_size, 1, self.feature_dim))
        #scale, shift = torch.chunk(p_emb, 2, dim=-1)
...
class ECAPA_TDNN(nn.Module):
    def __init__(self, C, out_dim):
        ...
        self.bn6 = RMSNorm(out_dim, feat_dim=1)
