...
class NoiseFC(nn.Module):
    ...
    def __init__(self, dim, trunc_noise=True):
        super().__init__()
        return
        # inspired by stylegan
        self.fcs = nn.Sequential(
            weight_norm(nn.Conv1d(dim, 512, kernel_size=1)),
...
class WNResBlock1(torch.nn.Module):
    ...
    def __init__(self, h, channels, kernel_size=3, dilation=(1, 3, 5)):
        ...
        self.noise_convs = nn.ModuleList([
            NoiseFC(channels, self.h.trunc_noise),
            NoiseFC(channels, self.h.trunc_noise),
            NoiseFC(channels, self.h.trunc_noise),
            NoiseFC(channels, self.h.trunc_noise),
            NoiseFC(channels, self.h.trunc_noise),
...
class ModulatedConv(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, stride=1, dilation=1, padding=0, groups=1, bias=True):
        super().__init__()
        self.stride = stride
        ...
        if kernel_size * dilation - dilation - 2 * padding == 0:
            ...
            raise Exception
        # weight
        weight = torch.randn(size=[out_channels, in_channels, kernel_size], dtype=torch.float32) / np.sqrt(out_channels)
    ...
    def forward(self, x):
        if not self.infer_mode:
            ...
            w = self.w_norm
        x = F.conv1d(x, w, bias=self.bias, stride=self.stride, padding=self.padding, dilation=self.dilation, groups=self.groups)
        return x
