    ...
    def forward(self, x):
        ...
        for i, l in enumerate(self.convs):
            ...
            if i >= self.fmap_depth:
                fmap.append(x)
        x = self.conv_post(x).to(dtype=x.dtype)
        fmap.append(x)
...
class MultiScaleSTFTDiscriminator(nn.Module):
    ...
    def __init__(self, filters: int,
        ...
        self.mpds = nn.ModuleList([
            ...
            #DiscriminatorP(7, fmap_depth=2),
            #DiscriminatorP(11, fmap_depth=2),
        ])

    def forward(self, x: torch.Tensor, y: torch.Tensor) -> DiscriminatorOutput:
