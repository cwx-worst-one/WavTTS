...
def random_formant_shift(wav):
    B = wav.shape[0]
    formant_shift_upper = 1.15
    formant_shift_lower = 0.85
    formant_shift_ratios = np.random.rand(B)*(formant_shift_upper-formant_shift_lower) + formant_shift_lower
    pitch_shift_ratios = np.random.rand(B)*(formant_shift_ratios-1.0)+1.0
    shift_wavs = []
    for i in range(B):
        sound = wav_to_Sound(wav[i, :], 24000)
        shift_wav = apply_formant_and_pitch_shift(sound,
                formant_shift_ratios[i],
                pitch_shift_ratios[i],
                1, 1).values[0]
        shift_wavs.append(shift_wav)
    return np.stack(shift_wavs)
...
def parametric_equalizer(wav: torch.Tensor, sr: int) -> torch.Tensor:
    ...
    # gains = [random.uniform(-24, 24) for _ in range(num_filters)]
