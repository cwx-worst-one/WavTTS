...
class SoundStormMaskingScheme(MaskingScheme):
    ...
    def __init__(
        ...
        Args:
            sample_q_uniformly (bool): Sample all quantizers uniformly, or to use a cosine
                schedule for sampling q's (favouring coarse quantizers).

            sample_t (bool): Choose timestep $t$ for masking all tokens after $t$
            max_sample_t (Optional[int]): The maximum timestep we use for sample_t (exclusive)
            keep_coarse_quant_idx (Optional[int], optional): Keep coarse quantizers lower than this
                index (e.g., for training a fine-token SoundStorm). Defaults to None.
        """
        super().__init__()
    ...
    def forward(
        ...
        # offset the n_quantizers we can sample from first
        if self.keep_coarse_quant_idx is not None:
            n_quantizers = n_quantizers - self.keep_coarse_quant_idx

        if self.sample_t:
            max_t = self.max_sample_t if self.max_sample_t is not None else seq_len
            rand_ts = torch.randint(0, max_t - 1, (batch_size,), device=device)
