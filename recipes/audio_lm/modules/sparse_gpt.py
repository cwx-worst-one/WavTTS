...
class FixedAbsolutePositionEmbedding(nn.Module):
    def __init__(self, max_pos, hidden_size, pos_type="rope"):
        ...
        inv_freq = 1.0 / (
            10000 ** (torch.arange(0, hidden_size, 2, dtype=torch.float) / hidden_size)
        )
    ...
    def __init__(self, config, is_cross_attention=False, layer_idx=None):
        ...
        if self.pos_type == "rope_attn":
            self.wpe = FixedAbsolutePositionEmbedding(
                max_positions, self.head_dim, self.pos_type
            )
    ...
    def __init__(self, config, layer_idx=None):
        ...
        if norm_fn == "layer_norm":
            ...
            raise NotImplementedError(
                'Norm function must be "layer_norm" or "rms_norm", but get {}'.format(
                    norm_fn
                )
            )
        ...
        if config.add_cross_attention:
            ...
            self.ln_cross_attn = norm_fn(hidden_size, eps=config.layer_norm_epsilon)
