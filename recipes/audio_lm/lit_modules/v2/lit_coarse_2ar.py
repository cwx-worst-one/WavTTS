...
class CoarseModule(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        if self.local_rank == 0:
            print("Coarse 2AR Training, V2...")
        loss = self.criterion(logits[:, 0 : org_len - 1, :], input_tokens[:, 1:org_len])
        accu = (
            100
            * (
                logits[:, 0 : org_len - 1, :].argmax(dim=-1)
                == input_tokens[:, 1:org_len]
            ).sum()
            / torch.ones_like(input_tokens[:, 1:org_len]).sum()
        )
        self.log_dict(
    ...
    def prepare_feature(self, wavs):
        ...
        # prepare wav ids, offset: semantic 1024
        ...
        # eos_id, offset: semantic 1024, num_coarse
        eos_ids = (
        ...
        # mulan_tokens 12 + prompt 1 + eos 1 + w2v 748 + eos 1 + ss 4800
        input_tokens = torch.cat([mulan_tokens, eos_ids + 1, wav_ids], dim=1)
