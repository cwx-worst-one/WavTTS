...
class CoarseModule(pl.LightningModule):
    ...
    def shared_step(self, batch):
        ...
        with torch.autocast(device_type="cuda", enabled=False):
            mulan_embeds, w2v_tokens, input_tokens = self.prepare_feature(batch)
            ...
            mulan_embeds = self.model.cond_layer(mulan_embeds)
            inputs_embeds = torch.cat([mulan_embeds, inputs_embeds], dim=1)
        logits = self.model(inputs_embeds=inputs_embeds)["logits"]

        if self.local_rank == 0:
            print("Coarse 3AR Training, V6_3 finetune...")
    ...
    def prepare_feature(self, batch):
        ...
        w2v_tokens = self.get_w2v_token(batch["audio"].float())
        b, t = w2v_tokens.size()

        # offset: semantic
        wav_ids = self.get_quant(batch["audio"].float())
        wav_ids = (
        ...
        mulan_embeds = (
            self.requires["mulan"].text_encoder(*text_input).unsqueeze(1)
        )  # [b, 1, d]
