...
if [ ! -e last.ckpt ]
then
    bash recipes/audio_lm/bootstrap.sh fit --config recipes/audio_lm/conf/fine_v3_sparse.yaml --run_opts.hdfs_path=$1
else
    bash recipes/audio_lm/bootstrap.sh fit --config recipes/audio_lm/conf/fine_v3_sparse.yaml --run_opts.hdfs_path=$1 --ckpt_path=./last.ckpt
fi
