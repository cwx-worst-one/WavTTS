...
class WaveformVAEModule(pl.LightningModule):
    ...
    def training_step_G_and_D(self, batch, batch_idx):
        ...
        if self.current_step > self.train_cls.get("discriminator_train_start_steps"):
            self.log_dict(
                {
                    "training/loss_disc": loss_disc,
                    "training/loss_gen": loss_gen,
                    "training/loss_fm": loss_fm,
                    "training/loss_mel": loss_mel,
                    "training/loss_sc": loss_sc,
                    "training/loss_mag": loss_mag,
                    "training/loss_kl": loss_kl,
                    "training/grad_norm_d": grad_norm_d,
                    "training/grad_norm_g": grad_norm_g,
                    "training/loss_mel_z": loss_mel_z,
        ...
        self.log_dict(
            {
                ...
                "aux/batch": batch.size(0),
                "training/mfu": mfu,
                "training/max_memory_alloc": torch.cuda.max_memory_allocated() / 2**30,
