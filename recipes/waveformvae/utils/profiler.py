...
"""
    ...
    Copy from pytorch_lightning. The original implementation
    does not support torch.jit.script applied module.
"""
...
class ScheduleWrapper:
    ...
    def __call__(self, num_step: int) -> "ProfilerAction":
        ...
        if (
            self._prev_schedule_action == ProfilerAction.RECORD
            and action == ProfilerAction.WARMUP
        ):
...
class PyTorchProfiler(Profiler):
    ...
    def __init__(
        ...
        **profiler_kwargs: Any,
        ...
        self._sort_by_key = (
            sort_by_key
            or f"{'cuda' if profiler_kwargs.get('use_cuda', False) else 'cpu'}_time_total"
        )
        ...
        for key in self._table_kwargs:
            ...
            valid_table_keys = set(
                inspect.signature(EventList.table).parameters.keys()
                    ...
                    if self._export_to_chrome:
                        handler = tensorboard_trace_handler(
                            ...
                            self._prepare_filename(
                                action_name=action_name, extension=""
                            ),
                        )
                        handler(profiler)
    ...
    def summary(self) -> str:
        ...
        table = data.table(
            sort_by=self._sort_by_key, row_limit=self._row_limit, **self._table_kwargs
        )
