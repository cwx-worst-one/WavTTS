...
class ResidualCouplingBlock(nn.Module):
    ...
    def flops(self, in_shape):
        return self.flows[0].flops(in_shape)[0] * len(self.flows), in_shape
...
class DiscriminatorP(torch.nn.Module):
    ...
    def flops(self, in_shape):
        flops_ = 0.0
        b, c, d = in_shape
        cur_shape = [b, 1, d//self.pqmf.subbands, self.pqmf.subbands] #self.pqmf.analysis and transposed shape
        for i, l in enumerate(self.convs):
            cur_flops, cur_shape = flops_conv2d(l, cur_shape)
            flops_ += cur_flops + calculate_product(cur_shape)
        return flops_
...
class DiscriminatorS(torch.nn.Module):
    ...
    def flops(self, in_shape):
        flops_ = 0.0
        cur_shape = copy.deepcopy(list(in_shape))
        for l in self.convs:
            cur_flops, cur_shape = flops_conv1d(l, cur_shape)
            flops_ += cur_flops+calculate_product(cur_shape)
        cur_flops, cur_shape = flops_conv1d(self.conv_post, cur_shape)
        flops_ += cur_flops
        return flops_
...
class MultiBandDiscriminator(torch.nn.Module):
    ...
    def flops(self, y_shape, y_hat_shape):
        flops_ = 0.0
        flops_ += self.full_discriminator.flops(y_shape)
        flops_ += self.full_discriminator.flops(y_hat_shape)
        y_shape_sliced = copy.deepcopy(list(y_shape))
        y_hat_shape_sliced = copy.deepcopy(list(y_hat_shape))
        y_shape_sliced[1] = 1
        y_hat_shape_sliced[1] = 1
        for i, d in enumerate(self.discriminators):
            flops_ += d.flops(y_shape_sliced)
            flops_ += d.flops(y_hat_shape_sliced)
...
class MRD(nn.Module):
    ...
    def flops(self, in_shape):
        ...
        bs, c, t = in_shape
        TT = t // int(self.window_length * self.hop_factor)
        bands_shapes = [ [bs, 2, TT, b[1]-b[0]] for b in self.bands]
        flops_ = 0.0
        for band_shape, stack in zip(bands_shapes, self.band_convs):
            for layer in stack:
                if isinstance(layer, torch.nn.Sequential):
                    cur_flops, band_shape = flops_conv2d(layer[0], band_shape)
                    flops_ += cur_flops + calculate_product(band_shape)
                else:
                    cur_flops, band_shape = flops_conv2d(layer, band_shape)
                    flops_ += cur_flops
        return flops_
...
class MBPRD(torch.nn.Module):
    ...
    def flops(self, y_shape, y_hat_shape):
        assert self.ifmbd
        flops_ = 0.0
        flops_ += self.extraD.flops(y_shape, y_hat_shape)
        for i, d in enumerate(self.mpd):
            flops_ += d.flops(y_shape)
            flops_ += d.flops(y_hat_shape)
...
class Downsample(nn.Module):
    ...
    def flops(self, in_shape):
        cur_flops, cur_shape = flops_conv1d(self.conv, in_shape)
        return cur_flops, cur_shape
...
class WVEncoder(torch.nn.Module):
    ...
    def flops(self, in_shape):
        assert isinstance(self.resblocks[0], modules.ResBlock1)
        flops_ = 0.0
        cur_shape = in_shape
        cur_flops, cur_shape = flops_conv1d(self.conv_pre, cur_shape)
        flops_ += cur_flops
        for i in range(self.num_downsamples):
            flops_ += calculate_product(cur_shape) #leaky_relu
            cur_flops, cur_shape = self.downs[i].flops(cur_shape)
            flops_ += cur_flops
            for j in range(self.num_kernels):
                cur_flops, cur_shape = self.resblocks[i*self.num_kernels+j].flops(cur_shape)
                flops_ += cur_flops
            flops_ += calculate_product(cur_shape) * self.num_kernels # res and mean
        flops_ += 3 * calculate_product(cur_shape) #dropout add relu
        cur_flops, cur_shape = flops_conv1d(self.conv_post, cur_shape)
        flops_ += cur_flops
        flops_ += calculate_product(cur_shape) #compute z
        cur_shape[1] = cur_shape[1] // 2
...
class MelPredictor(nn.Module):
    ...
    def flops(self, in_shape):
        flops_ = 0.0 
        cur_shape = in_shape
        cur_flops, cur_shape = flops_conv1d(self.pre, cur_shape)
        flops_ += cur_flops
        cur_flops, cur_shape = self.net.flops(cur_shape)
        flops_ += cur_flops
        cur_flops, cur_shape = flops_conv1d(self.out, cur_shape)
        return cur_flops, cur_shape
...
class SpecAccelerator(nn.Module):
    ...
    def flops(self, in_shape):
        ...
        cur_flops, cur_shape = flops_conv1d(self.pre, cur_shape)
        flops_ += cur_flops
        cur_flops, cur_shape = self.net_1.flops(cur_shape)
        flops_ += cur_flops
        cur_flops, cur_shape = self.down.flops(cur_shape)
        flops_ += cur_flops
        cur_flops, cur_shape = self.net_2.flops(cur_shape)
        flops_ += cur_flops
        cur_flops, cur_shape = flops_conv1d(self.out, cur_shape)
        flops_ += cur_flops
        return flops_, cur_shape
...
class WaveformVAE(nn.Module):
    def __init__(self,
        ...
        self.mel_accelerator = SpecAccelerator(80, downsample_initial_channel * (2 ** len(downsample_rates)), 256)
    ...
    def forward(self, x, spec, mel):
        # encoder
        accer = self.spec_accelerator(spec)
        accer_mel = self.mel_accelerator(mel)
        z, m, logs = self.encoder(x, accer + accer_mel)
    ...
    def flops(self, x_shape, spec_shape, mel_shape):
        flops_ = 0.0 
        cur_flops, __ = self.spec_accelerator.flops(spec_shape)
        flops_ += cur_flops
        cur_flops, __ = self.mel_accelerator.flops(mel_shape)
        flops_ += cur_flops
        cur_flops, z_shape = self.encoder.flops(x_shape)
        flops_ += cur_flops
        cur_flops, __ = self.mel_predictor.flops(z_shape)
        flops_ += cur_flops
        cur_flops, __ = self.flow.flops(z_shape)
        flops_ += cur_flops
        cur_flops, __ = self.vocoder.flops(z_shape)
        flops_ += cur_flops

        return flops_, z_shape


    def infer(self, x, spec, mel):
        # encoder
        accer = self.spec_accelerator(spec)
        accer_mel = self.mel_accelerator(mel)
        z, m, logs = self.encoder(x, accer + accer_mel)
        print(f"m mean: {m.mean().item()}, m var: {m.var().item()}, logs mean: {logs.mean().item()}, logs var: {logs.var().item()}")
        # decoder
        out = self.vocoder(z)
        return out
    ...
    def infer_test_sampling(self, x, spec):
        # encoder
        accer = self.spec_accelerator(spec)
        _, m, logs = self.encoder(x, accer)

        outs = []
        for scale in [0, 1, 2, 3, 1, 1, 1, 1, 1]:
            z = m + torch.randn_like(m) * torch.exp(logs) * scale
            # decoder
            out = self.vocoder(z)
            outs.append(out)
        return outs
