...
from recipes.waveformvae.models.waveformvae_modules.bigvgan_newup import BigVGAN
...
class WaveformVAE(nn.Module):
    def __init__(self,
                 ...
                 segment_size=9600,
                 bidirection_flow=False):
        super(WaveformVAE, self).__init__()
        self.bidirection_flow = bidirection_flow
        self.segment_size = segment_size
        self.down_size = 1
        for rate in downsample_rates:
            self.down_size *= rate

        with open(os.path.join("recipes", "waveformvae", "conf", "bigvgan_newup.json")) as f:
            data = f.read()
        ...
        self.posterior_encoder = SpecAccelerator(spec_channels, 2 * sd_channels, 256)
    ...
    def forward(self, x, spec):
        ...
        stats = self.posterior_encoder(spec)
        m_spec, logs_spec = torch.split(stats, stats.size(1) // 2, dim=1)
        
        z_p, logdet = self.flow(z, mask)
        kl_loss = kl_loss_no_form(z_p, logdet, logs, mask)
        # kl_loss = kl_loss_vits(z_p, logs, m_spec, logs_spec, mask)

        if self.bidirection_flow:
            z_spec = (m_spec + torch.randn_like(m_spec) * torch.exp(logs_spec))
            reverse_z_spec, _ = self.flow(z_spec, mask, reverse=True)
            kl_loss += kl_loss_vits(reverse_z_spec, logs_spec, m, logs, mask)
