...
from recipes.text2semantic.modules.ECAPA_TDNN_bias import ECAPA_TDNN
...
class VAELLaMaLangSpkSerAli(LLaMa):
    def __init__(
        ...
        if params.use_ref_enc:
            self.ref_enc = ECAPA_TDNN(params.out_dim, params.ref_enc_dim, params.dim, params.ref_enc_bn_dim)
    ...
    def forward(
        ...
        if self.use_ref_enc and crop_bn is not None:
            ...
            spk_embeds = spk_embd_masks.unsqueeze(-1).unsqueeze(-1) * spk_embeds + (1 - spk_embd_masks.unsqueeze(-1).unsqueeze(-1)) * spk_embeds_ref
