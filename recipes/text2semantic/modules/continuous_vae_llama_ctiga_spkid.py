...
class LLaMa(nn.Module):
    def __init__(self, params: ModelArgs, token_input=True, provider="default", state_dict_path=None):
        super().__init__()
        self.params = params
        ...
        if self.token_input:
            ...
            self.spk_embeddings = nn.Embedding(params.spk_num, params.dim)
    ...
    def forward(self, text_ids: torch.Tensor, text_id_lens: torch.Tensor, 
                    ...
                    inputs: torch.Tensor, spk_id: torch.Tensor,
                    start_pos: int = 0, use_cache=False, inference_params=None):
        ...
        if bns.shape[1] > 0:  # 正常训练和带prompt推理都应该走这个
            ...
            bn_in_h = torch.zeros([1, 0, self.params.dim]).to(bns.device) # noprompt 推理用的占位符, T为0所以等于没有任何内容
        ...
        if use_cache:
            ...
            spk_id_in_h = self.spk_embeddings(spk_id)
            h = []
            for i in range(bsz):
                h.append(
                    torch.cat(
                        # bos_text_sep + spkid + bn + eos_pad0
                        (token_in_h[i, :text_id_lens[i]+2, :], spk_id_in_h[i], bn_in_h[i, :bn_lens[i], :], token_in_h[i, bn_lens[i]+text_id_lens[i]+3:, :]),
                        dim=-2
                    )
                )
            h = torch.stack(h)
        ...
        h = self.forward_layers(h, start_pos=start_pos, seqlen=seqlen, inference_params=inference_params)
...
def checkpoint_converter(
    origin_ckpt_path, ctiga_ckpt_path, only_weights=False,
):
    import copy
    ...
    def map_to_ctiga(hf_state_dict, params):
        ...
        if only_weights:
            ctiga_state_dict["tok_embeddings.weight"] = ctiga_state_dict.pop("model.tok_embeddings.weight")
...
if __name__ == '__main__':
    pass
    # origin_ckpt_path = "/mnt/bn/cyz-lq-nas/work_dir/text2semantic/vae/avg_32A100_0712/0.0.1/checkpoints/epoch=13-step=51000-kl_loss=0.59.ckpt"
    # ctiga_state_dict_path = "/mnt/bn/cyz-lq-nas/work_dir/text2semantic/vae/avg_ctiga_51k/ctiga_only_weights.ckpt"
