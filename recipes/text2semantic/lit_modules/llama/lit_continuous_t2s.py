...
class ContinuousT2SModule(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        with self.profiler.profile("[LightningModule]CoarseModule.prepare_feature"):
            with torch.autocast(device_type="cuda", enabled=False):
                ...
                # bn_B, bn_T, bn_C = bns.shape
                # bn_loss_mask = sequence_mask(bn_lens, max_len=bn_T, device="cuda")
        ...
        with self.profiler.profile("[LightningModule]CoarseModule.model_forward"):
            ret_dict = self.model(text_ids, text_id_lens, bns, bn_lens, input_tokens)
        ...
        mae_loss = self.dense_criterion(pred_dense.float(), targets_dense, mask=loss_mask[:, 1:])
    ...
    def inference_from_text(self, batch, tokenizer):
        ...
        for i in tqdm(range(1500)):
            if i == 0:
                model_outputs = self.model(text_ids, text_id_lens, bns, bn_lens, input_tokens, start_pos=start_pos)
            else:
                model_outputs = self.model(None, None, bns, None, input_tokens, start_pos=start_pos, use_cache=True)
            ...
            if i > 25 and samples.item() == self.tokenizer_len + self.n_semantic + 2:
                break
