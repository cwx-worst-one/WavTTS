    ...
    def inference_from_text(self, batch, tokenizer):
        ...
        with torch.autocast(device_type="cuda", enabled=True):
            for i in tqdm(range(3000)):
                if i == 0:
                    model_outputs = self.model(text_ids, text_id_lens, bns, bn_lens, input_tokens, start_pos=start_pos, inference_params=inference_params)
                else:
                    model_outputs = self.model(None, None, bns, None, input_tokens, start_pos=start_pos, use_cache=True, inference_params=inference_params)
