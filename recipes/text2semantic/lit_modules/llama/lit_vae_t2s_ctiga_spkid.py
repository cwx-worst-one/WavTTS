...
class VAET2SModule(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        with self.profiler.profile("[LightningModule]CoarseModule.prepare_feature"):
            with torch.autocast(device_type="cuda", enabled=False):
                text_ids, text_id_lens, bns, bn_lens, input_tokens, seq_lens, pos_ids, seq_sen_ids, _, _, spk_id = batch
                b, t = input_tokens.shape
                loss_mask = sequence_mask(seq_lens, max_len=t, device="cuda")
                _, text_t = text_ids.shape
                text_loss_mask = sequence_mask(text_id_lens+3, max_len=text_t+3, device="cuda")
        ...
        with self.profiler.profile("[LightningModule]CoarseModule.model_forward"):
            ret_dict, _  = self.model(text_ids, text_id_lens, bns, bn_lens, input_tokens, spk_id)
        ...
        for i in range(bsz):
            targets_dense.append(
                F.pad(bns[i, :bn_lens[i], :], (0, 0, text_id_lens[i]+3, t-(bn_lens[i]+text_id_lens[i]+3)), "constant", 0)
            )
    ...
    def inference_from_text(self, batch, tokenizer):
        text_ids, text_id_lens, bns, bn_lens, seqs, seq_lens, pos_ids, seq_sen_ids, full_seqs, utt_ids, spk_ids = batch
        print(spk_ids)
        # text_ids, text_id_lens, bns, bn_lens, seqs, seq_lens, pos_ids, seq_sen_ids, utts, spk_id = batch
        b, t = seqs.shape   
        ...
        with torch.autocast(device_type="cuda", enabled=True):
            for i in tqdm(range(3000)):
                if i == 0:
                    model_outputs, bn_in_z = self.model(text_ids, text_id_lens, bns, bn_lens, input_tokens, spk_ids, start_pos=start_pos, inference_params=inference_params)
                else:
                    model_outputs, bn_in_z = self.model(None, None, bns, None, input_tokens, spk_ids, start_pos=start_pos, use_cache=True, inference_params=inference_params)
