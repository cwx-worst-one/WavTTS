...
train_meta_lst=/mnt/bd/huangzhiying-lq-valle-volume9/data/bytegen/valle/en_zh_TTS/meta_list_all.txt.add_metalen.shuf.train.fiter_90_2040.rm_libritts
valid_meta_lst=/mnt/bd/huangzhiying-lq-valle-volume9/data/bytegen/valle/en_zh_TTS/meta_list_all.txt.add_metalen.shuf.valid.fiter_90_2040.rm_libritts
...
if [ ${stage} -eq 1 ];then
    cd $work_dir
    log_name="ar"
    version="llama_fp16_lr3e-4_batch${batch_total_tokens}_90_2040_rm_libritts"
...
if [ ${stage} -eq 2 ];then
    cd $work_dir
    log_name="nar"
    version="gpt2_fp16_lr4e-4_batch${batch_total_tokens}_90_2040_rm_libritts"
    latest_ckpt_path=$log_dir/$log_name/$version/checkpoints/last.ckpt
    tensorboard --logdir $log_dir/${log_name} --port $ARNOLD_TENSORBOARD_CURRENT_PORT --bind_all &
