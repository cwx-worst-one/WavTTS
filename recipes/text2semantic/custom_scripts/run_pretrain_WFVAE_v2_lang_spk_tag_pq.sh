...
if [ ${stage} -eq 1 ];then
    log_name=pretrain_WFVAE_v2_labv3_punc_debug
...
# infer seed1996
if [ ${stage} -eq 2 ];then
    hdfs_path=hdfs://haruna/home/byte_data_seed/lf_lq/speech/user/huangzhiying.92/exp/samantha_bigtts_ref_enc/text2semantic/pretrain_WFVAE_v2_labv3_punc_data_id373_bt14000_16A100_accu5_scr0.15_spFalse_v2
    local_path=/mnt/bn/huangzhiying-nas-speech2speech-volume1/exp/samantha_bigtts_ref_enc/text2semantic/pretrain_WFVAE_v2_labv3_punc_data_id373_bt14000_16A100_accu5_scr0.15_spFalse_v2
    ar_ckpt_paths=`hdfs dfs -ls $hdfs_path/checkpoints/ | awk '{print $8}' | grep -E '90000'`
    for ar_ckpt_path in $ar_ckpt_paths;do
        step=`echo $ar_ckpt_path | awk -F"/" '{print $NF}' | cut -d "-" -f 2 | cut -d "=" -f 2`
        tag_id=2
        infer_spk_name='default'
        for pair in icl_testset_fighting-zh_better_studio;do
            ...
            lang='en'
            if [ $pair == "flow_tts_zh-wer_rand20" -o $pair == "flow_tts_zh-yuqici" -o $pair == "flow_tts_zh-temp_demo20231104_v7" -o $pair == "flow_tts_zh-deploy_test" -o $pair == "flow_tts_zh-erhuayin" -o $pair == "icl_testset_fighting-zh_better_studio" ]; then
                lang='zh'
            fi
            ...
            out_wav_dir=$local_path/infer/${pair}/step${step}_${infer_spk_name}_tag_id${tag_id}/wav
