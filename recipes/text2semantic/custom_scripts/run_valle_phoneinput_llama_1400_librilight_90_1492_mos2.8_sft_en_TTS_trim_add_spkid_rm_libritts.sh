...
train_meta_lst=/mnt/bd/huangzhiying-lq-valle-volume7/data/bytegen/valle/en_TTS_trim/meta_list_all.txt.add_metalen.fiter_90_1492.shuf.train.add_spkid.rm_libritts
valid_meta_lst=/mnt/bd/huangzhiying-lq-valle-volume7/data/bytegen/valle/en_TTS_trim/meta_list_all.txt.add_metalen.fiter_90_1492.shuf.valid.add_spkid.rm_libritts
...
if [ ${stage} -eq 1 ];then
    cd $work_dir
    log_name="ar"
    version="fp16_lr4e-4_batch${batch_total_tokens}_sft_en_TTS_trim_add_spkid_rm_libritts_retry"
...
if [ ${stage} -eq 2 ];then
    cd $work_dir
    log_name="nar"
    version="fp16_lr5e-5_batch${batch_total_tokens}_sft_en_TTS_trim_add_spkid_rm_libritts"
    ...
    base_config="--config /mnt/bn/huangzhiying-nas-speech2speech-volume1/code/samantha/recipes/valle/conf/valle_phoneme_fine_dynba_sft_add_spkid.yaml \
                --run_opts.train_meta_lst $train_meta_lst \
...
if [ ${stage} -eq 3 ];then
    ...
    for x in 'epoch=08-step=300-accu=31.31.ckpt' 'epoch=13-step=450-accu=32.86.ckpt' 'epoch=18-step=630-accu=37.26.ckpt' 'epoch=23-step=810-accu=39.75.ckpt' \
            'epoch=28-step=960-accu=40.73.ckpt' 'epoch=33-step=1140-accu=34.64.ckpt' 'epoch=38-step=1320-accu=43.69.ckpt' 'epoch=43-step=1470-accu=45.54.ckpt' \
            'epoch=48-step=1650-accu=39.90.ckpt' 'epoch=53-step=1830-accu=49.48.ckpt' 'epoch=58-step=1980-accu=44.60.ckpt' 'epoch=63-step=2160-accu=55.37.ckpt' \
            'epoch=68-step=2340-accu=51.31.ckpt' 'epoch=73-step=2490-accu=60.22.ckpt' 'epoch=78-step=2670-accu=63.25.ckpt' 'epoch=83-step=2850-accu=65.76.ckpt';do
        coarse_path=/mnt/bn/huangzhiying-nas-speech2speech-volume1/exp/samantha/bark/valle_phoneinput_1400_librilight_90-1492_mosnet2.8-A100-80GB-3-8/ar/fp16_lr4e-4_batch80000_sft_en_TTS_trim_add_spkid_rm_libritts_retry/checkpoints/$x
