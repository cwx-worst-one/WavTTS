...
train_meta_lst=/mnt/bd/huangzhiying-lq-valle-volume9/data/bytegen/valle/en_zh_TTS/meta_list_all.txt.add_metalen.shuf.train.fiter_90_1492.rm_libritts
valid_meta_lst=/mnt/bd/huangzhiying-lq-valle-volume9/data/bytegen/valle/en_zh_TTS/meta_list_all.txt.add_metalen.shuf.valid.fiter_90_1492.rm_libritts
...
if [ ${stage} -eq 1 ];then
    cd $work_dir
    log_name="ar"
    version="llama_fp16_lr3e-4_batch${batch_total_tokens}_90_1492_rm_libritts"
    ...
    base_config="--config recipes/bark/conf/llama/valle_coarse_llama.yaml \
                ...
                --llama_config.vocab_size 9027 \
                --llama_config.out_dim 9027 \
...
if [ ${stage} -eq 2 ];then
    cd $work_dir
    log_name="nar"
    version="gpt2_fp16_lr4e-4_batch${batch_total_tokens}_90_1492_rm_libritts"
    latest_ckpt_path=$log_dir/$log_name/$version/checkpoints/last.ckpt
    tensorboard --logdir $log_dir/${log_name} --port $ARNOLD_TENSORBOARD_CURRENT_PORT --bind_all &
...
if [ ${stage} -eq 3 ];then
    ...
    log_dir=/mnt/bn/huangzhiying-nas-speech2speech-volume1/exp/samantha/valle/valle_phoneinput_en_zh_TTS-A100-80GB-1-8
    ar_version=llama_fp16_lr3e-4_batch100000_90_1492_rm_libritts
    nar_version=gpt2_fp16_lr4e-4_batch15000_90_1492_rm_libritts
    coarse_path=${log_dir}/ar/${ar_version}/checkpoints/last.ckpt
    fine_path=${log_dir}/nar/${nar_version}/checkpoints/last.ckpt
    ...
    out_dir=$coarse_dir/../wav_infer/${spk}_${testset}_coarse_${coarse_stat}_fine_${fine_stat}
    [ ! -d $out_dir ] && mkdir -p $out_dir

    cp $coarse_path $out_dir/last_ar.ckpt
    cp $fine_path $out_dir/last_nar.ckpt

    metalst=/mnt/bn/huangzhiying-nas-speech2speech-volume1/code/samantha/recipes/valle/testset/${spk}/meta.lst.${testset}
    samantha_dir=$(cd $(dirname $0); cd ../../../; pwd)
    timestamp=$(date +%s)
...
if [ ${stage} -eq 4 ];then
    ...
    coarse_stat=$(python3 recipes/valle/utils/get_step_epoch_from_ckpt.py $coarse_path)
    fine_stat=$(python3 recipes/valle/utils/get_step_epoch_from_ckpt.py $fine_path)
    out_dir=$coarse_dir/../wav_infer/${spk}_${testset}_coarse_${coarse_stat}_fine_${fine_stat}_congjian
    [ ! -d $out_dir ] && mkdir -p $out_dir
    ...
    for rank in {0..7};do
        python3 recipes/valle/scripts/infer_valle_pre-nar_raw2.py \
            --ar_ckpt_path $coarse_path \
            --nar_ckpt_path $fine_path \
            --device cuda:$rank \
            --metaid_to_textid_path $samantha_dir/recipes/valle/datasets/dict/metaid_to_textid.json \
