...
dataname=libri_light_norm_trim-923h-fanqie_v1-1482h
...
train_meta_lst=/mnt/bn/huangzhiying-nas-speech2speech-volume1/data/bytegen/valle/${dataname}/meta_list_all.txt.add_metalen.shuf.train.fiter_90_1492
valid_meta_lst=/mnt/bn/huangzhiying-nas-speech2speech-volume1/data/bytegen/valle/${dataname}/meta_list_all.txt.add_metalen.shuf.valid.fiter_90_1492
...
if [ ${stage} -eq 2 ];then
    cd $work_dir
    log_name="nar"
    version="fp16_lr5e-5_gpt2_batch${batch_total_tokens}"
    latest_ckpt_path=$log_dir/$log_name/$version/checkpoints/last.ckpt
    tensorboard --logdir $log_dir/${log_name} --port $ARNOLD_TENSORBOARD_CURRENT_PORT --bind_all &
...
if [ ${stage} -eq 3 ];then
    cd $work_dir
    coarse_dir=/mnt/bn/huangzhiying-nas-speech2speech-volume1/exp/samantha/valle/valle_phoneinput_libri_light_norm_trim-923h-fanqie_v1-1482h-A100-80GB-4-8/ar/fp16_lr4e-4_batch140000/checkpoints/
    coarse_name="last.ckpt"
    coarse_path=$coarse_dir/$coarse_name
    coarse_dir=`dirname $coarse_path`

    fine_dir=/mnt/bn/huangzhiying-nas-speech2speech-volume1/exp/samantha/valle/valle_phoneinput_libri_light_norm_trim-923h-fanqie_v1-1482h-A100-80GB-2-8/nar/fp16_lr5e-5_gpt2_batch15000/checkpoints/
    fine_name="last.ckpt"
    fine_path=$fine_dir/$fine_name
    fine_dir=`dirname $fine_path`
