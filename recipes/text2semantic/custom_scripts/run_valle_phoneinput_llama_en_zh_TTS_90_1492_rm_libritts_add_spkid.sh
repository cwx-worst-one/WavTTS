...
train_meta_lst=/mnt/bd/huangzhiying-lq-valle-volume9/data/bytegen/valle/en_zh_TTS/meta_list_all.txt.add_metalen.shuf.train.fiter_90_1492.rm_libritts.add_spkid
valid_meta_lst=/mnt/bd/huangzhiying-lq-valle-volume9/data/bytegen/valle/en_zh_TTS/meta_list_all.txt.add_metalen.shuf.valid.fiter_90_1492.rm_libritts.add_spkid
...
if [ ${stage} -eq 1 ];then
    cd $work_dir
    log_name="ar"
    version="llama_fp16_lr3e-4_batch${batch_total_tokens}_90_1492_rm_libritts_add_spkid"
...
if [ ${stage} -eq 2 ];then
    cd $work_dir
    log_name="nar"
    version="gpt2_fp16_lr4e-4_batch${batch_total_tokens}_90_1492_rm_libritts_add_spkid"
    latest_ckpt_path=$log_dir/$log_name/$version/checkpoints/last.ckpt
    tensorboard --logdir $log_dir/${log_name} --port $ARNOLD_TENSORBOARD_CURRENT_PORT --bind_all &
...
if [ ${stage} -eq 3 ];then
    ...
    ar_version=llama_fp16_lr3e-4_batch100000_90_1492_rm_libritts_add_spkid
    ...
    fine_path=/mnt/bn/huangzhiying-nas-speech2speech-volume1/exp/samantha/valle/valle_phoneinput_en_zh_TTS-A800-80GB-1-8/nar/gpt2_fp16_lr4e-4_batch15000_90_1492_rm_libritts_add_spkid/checkpoints/last.ckpt
    ...
    for rank in {0..7};do
        python3 recipes/valle/scripts/infer_valle_pre-nar_raw2_spkid.py \
