...
train_meta_lst=/mnt/bn/huangzhiying-nas-speech2speech-volume1/data/bytegen/valle/en_TTS_all-libritts_clean_460-other_500/meta_list_all.txt.add_metalen.shuf.train.fiter_90_1492.wer0.1
valid_meta_lst=/mnt/bn/huangzhiying-nas-speech2speech-volume1/data/bytegen/valle/en_TTS_all-libritts_clean_460-other_500/meta_list_all.txt.add_metalen.shuf.valid.fiter_90_1492.wer0.1
...
if [ ${stage} -eq 1 ];then
    cd $work_dir
    log_name="ar"
    version="fp16_lr4e-4_batch${batch_total_tokens}_sft_en_TTS_all-libritts_clean_460-other_500"
...
if [ ${stage} -eq 3 ];then
    cd $work_dir
    coarse_dir=/mnt/bn/huangzhiying-nas-speech2speech-volume1/exp/samantha/valle/valle_phoneinput_1400_librilight_90-1492_mosnet2.8-A100-80GB-3-8/ar/fp16_lr4e-4_batch100000_sft_en_TTS_all-libritts_clean_460-other_500/checkpoints
    coarse_paths=`ls -l $coarse_dir/*.ckpt | awk '{print $9}' | grep -v 'last'`
    coarse_paths="epoch=42-step=109600-accu=42.10.ckpt epoch=53-step=110800-accu=36.65.ckpt epoch=63-step=112000-accu=38.44.ckpt epoch=73-step=113200-accu=45.49.ckpt epoch=84-step=114400-accu=48.78.ckpt epoch=94-step=115600-accu=42.20.ckpt"
    for coarse_path in $coarse_paths;do
    coarse_path=$coarse_dir/$coarse_path
...
if [ ${stage} -eq 4 ];then
    cd $work_dir
    coarse_dir=/mnt/bn/huangzhiying-nas-speech2speech-volume1/exp/samantha/valle/valle_phoneinput_1400_librilight_90-1492_mosnet2.8-A100-80GB-3-8/ar/fp16_lr4e-4_batch100000_sft_en_TTS_all-libritts_clean_460-other_500/checkpoints
    coarse_paths=`ls -l $coarse_dir/*.ckpt | awk '{print $9}' | grep -v 'last' | awk 'NR%6==1'`
    for coarse_path in $coarse_paths;do
    fine_path=/mnt/bn/jcong5/logs/sami_ai_models/valle_phoneinput_1400_librilight_90-1492--1-8/nar/fp16_lr5e-5_batch15000/checkpoints/last.ckpt
    coarse_dir=`dirname $coarse_path`
...
### youtube-librispeech_test_clean_minlen
if [ ${stage} -eq 6 ];then
    cd $work_dir
    coarse_dir=/mnt/bn/huangzhiying-nas-speech2speech-volume1/exp/samantha/valle/valle_phoneinput_1400_librilight_90-1492_mosnet2.8-A100-80GB-3-8/ar/fp16_lr4e-4_batch100000_sft_en_TTS_all-libritts_clean_460-other_500/checkpoints
    coarse_path=`ls -l $coarse_dir/*.ckpt | awk '{print $9}' | grep -v 'last' | tail -n 1`
    ...
    for retry in {0..4};do
    for pair in youtube-librispeech_test_clean;do
        testspk=`echo $pair | cut -d "-" -f 1`
        testset=`echo $pair | cut -d "-" -f 2`

        coarse_stat=$(python3 recipes/valle/utils/get_step_epoch_from_ckpt.py $coarse_path)
        fine_stat=$(python3 recipes/valle/utils/get_step_epoch_from_ckpt.py $fine_path)
        out_dir=$coarse_dir/../wav_infer/${testspk}_${testset}_coarse_${coarse_stat}_fine_${fine_stat}_minlen_retry${retry}
        [ ! -d $out_dir ] && mkdir -p $out_dir
