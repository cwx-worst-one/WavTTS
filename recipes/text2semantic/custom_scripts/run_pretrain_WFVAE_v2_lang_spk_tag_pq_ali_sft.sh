...
if [ ${stage} -eq 1 ];then
    log_name=sft_WFVAE_v2_labv3_punc_debug
    version=data_id${data_id}_bt${batch_total_tokens}_16A100_accu5_scr${spk_cfg_rate}_sp${use_sp}_ali${split_cfg_rate}
    ...
    base_config="--config recipes/text2semantic/conf/llama/vae_llama_ctiga_parquet_lang_spk_tag_ser_ali.yaml \
                ...
                --run_opts.split_cfg_rate ${split_cfg_rate}"
...
# infer seed1996
if [ ${stage} -eq 2 ];then
    hdfs_path=hdfs://haruna/home/byte_data_seed/lf_lq/speech/user/huangzhiying.92/exp/samantha_bigtts_ref_enc/text2semantic/pretrain_WFVAE_v2_labv3_punc_data_id534_bt14000_16A100_accu5_scr0.15_spFalse_ali0.5
    local_path=/mnt/bn/huangzhiying-nas-speech2speech-volume1/exp/samantha_bigtts_ref_enc/text2semantic/pretrain_WFVAE_v2_labv3_punc_data_id534_bt14000_16A100_accu5_scr0.15_spFalse_ali0.5
    ar_ckpt_paths=`hdfs dfs -ls $hdfs_path/checkpoints/ | awk '{print $8}' | grep -E '110000'`
    for ar_ckpt_path in $ar_ckpt_paths;do
        ...
        for infer_spk_name in tts_Lmand_Sinhouse-multistyle_P1/xudong_conv_1028;do
            ...
            for pair in flow_tts_zh-listen_all_new_zh_select_v2;do
                ...
                lang='en'
                if [ $pair == "flow_tts_zh-wer_rand20" -o $pair == "flow_tts_zh-yuqici" -o $pair == "flow_tts_zh-temp_demo20231104_v7" -o $pair == "flow_tts_zh-deploy_test" -o $pair == "flow_tts_zh-erhuayin" -o $pair == "flow_tts_zh-listen_all_new_zh_select_v2" ]; then
                    lang='zh'
                fi
                ...
                bash launch_gpu1.sh predict \
                    -c recipes/text2semantic/conf/llama/inference_wvae_icl_lang_spk_tag_ali.yaml \
                    --run_opts.meta_lst $metalst \
                    --run_opts.ckpt_path $ar_ckpt_path \
                    --run_opts.output_dir $out_wav_dir \
                    --run_opts.wvae_encoder hdfs://haruna/home/byte_data_seed/lf_lq/speech/user/wangxin.colin/ckpts/wvae_1.0/wavevae_encoder_%d.pt \
                    --run_opts.wvae_decoder hdfs://haruna/home/byte_data_seed/lf_lq/speech/user/wangxin.colin/ckpts/wvae_1.0/wavevae_decoder_%d.pt \
                    --run_opts.ar_model_name 'VAET2SLangSpkSerAliModule' \
