...
class TextToTacolabID:
    def __init__(self, metaid_to_textid_path, lab='old', symbol_sets=None, text2id_version='v1', use_sy=False, tacolab_version='oldv1') -> None:
    ...
    def __call__(self, text=None, tacolab_list=None):
        if self.text2id_version == 'v1':
            ...
            text_ids = np.asarray([self.text_converter_dict.get(str(x), self.text_converter_dict.get("oov")) for x in text_ids]).astype(np.int64)
    ...
    def get_lang_by_tacolab_list(self, tacolab_list):
        if len(tacolab_list[0].split('\t')) != 5:
            if tacolab_list[0] == 'phn\ttone\tws\tpwpp\tsentype\tword' or tacolab_list[0] == 'phn\ttone\tws\tpwpp\tsentype\tword\tunit':
    ...
    def get_labs_v3(self, tacolab_list, lang):
        labs = []
        if lang == 'zh':
            assert len(tacolab_list[0].split('\t')) == 7, (len(tacolab_list[0].split('\t')), tacolab_list[0])
            ...
            assert len(tacolab_list[0].split('\t')) == 7 or len(tacolab_list[0].split('\t')) == 6, (len(tacolab_list[0].split('\t')), tacolab_list[0])
            if tacolab_list[0] == 'phn\ttone\tws\tpwpp\tsentype\tword\tunit' or tacolab_list[0] == 'phn\ttone\tws\tpwpp\tsentype\tword':
                tacolab_list = tacolab_list[1:]
