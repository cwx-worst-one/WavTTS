...
from recipes.text2semantic.datasets import PhoneTokenizerWithAudioTokens
...
def main(args):
    ...
    for line in open(args.meta_file):
        for retry in range(0, 200):
            ...
            # min_pred_wav_id_len = int((pred_text_id_len * 0.7) * (wav_id.shape[0] / prompt_text_id.shape[0]))
            ...
            np.save(os.path.join(out_dir, "{}_retry{}.npy".format(utts[0], retry)), full_seq[0, :, unmask_len:].cpu().numpy())
            ...
            save_wav(wav_gen, os.path.join(out_dir, "{}_retry{}.wav".format(utts[0], retry)),sr=24000)
        exit()
