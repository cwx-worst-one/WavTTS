...
def generate_tacolabels_from_text_by_spk_utt_text(spk_utt_text_path, lab_output_dir_prefix, language='Chinese'):
    text_dict = parse_raw_text(spk_utt_text_path)
    if language == 'Chinese' or language == 'English':
        speaker = 'front_end'
    elif language == 'Japanese':
        speaker = 'front_end_jp'
    ...
    print("text_dict.items(): ", len(text_dict.items()))
    for file_id, text in tqdm(text_dict.items()):
        spk, utt = file_id.split('|')

        lab_output_dir = osp.join(lab_output_dir_prefix, spk, 'tacofrontend')
        os.makedirs(lab_output_dir, exist_ok=True)
        output_path = osp.join(lab_output_dir, f'{utt}.lab')
        if os.path.exists(output_path):
            continue
        lab_data, file_id, invoke_response = InvokeServer(file_id, text, speaker)
        if lab_data is None:
...
spk_utt_text_path = sys.argv[1]
taco_lab_dir_prefix = sys.argv[2]

# generate lab file from text_file and output to output_dir
sucess_labs = generate_tacolabels_from_text_by_spk_utt_text(
    spk_utt_text_path,
    taco_lab_dir_prefix,
    language='English'
)
