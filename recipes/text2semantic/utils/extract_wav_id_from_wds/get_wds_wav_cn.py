...
def load_pickle(res_file):
    return pickle.load(open(res_file, "rb"))

class WDSDataset(IterableDataset):
    def __init__(self, wds_lst, meta_lst, 
    ...
    def quality_check(self, sent):
        ...
        try:
            ...
            # if sent['speaker_similarity'] is None:
            #     speaker_similarity_min = 1.0
            # else:
            ...
            if rms_max >= -13 and snr >= 7 and speaker_similarity_min >= 0.6 and mos >= 4.2 and end_time - start_time < 25:
                ...
                if not (snr >= 7):
                    print("snr: ", snr)
                if not (speaker_similarity_min >= 0.6):
                    print("speaker_similarity_min: ", speaker_similarity_min)
                if not (mos >= 4.2):
                    print("mos: ", mos)
                if not (end_time - start_time < 25):
    ...
    def _split_wav(self, samples, sr=24000):
        ...
        for item in samples: # item表示一个20mins的片段: __key__, __url__, bin
            ...
            if 'sent' not in meta_data[key].keys():
                print(f"Warning: {key} missing detail info!")
                continue
                ...
                # if True:
                    try:
                        ...
                        if self.longform: # fake data for longform
                            ...
                            speaker_similarity_min = 1.0
                            # if data['speaker_similarity'] is None:
                            #     speaker_similarity_min = 1.0
                            # else:
...
if __name__ == "__main__":
    ...
    for line in tqdm(lines):
        ...
        for x in tqdm(dataset):
            ...
            cur_wav_dir = os.path.join(out_dir, 'wav_24k')
            os.makedirs(cur_wav_dir, exist_ok=True)
            cur_wav_path = os.path.join(cur_wav_dir, uttname + '.wav')
            save_wav(cur_wav, cur_wav_path)

            cur_text_dir = os.path.join(out_dir, 'text')
            os.makedirs(cur_text_dir, exist_ok=True)
            cur_text_path = os.path.join(cur_text_dir, uttname + '.txt')
            f_w = open(cur_text_path, 'w')
            f_w.write(uttname + '\t' + text + '\n')
            f_w.close()

            cur_labels_dir = os.path.join(out_dir, 'tacofrontend')
            os.makedirs(cur_labels_dir, exist_ok=True)
            cur_labels_path = os.path.join(cur_labels_dir, uttname + '.lab')
            f_w = open(cur_labels_path, 'w')
            f_w.write(labels)
            f_w.close()
