...
def trt_sound_stream_encoder_infer(input, context, inputs, outputs, bindings, stream):
    input = input.detach().cpu().numpy()
    # pad zero if audio length less than 5s
    if input.shape[2] < 5 * 24000:
        input = np.pad(input, ((0,0), (0,0), (0, 24000 * 5 - input.shape[2])), "constant")
    context.set_binding_shape(0, (input.shape[0], input.shape[1], input.shape[2]))
    inputs[0].host = np.array(input, dtype=np.float32, order="C")
    trt_out = common.do_inference_v2(context, bindings=bindings, inputs=inputs, outputs=outputs, stream=stream)

    valid_out = trt_out[0][:input.shape[0] * 256 * math.ceil(input.shape[2] / 300)]
    return torch.from_numpy(valid_out.reshape(input.shape[0], 256, -1))


def init_trt_sound_stream_encoder(file_path, cache_dir=None):
...
def init_sound_stream_quant(hpath, local_rank, cache_dir=None):
    if cache_dir is not None:
        os.makedirs(cache_dir, exist_ok=True)

    device = torch.device(f"cuda:{local_rank}")

    h_ss = f"{hpath}/ss_quant_{local_rank}.pt"
    local_path = f"{cache_dir}/ss_quant_{local_rank}.pt"

    if hpath.startswith("hdfs://"):
        if not os.path.exists(local_path):
            if not hh.get(h_ss, local_path):
                raise ConnectionError(f"Cannot retrieve file from {h_ss}.")
        return {"ss_quant": load_torch_script_module(local_path, device)}
    else:
        return {"ss_quant": load_torch_script_module(h_ss, device)}
...
if __name__ == "__main__":
    ...
    # vqgan_model_encoder = init_sound_stream_encoder("hdfs://harunava/home/byte_speech_sv/litang/2023-01-17_causal_x300_1024_6book_doubleG_export", local_rank, cache_dir=".cache_dir")["ss_enc"]
    # vqgan_model_encoder.eval()

    # # vqgan_model_decoder = init_sound_stream_decoder("hdfs://harunava/home/byte_speech_sv/litang/2023-01-17_causal_x300_1024_6book_doubleG_export", local_rank, cache_dir=".cache_dir")["ss_dec"]
    vqgan_model_decoder = init_sound_stream_decoder("./", local_rank, cache_dir=".cache_dir")["ss_dec"]
    vqgan_model_decoder.eval()

    # load encoder quant script
    vqgan_model_quant = init_sound_stream_quant("./", local_rank, cache_dir=".cache_dir")["ss_quant"]
    vqgan_model_quant.eval()

    # load encoder trt engine
    vqgan_model_encoder_trt = init_trt_sound_stream_encoder("./ss_encoder.engine", cache_dir=".cache_dir")

    context = vqgan_model_encoder_trt.create_execution_context()
    inputs, outputs, bindings, stream = common.allocate_buffers(vqgan_model_encoder_trt, 0)
    ...
    for line in tqdm(lines):
        ...
        for x in tqdm(torch.utils.data.DataLoader(dataset, num_workers=args.data_workers, batch_size=40, collate_fn=DummyCollator())):
            ...
            # wav_ids_ori = torch.stack(vqgan_model_encoder(cur_wavs[:2, :].to(f"cuda:{local_rank}"))[2], dim=2)
            # import pdb; pdb.set_trace()
            enc_out = trt_sound_stream_encoder_infer(cur_wavs.unsqueeze(1), context, inputs, outputs, bindings, stream)
            y, _, quant_idx = vqgan_model_quant(enc_out.to(f"cuda:{local_rank}"))
            wav_ids_ori = torch.stack(quant_idx, dim=2)
            # print(cur_wavs)
            # print(wav_ids_ori_torch)
            # print(wav_ids_ori)
            # wav_ids_ori = get_codec_codes(requires, cur_wavs)
            #wav_ids = torch.stack(wav[2], dim=2).cpu().numpy()
            ...
            for i in range(batch_size):
                ...
                # wav1 = cur_wavs[i:i+1,:].detach().cpu().numpy().transpose()
                # save_wav(wav1, os.path.join(uttnames[i] + '_ori.wav'))
                writer.write({
                    "__key__": uttnames[i],
