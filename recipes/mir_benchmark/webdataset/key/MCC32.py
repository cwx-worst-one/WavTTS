...
_HDFS_DIRS = {
    ...
    "SheetDoctor/labels/MCC3rd_keys.zip",
}
...
class MCC32Dataset(AbstractWebDatasetGeneratorBasedBuilder):
    ...
    def _split_data(self, data_dir):
        keys = []
        for k in os.listdir(os.path.join(data_dir, "MCC3rd_keys")):
            k = k.split(".txt")[0]
            if k != ".DS_Store":
                keys.append(k)
    ...
    def _load_example(self, _id):
        ...
        # read annotations
        with open(os.path.join(self.label_dir, "MCC3rd_keys", _id + ".txt"), "r") as f:
            ss = f.read().splitlines()
            for i, line in enumerate(ss):
                s = line.split("\t")

                start = float(s[0])
                end = float(len(np_audio) / self.SAMPLING_RATE)
