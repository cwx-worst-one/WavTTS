...
_HDFS_DIRS = {
    ...
    "SheetDoctor/labels/MCC126_keys.zip",
}


class MCC126Dataset(AbstractWebDatasetGeneratorBasedBuilder):
    """
    MCC126 Dataset with fixed labels
    """

    SAMPLING_RATE = 24000
    DATASET_NAME = "MCC126_key_24kHz"
    ...
    def _split_data(self, data_dir):
        keys = []
        for k in os.listdir(os.path.join(data_dir, "hum2song_keys")):
            k = k.split(".txt")[0]
            if k != ".DS_Store":
                keys.append(k)
    ...
    def _load_example(self, _id):
        ...
        # read annotations
        with open(
            os.path.join(self.label_dir, "hum2song_keys", _id + ".txt"), "r"
        ) as f:
