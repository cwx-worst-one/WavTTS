"""
Isophonic Key Recognition Dataset.
"""
...
_HDFS_DIRS = {
    ...
    "SheetDoctor/labels/isophonic_keys.zip",
}


class IsophonicdDataset(AbstractWebDatasetGeneratorBasedBuilder):
    """
    Isophonic Dataset with fixed labels
    """

    SAMPLING_RATE = 24000
    DATASET_NAME = "isophonic_key_24kHz"
    ...
    def _split_data(self, data_dir):
        keys = []
        for k in os.listdir(os.path.join(data_dir, "isophonic_keys")):
            k = k.split(".txt")[0]
            if k != ".DS_Store":
                keys.append(k)
    ...
    def _load_example(self, _id):
        ...
        # read annotations
        with open(
            os.path.join(self.label_dir, "isophonic_keys", _id + ".txt"), "r"
        ) as f:
        ...
        audio_path = os.path.join(self.audio_dir, _id + ".mp3")
