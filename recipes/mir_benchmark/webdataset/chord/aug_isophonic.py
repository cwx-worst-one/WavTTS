"""
Isophonic Chord Recognition Dataset.
"""
...
_HDFS_DIRS = {
    ...
    "SheetDoctor/raw_data/isophonic/audio",
    "chord_labels": "hdfs://harunava/home/byte_speech_sv/data/"
    "SheetDoctor/labels/isophonic_chords.zip",
}


class IsophonicDataset(AbstractWebDatasetGeneratorBasedBuilder):
    """
    Isophonic Dataset
    """

    SAMPLING_RATE = 24000
    DATASET_NAME = "isophonic_chord_24kHz"
    ...
    def _split_data(self, data_dir):
        keys = []
        for k in os.listdir(os.path.join(data_dir, "isophonic_chords")):
            if k[0] != ".":
                k = k.split(".txt")[0]
    ...
    def _load_example(self, key):
        ...
        with open(
            os.path.join(
                self.label_dir, "isophonic_chords", key.replace("$$", ".") + ".txt"
            )
        ) as f:
            chords = f.readlines()
            intervals = []
        ...
        np_audio, _ = librosa.load(audio_path, sr=IsophonicDataset.SAMPLING_RATE)
        return {
            "dataset.txt": IsophonicDataset.DATASET_NAME,
            "__key__": key,
            "intervals.pickle": intervals,
            "chords.pickle": symbols,
            "audio.npy": np_audio.astype(IsophonicDataset.AUDIO_NUMPY_DTYPE),
        }
