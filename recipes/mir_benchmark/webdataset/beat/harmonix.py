...
_HDFS_DIRS = {
    ...
    "SheetDoctor/raw_data/harmonixset/new_mp3s.zip",
    "beat_labels": "hdfs://harunava/home/byte_speech_sv/data/"
    "SheetDoctor/labels/harmonix_truth_beats.zip",
}


class HarmonixDataset(AbstractWebDatasetGeneratorBasedBuilder):
    """
    Harmonix Dataset
    """

    SAMPLING_RATE = 24000
    DATASET_NAME = "harmonix_beat_24kHz"
    ...
    def _split_data(self, data_dir):
        keys = []
        for k in os.listdir(os.path.join(data_dir, "harmonix_truth_beats")):
            k = k.split(".")[0]
            audio_fn = os.path.join(self.audio_dir, "mp3s", k + ".mp3")
    ...
    def _load_example(self, key):
        ...
        with open(
            os.path.join(
                self.label_dir, "harmonix_truth_beats", key.replace("$$", ".") + ".txt"
            )
        ) as f:
            beats = f.readlines()
        ...
        audio_path = os.path.join(
            self.audio_dir, "mp3s", key.replace("$$", ".") + ".mp3"
        )
        np_audio, _ = librosa.load(audio_path, sr=HarmonixDataset.SAMPLING_RATE)
        return {
            "dataset.txt": HarmonixDataset.DATASET_NAME,
            "__key__": key,
            "beats.pickle": beats,
            "audio.npy": np_audio.astype(HarmonixDataset.AUDIO_NUMPY_DTYPE),
        }
