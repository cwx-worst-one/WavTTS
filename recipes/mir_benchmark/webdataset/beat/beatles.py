...
_HDFS_DIRS = {
    ...
    "SheetDoctor/raw_data/beatles/audio",
    "beat_labels": "hdfs://harunava/home/byte_speech_sv/data/"
    "SheetDoctor/labels/beatles_truth_beats.zip",
}


class BeatlesDataset(AbstractWebDatasetGeneratorBasedBuilder):
    """
    Beatles Dataset
    """

    SAMPLING_RATE = 24000
    DATASET_NAME = "beatles_beat_24kHz"
    ...
    def _split_data(self, data_dir):
        keys = []
        for k in os.listdir(os.path.join(data_dir, "beatles_truth_beats")):
            k = k.split(".txt")[0]
            audio_fn = os.path.join(self.audio_dir, k + ".wav")
    ...
    def _load_example(self, key):
        ...
        with open(
            os.path.join(
                self.label_dir, "beatles_truth_beats", key.replace("$$", ".") + ".txt"
            )
        ) as f:
            beats = f.readlines()
            beats = [line.rstrip().split() for line in beats]
            beats = [[float(x[0]), int(x[1])] for x in beats]
        audio_path = os.path.join(self.audio_dir, key.replace("$$", ".") + ".wav")
        np_audio, _ = librosa.load(audio_path, sr=BeatlesDataset.SAMPLING_RATE)
        return {
            "dataset.txt": BeatlesDataset.DATASET_NAME,
            "__key__": key,
            "beats.pickle": beats,
            "audio.npy": np_audio.astype(BeatlesDataset.AUDIO_NUMPY_DTYPE),
        }
