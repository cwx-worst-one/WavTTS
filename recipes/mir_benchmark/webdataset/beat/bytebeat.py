...
_HDFS_DIRS = {
    ...
    "SheetDoctor/raw_data/bytedance_clip/audio190",
    "beat_labels": "hdfs://harunava/home/byte_speech_sv/data/"
    "SheetDoctor/labels/bytebeat_beats.zip",
}


class ByteBeatDataset(AbstractWebDatasetGeneratorBasedBuilder):
    """
    ByteBeat Dataset
    """

    SAMPLING_RATE = 24000
    DATASET_NAME = "bytebeat_beat_24kHz"
    ...
    def _split_data(self, data_dir):
        keys = []
        for k in os.listdir(os.path.join(data_dir, "template_beats0809")):
            k = k.split(".txt")[0]
    ...
    def _load_example(self, key):
        ...
        with open(
            os.path.join(
                self.label_dir, "template_beats0809", key.replace("$$", ".") + ".txt"
            )
        ) as f:
            beats = f.readlines()
        ...
        np_audio, _ = librosa.load(audio_path, sr=ByteBeatDataset.SAMPLING_RATE)
        return {
            "dataset.txt": ByteBeatDataset.DATASET_NAME,
            "__key__": key,
            "beats.pickle": beats,
            "audio.npy": np_audio.astype(ByteBeatDataset.AUDIO_NUMPY_DTYPE),
        }
