...
_HDFS_DIRS = {
    ...
    "SheetDoctor/raw_data/ballroom/audio",
    "beat_labels": "hdfs://harunava/home/byte_speech_sv/data/"
    "SheetDoctor/labels/ballroom_truth_beats.zip",
}


class BallroomDataset(AbstractWebDatasetGeneratorBasedBuilder):
    """
    Ballroom Dataset
    """

    SAMPLING_RATE = 24000
    DATASET_NAME = "ballroom_beat_24kHz"
    ...
    def _split_data(self, data_dir):
        keys = []
        for k in os.listdir(os.path.join(data_dir, "ballroom_truth_beats")):
            k = k.split(".txt")[0]
    ...
    def _load_example(self, key):
        ...
        with open(
            os.path.join(
                self.label_dir, "ballroom_truth_beats", key.replace("$$", ".") + ".txt"
            )
        ) as f:
            beats = f.readlines()
        ...
        np_audio, _ = librosa.load(audio_path, sr=BallroomDataset.SAMPLING_RATE)
        return {
            "dataset.txt": BallroomDataset.DATASET_NAME,
            "__key__": key,
            "beats.pickle": beats,
            "audio.npy": np_audio.astype(BallroomDataset.AUDIO_NUMPY_DTYPE),
        }
