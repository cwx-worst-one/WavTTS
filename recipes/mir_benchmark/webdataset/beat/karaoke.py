...
class KaraokeBeatDataset(AbstractWebDatasetGeneratorBasedBuilder):
    """
    Karaoke Beat Dataset
    """

    SAMPLING_RATE = 24000
    DATASET_NAME = "karaoke_beat_24kHz"
    ...
    def _split_data(self, data_dir):
        keys = []
        for k in os.listdir(os.path.join(data_dir, "karaoke_truth_beats")):
            k = k.split(".txt")[0]
            audio_fn = os.path.join(self.audio_dir, "full", k[1:] + "_full.mp3")
    ...
    def _load_example(self, key):
        ...
        with open(
            os.path.join(
                self.label_dir, "karaoke_truth_beats", key.replace("$$", ".") + ".txt"
            )
        ) as f:
            beats = f.readlines()
        ...
        audio_path = os.path.join(
            self.audio_dir, "full", key[1:].replace("$$", ".") + "_full.mp3"
        )
        np_audio, _ = librosa.load(audio_path, sr=KaraokeBeatDataset.SAMPLING_RATE)
        return {
            "dataset.txt": KaraokeBeatDataset.DATASET_NAME,
            "__key__": key,
            "beats.pickle": beats,
            "audio.npy": np_audio.astype(KaraokeBeatDataset.AUDIO_NUMPY_DTYPE),
        }
