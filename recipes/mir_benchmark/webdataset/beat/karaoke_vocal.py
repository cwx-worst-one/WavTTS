...
_HDFS_DIRS = {
    ...
    "karaoke_preview/processed/lead_vocal.zip",
...
class KaraokeVocalDataset(AbstractWebDatasetGeneratorBasedBuilder):
    ...
    SAMPLING_RATE = 24000
    DATASET_NAME = "karaoke_beat_vocal_24kHz"
    ...
    def _load_example(self, _id):
        audio_path = os.path.join(self.audio_dir, "lead_vocal", _id[2:] + ".mp3")
        ...
        # read annotations
        with open(
            os.path.join(self.label_dir, "karavocal_truth_beats", _id + ".txt"), "r"
        ) as f:
            beats = f.readlines()
