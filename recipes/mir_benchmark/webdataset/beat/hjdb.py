...
_HDFS_DIRS = {
    ...
    "SheetDoctor/raw_data/hjdb/HJDB_WAV",
    "beat_labels": "hdfs://harunava/home/byte_speech_sv/data/"
    "SheetDoctor/labels/hjdb_beats.zip",
}


class HJDBDataset(AbstractWebDatasetGeneratorBasedBuilder):
    """
    HJDB Dataset
    """

    SAMPLING_RATE = 24000
    DATASET_NAME = "hjdb_beat_24kHz"
    ...
    def _split_data(self, data_dir):
        keys = []
        for k in os.listdir(os.path.join(data_dir, "hjdb_beats")):
            k = k.split(".txt")[0]
    ...
    def _load_example(self, key):
        # read annotations
        with open(
            os.path.join(self.label_dir, "hjdb_beats", key.replace("$$", ".") + ".txt")
        ) as f:
            beats = f.readlines()
            beats = [[float(line.rstrip().split()[0]), 0] for line in beats]
        audio_path = os.path.join(self.audio_dir, key.replace("$$", ".") + ".wav")
        np_audio, _ = librosa.load(audio_path, sr=HJDBDataset.SAMPLING_RATE)
        return {
            "dataset.txt": HJDBDataset.DATASET_NAME,
            "__key__": key,
            "beats.pickle": beats,
            "audio.npy": np_audio.astype(HJDBDataset.AUDIO_NUMPY_DTYPE),
        }
