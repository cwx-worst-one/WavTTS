...
_HDFS_DIRS = {
    ...
    "SheetDoctor/raw_data/smc/audio",
    "beat_labels": "hdfs://harunava/home/byte_speech_sv/data/"
    "SheetDoctor/labels/smc_truth_beats.zip",
}


class SMCDataset(AbstractWebDatasetGeneratorBasedBuilder):
    """
    SMC Dataset
    """

    SAMPLING_RATE = 24000
    DATASET_NAME = "smc_beat_24kHz"
    ...
    def _split_data(self, data_dir):
        keys = []
        for k in os.listdir(os.path.join(data_dir, "smc_truth_beats")):
            k = k.split(".txt")[0]
    ...
    def _load_example(self, key):
        ...
        with open(
            os.path.join(
                self.label_dir, "smc_truth_beats", key.replace("$$", ".") + ".txt"
            )
        ) as f:
            beats = f.readlines()
            beats = [[float(line.rstrip()), 0] for line in beats]
        audio_path = os.path.join(self.audio_dir, key.replace("$$", ".") + ".mp3")
        np_audio, _ = librosa.load(audio_path, sr=SMCDataset.SAMPLING_RATE)
        return {
            "dataset.txt": SMCDataset.DATASET_NAME,
            "__key__": key,
            "beats.pickle": beats,
            "audio.npy": np_audio.astype(SMCDataset.AUDIO_NUMPY_DTYPE),
        }
