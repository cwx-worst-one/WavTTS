...
class AMP_PHA_Spectrum(nn.Module):
    def __init__(self, n_fft, hop_length, win_length, audio_channels=1, normalize_spec=False):
        super().__init__()
        self.n_fft = n_fft
        self.hop_length = hop_length
        self.win_length = win_length
    ...
    def forward(self, y):
        ...
        specs = amp_pha_spectrum(y, self.n_fft, self.hop_length, self.win_length, normalize_spec=self.normalize_spec)
...
def amp_pha_spectrum(y, n_fft, hop_length, win_length, normalize_spec=False):
    hann_window=torch.hann_window(win_length).to(y.device)
    stft_spec=torch.stft(y, n_fft, hop_length=hop_length, win_length=win_length, window=hann_window,center=True, return_complex=True) #[batch_size, n_fft//2+1, frames, 2]
