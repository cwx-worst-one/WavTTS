...
# from recipes.sacodec.models.components.loss import feature_loss, generator_loss, discriminator_loss
...
class SACodecModule(pl.LightningModule):
    def __init__(
        ...
        **kwargs
        ...
        self.multiresddisc = MultiResolutionDiscriminator(in_channels=audio_channels)
    ...
    def forward(self, audio_input, **kwargs):
        ## Only to masking on non-discriminator steps
        logamp, pha, rea, imag = self.encoder.audio_to_spec(audio_input)

        latents, loss_kl = self.encoder(logamp, pha)
    ...
    def reconstruct_audio(self, audio: torch.Tensor):
        ...
        latent, loss_kl = self.encoder(logamp, pha)
    ...
    def training_step(self, batch, batch_idx, **kwargs):
        ...
        if self.global_step >= self.hparams.pretrain_mel_steps and self.global_step % 2 == 0:
