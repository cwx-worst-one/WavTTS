...
def main(test_step_size=20000):
    ...
    for idx, batch_data in enumerate(dataloader):
        ...
        if idx == 0:
            ...
            total_time_used += cur_time
            total_batch_count += 1
        logging.error(f"rank:{rank}, world_size:{world_size} iter:{idx}, time:{cur_time}, bsz:{batch_data['src'].shape}")
        last_time = time.time()
        if idx > test_step_size:
            break
    del dataloader
    logging.error(f"rank:{rank}, world_size:{world_size} get first batch use: {first_time}")
    logging.error(f"rank:{rank}, world_size:{world_size} get {total_batch_count} batchs use: {total_time_used}s, speed: {total_time_used/total_batch_count} sec per batch")


# cmd: python3 tests/benchmarks/dataloader/test_base_dataloader.py
# --config configs/asr/librispeech_rnnt_mix.py

# cmd: mpirun -np $ARNOLD_WORKER_GPU python3 tests/benchmarks/dataloader/test_base_dataloader.py
# --config configs/asr/streaming_conformer_rnnt_input_10k.py
