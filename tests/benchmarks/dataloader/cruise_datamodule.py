...
from core.dataset.batching import setup_bucket
...
class ValleProcessor:
    def __init__(self,
                 ...
                 max_token_count=140000,
                 tokenizer_pad=0.,
                 block_sparse=False):
        self.batcher = batcher
        self.max_batch_size = max_token_count
        self.hp = DotDict({
    ...
    def get_buffer_length(self, length, enable=True, max_tokens=None):
    ...
    def transform(self, sample):
        ...
        item = dict(
            ...
            full_seq=full_seq,
            length=self.get_buffer_length(seq.shape[0], self.enable_buffer_length, self.max_token_count - 1)
        )
    ...
    def batch_transform(self, batch_datas, collate_last=False):
        for item in batch_datas:
            batch_data = self.batcher.collate_batch(item, self.max_batch_size)
...
train_file_list = dict()
train_file_list['data_root'] = 'hdfs://haruna/home/byte_speech_sv/user/yanlin.chn/bigTTS/data/webdataset_out'
train_file_list['file_prefix'] = 'shard_00*/*.parquet'
class ValleDataModule(CruiseDataModule):
    ...
    def __init__(self,
                 ...
                 bucket_schedule='100,200,300,400,500,600,700,800,900,1000,1200,1493',
                 drop_last=False,
                 shuffle=True,
    ...
    def setup(self):
        ...
        for cfg in self.hparams.train_file_list:
            data_root, file_prefix, file_num = cfg['data_root'], cfg['file_prefix'], cfg.get('file_num', None)
    ...
    def train_dataloader(self):
        ...
        batch_strategy = self.hparams.get('batch_strategy', 'BucketBatching')
        batcher = setup_bucket(self.hparams, batch_strategy)
...
if __name__ == "__main__":
    ...
    data_module = ValleDataModule(**kwargs['data'])
    data_module.setup()
