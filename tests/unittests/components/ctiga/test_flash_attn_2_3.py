...
def non_zero_value(value):
    if np.isnan(value) or value==0.0:
        return max(2e-2, max_numerical_error())
    return value


def max_numerical_error():
    ref = torch.randn((1024,))
    return (ref - ref * 3.0 / 3.0).abs().max().item()


def print_tensor(tensor, comment):
    print(f"{comment} {tensor.shape} {tensor.dtype}")
    ...
    for bidx in range(b):
        for hidx in range(h):
            for sidx in range(s):
                for didx in range(d):
                    v = tensor[bidx, hidx, sidx, didx]

                    print(f"{v.int() if v.dtype==torch.bool else v}", end=" ")
                print("")
...
def generate_random_padding_mask(max_seqlen, batch_size, device, mode="random"):
    ...
    if mode == "full":
        ...
        lengths[random.randint(0, batch_size-1)] = max_seqlen
    elif mode == "third":
        lengths = torch.randint(
...
@pytest.mark.parametrize("seqlen", [97, 128, 200, 384, 246, 309, 512, 677, 768, 1024, 1025, 2048])
@pytest.mark.parametrize("dropout_p", [0.0, 0.17])
def test_flash_attn_qkvpacked(seqlen, d, dropout_p, causal, local, window_type, dtype):
    print()
    if causal and local:
        pytest.skip()
    if not local and window_type in WINDOW_MASK_TYPES:
        pytest.skip()
    if (
        seqlen >= 2048
    ...
    if not local:
        ...
        if window_type == BLOCKWISE_WINDOW_MASK:
            # window_size = [-1, int(math.pow(2,random.randint(0, min(6, seqlen_k))))]
            window_size = [-1, random.randint(2, min(16, seqlen))]
        else:
            window_size = [random.randint(-1, min(16, seqlen)) for _ in range(2)]
            if all([x>0 for x in window_size]) < 0:
                window_size[random.randint(0,1)] = random.randint(0, min(16, seqlen))

    print("seqlen={}, window_type={}, window_size={}".format(seqlen, window_type, window_size))

    qkv = torch.randn(
        batch_size, seqlen, 3, nheads, d, device=device, dtype=dtype, requires_grad=True
    )
    out, lse, _, S_dmask = flash_attn_qkvpacked_func(
        qkv,
        dropout_p,
        causal=causal,
        window_size=window_size,
    ...
    if d <= MAX_HEADDIM_SM8x or (is_sm80 or is_sm90):
        ...
        print(f"isnan out={torch.any(torch.isnan(out))} out_ref={torch.any(torch.isnan(out_ref))} out_pt={torch.any(torch.isnan(out_pt))}")
        print(f"isnan dqkv={torch.any(torch.isnan(dqkv))} dqkv_ref={torch.any(torch.isnan(dqkv_ref))} dqkv_pt={torch.any(torch.isnan(dqkv_pt))}")    
    ...
    assert (out - out_ref).abs().max().item() <= 2 * non_zero_value((
        out_pt - out_ref
    ).abs().max().item())

    if dropout_p > 0.0:
        assert (attn - attn_ref).abs().max().item() <= 2 * non_zero_value((
            attn_pt - attn_ref
        ).abs().max().item())
        assert abs(dropout_fraction - dropout_p) <= (0.01 if not local else 0.025)

    if d <= MAX_HEADDIM_SM8x or (is_sm80 or is_sm90):
        assert (dqkv - dqkv_ref).abs().max().item() <= 2 * non_zero_value((
            dqkv_pt - dqkv_ref
        ).abs().max().item())
...
@pytest.mark.parametrize("causal", [False, True])
@pytest.mark.parametrize("d", [32, 40, 59, 64, 80, 96, 111, 128, 160, 192, 224, 256])
@pytest.mark.parametrize("seqlen", [31, 97, 128, 200, 257, 384, 444, 512, 768, 1025, 2048])
@pytest.mark.parametrize("dropout_p", [0.0, 0.17])
def test_flash_attn_varlen_qkvpacked(
    seqlen, d, dropout_p, causal, local, window_type, dtype
):
    print()
    if causal and local:
        pytest.skip()
    ...
    out_unpad, sm_lse, _, S_dmask = flash_attn_varlen_qkvpacked_func(
        qkv_unpad,
        cu_seqlens,
        max_seqlen,
        dropout_p,
...
@pytest.mark.parametrize("local", [False, True])
@pytest.mark.parametrize("causal", [False, True])
@pytest.mark.parametrize("d", [32, 40, 59, 64, 96, 111, 128, 133, 160, 192, 224, 256])
...
def test_flash_attn_output(
    ...
    if not local:
        ...
        if window_type == BLOCKWISE_WINDOW_MASK:
            # window_size = [-1, int(math.pow(2,random.randint(0, min(6, seqlen_k))))]
            window_size = [-1, random.randint(2, min(16, seqlen_k))]
        else:
            window_size = [random.randint(-1, min(16, seqlen_k)) for _ in range(2)]
            if all([x>0 for x in window_size]) < 0:
                window_size[random.randint(0,1)] = random.randint(0, min(16, seqlen_k))

    print("seqlen={}, window_type={} window_size={}".format((seqlen_q,seqlen_k),window_type,window_size))
    q = torch.randn(
    ...
    if kvpacked:
        out, lse, _, S_dmask = flash_attn_kvpacked_func(
            q,
            kv,
            dropout_p,
            causal=causal,
        ...
        out, lse, _, S_dmask = flash_attn_func(
            q,
            k,
            v,
            dropout_p,
    ...
    if dropout_p > 0.0:
        ...
        assert abs(dropout_fraction - dropout_p) <= (0.01 if not local else max(0.05, 2 * 1/(seqlen_k*seqlen_q)*batch_size*nheads))

    if d <= MAX_HEADDIM_SM8x or (is_sm80 or is_sm90):
        assert (dq - dq_ref).abs().max().item() <= 3 * non_zero_value((
            dq_pt - dq_ref
        ).abs().max().item())
        assert (dk - dk_ref).abs().max().item() <= 3 * non_zero_value((
            dk_pt - dk_ref
        ).abs().max().item())
        assert (dv - dv_ref).abs().max().item() <= 3 * non_zero_value((
            dv_pt - dv_ref
        ).abs().max().item())


# FIXME: barely case will failed (attntion diff 'nan' when dropout>0)
@pytest.mark.skip
@pytest.mark.parametrize("kvpacked", [True, False])
@pytest.mark.parametrize("dtype",[torch.float16, torch.bfloat16])
@pytest.mark.parametrize("mha_type", ["mha", "mqa", "gqa"])
...
def test_flash_attn_varlen_output(
    ...
    query_padding_mask =None if seqlen_q==1 else  generate_random_padding_mask(
        seqlen_q, batch_size, device, mode="random"
    )
    key_padding_mask =None if seqlen_k==1 else generate_random_padding_mask(
    ...
    if kvpacked:
        ...
        out_unpad, sm_lse, _, S_dmask = flash_attn_varlen_kvpacked_func(
            q_unpad,
            kv_unpad,
            cu_seqlens_q,
            cu_seqlens_k,
        ...
        out_unpad, sm_lse, _, S_dmask = flash_attn_varlen_func(
            q_unpad,
            k_unpad,
            v_unpad,
            cu_seqlens_q,
    ...
    if dropout_p > 0.0:
        try:
            ...
            print(f"FIXME!!!!!\n{e}\n")
        assert abs(dropout_fraction - dropout_p) <= (0.01 if not local else max(0.03, 2 * 1/(seqlen_k*seqlen_q)*batch_size*nheads))
