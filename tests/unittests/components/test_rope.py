...
def test_rope(dtype, use_complex, orig_llama) -> None:  # noqa
    ...
    rotary = RotaryEmbedding(dim_model=n_embd // n_head, use_complex=use_complex)
    rotary._update_cos_sin_tables(x, seq_dimension=-2)

    freqs_cis = orig_llama.precompute_freqs_cis(n_embd // n_head, seq_len)

    torch.testing.assert_close(freqs_cis, torch.view_as_complex(rotary._cache.squeeze()))

    x_rope = apply_rotary_pos_emb(x, rotary._cache)
