...
if _triton_available:
    try:
        ...
        from samantha.byteformers.triton.fused_linear_layer import FusedLinear
        from samantha.byteformers.triton.k_activations import get_triton_activation_index
        from samantha.byteformers.triton.k_fused_matmul_fw import fused_matmul
...
@pytest.mark.skip("TODO: This is currently broken, fix ASAP")
def test_fused_matmul(shape, dtype):
    ...
    torch.testing.assert_close(
        res_torch, res_triton, atol=1e-3, rtol=1e-3, msg="Fused matmul broken"
    )
...
@pytest.mark.parametrize("activation", [None] + [a.value for a in Activation])
@pytest.mark.parametrize("shape", SHAPES)
