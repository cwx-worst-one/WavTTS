...
try:
    ...
    from samantha.byteformers.triton.dropout import FusedDropoutBias
    from samantha.byteformers.triton.dropout import dropout as triton_dropout
    from samantha.byteformers.triton.utils import gpu_capabilities_older_than_70

    _triton_available = True
except ImportError as e:
    logging.warning(
        f"Triton is not available, some optimizations will not be tested: {e}"
    )
...
def test_dropout(shape, amp, bias, p):
    ...
    with autocast(enabled=amp):
        ...
        # Check that the drops are different for every row
        # (could catch broken seeds per row)
