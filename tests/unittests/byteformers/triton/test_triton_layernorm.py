...
try:
    from samantha.byteformers import is_triton_available
...
def test_layernorm_parity(shape, amp, device):
    ...
    torch_layernorm = torch.nn.LayerNorm(X.shape[-1], eps=eps).to(device)
    triton_layernorm = FusedLayerNorm(X.shape[-1], affine=True, bias=True, eps=eps).to(
        device
    )
...
def test_no_contiguous(dtype, device):
    ...
    triton_layernorm = FusedLayerNorm(X.shape[-1], bias=True).to(
